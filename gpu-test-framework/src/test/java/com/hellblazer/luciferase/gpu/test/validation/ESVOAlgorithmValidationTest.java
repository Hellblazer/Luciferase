...
import com.hellblazer.luciferase.gpu.esvo.ESVOCPUTraversal;
...
public class ESVOAlgorithmValidationTest extends CICompatibleGPUTest {
    ...
    void testCPUvsGPUTraversal() {
        ...
        IntersectionResult[] cpuResults = ESVOCPUTraversal.traverseRays(
            rays, octree, sceneMin, sceneMax, octreeDepth
        );
        ...
        for (int i = 0; i < rays.length; i++) {
            IntersectionResult cpu = cpuResults[i];
    ...
    void testPerformanceScaling() {
        ...
        for (int size : sizes) {
            ...
            // CPU timing
            long cpuStart = System.nanoTime();
            ESVOCPUTraversal.traverseRays(rays, octree, sceneMin, sceneMax, 6);
            long cpuTime = System.nanoTime() - cpuStart;
