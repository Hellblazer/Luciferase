...
public class ESVOOpenCLValidatorTest extends CICompatibleGPUTest {
        ...
        withGPUContext(context -> {
            ...
            for (int nodeCount : nodeCounts) {
                for (int rayCount : rayCounts) {
                    ...
                    IntersectionResult[] results = executeRayTraversal(
                        context, nodes, rays, createDefaultParams());
    ...
    private IntersectionResult[] executeRayTraversal(
            GPUContext context, OctreeNode[] nodes, Ray[] rays, TraversalParams params) {
        ...
        try {
            ...
            // Read results
            IntersectionResult[] results = ESVODataStructures.BufferUtils.readResults(
    ...
    private int[] executeShadowRayTest(GPUContext context, OctreeNode[] nodes, Ray[] rays) {
        try {
            String kernelSource = loadKernelSource();
            long program = ESVODataStructures.BufferUtils.compileProgram(context.context, kernelSource);
