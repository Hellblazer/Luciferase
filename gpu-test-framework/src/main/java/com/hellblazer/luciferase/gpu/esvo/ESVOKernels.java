package com.hellblazer.luciferase.gpu.esvo;
...
public class ESVOKernels {
    ...
    public static final String OPENCL_RAY_TRAVERSAL = """
        ...
        typedef struct {
            uint parent;
            uint childMask;  // 8-bit mask indicating which children exist
            uint childPtr;   // Pointer to first child
            uint voxelData;  // Material/color data
        } OctreeNode;
            ...
            const float3 sceneMax)
        {
            ...
            while (stackPtr > 0) {
                ...
                if (node.childMask == 0 || current.scale >= maxDepth) {
                    if (node.voxelData != 0 && current.tEntry < closestHit) {
                        closestHit = current.tEntry;
                        hitVoxel = node.voxelData;
                ...
                for (int i = 7; i >= 0; i--) {
                    ...
                    if (intersectAABB(ray, childBox, &childTEntry, &childTExit)) {
                        if (childTEntry < closestHit && stackPtr < 31) {
                            uint childIdx = node.childPtr + popcount(node.childMask & ((1 << i) - 1));
