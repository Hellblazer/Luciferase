...
/**
 * Validation framework for ensuring invariants are maintained in the Delaunay triangulation. This class can be enabled
 * during testing/debugging to catch violations early.
 *
...
public class ValidationManager {
    ...
    /**
     * Create a validation report without throwing exceptions. Useful for debugging and analysis.
     */
    ...
    /**
     * Enable or disable validation. When enabled, validation will be performed after each operation.
     */
    ...
    private void checkVertexAdjacent(Vertex v, Tetrahedron t) {
        ...
        if (v.getAdjacent() == null) {
            throw new AssertionError("Vertex " + v + " in tetrahedron " + t + " has null adjacent reference");
        }
    }
        ...
        for (Tetrahedron t : grid.tetrahedrons()) {
            ...
            if (nullNeighbors > 0) {
                ...
                if (nullNeighbors != 1) {
                    ...
                    "Boundary tetrahedron " + t + " has " + nullNeighbors + " null neighbors (expected 1)");
                }
            }
        }
    ...
    /**
     * Validate that the Delaunay property holds for all tetrahedra. No vertex should be inside the circumsphere of any
     * tetrahedron.
     */
    private void validateDelaunayProperty() {
        ...
        if (violationCount > 0) {
            throw new AssertionError(
            "Delaunay property violated: " + violationCount + " vertices found inside circumspheres");
        }
    }
        ...
        for (Tetrahedron t : grid.tetrahedrons()) {
            ...
            for (V v : V.values()) {
                ...
                if (neighbor != null && !neighbor.isDeleted()) {
                    ...
                    if (neighborV == null) {
                        throw new AssertionError(
                        "Inconsistent neighbor relationship: " + "tetrahedron " + t + " has neighbor " + neighbor
                    ...
                    if (commonVertices != 3) {
                        throw new AssertionError("Neighbor faces don't share 3 vertices: " + "only " + commonVertices
                                                 + " common vertices between faces");
                    }
                }
            }
        }
    ...
    private void validateVertexTetrahedronBidirectionalConsistency() {
        ...
        for (Vertex v : grid) {
            ...
            if (!v.getAdjacent().includes(v)) {
                throw new AssertionError("Inconsistent vertex-tetrahedron reference: " + "vertex " + v
                                         + " references tetrahedron that doesn't contain it");
            }
        }
    ...
    public static class ValidationReport {
        ...
        public boolean isValid() {
            return delaunayViolations == 0 && missingVertexReferences == 0 && staleVertexReferences == 0
            && inconsistentVertexReferences == 0 && inconsistentNeighborRelations == 0;
        }

        @Override
        public String toString() {
            return String.format("Validation Report:\n" + "  Total vertices: %d\n" + "  Total tetrahedra: %d\n"
                                 ...
                                 totalVertices, totalTetrahedra, delaunayViolations, missingVertexReferences,
                                 staleVertexReferences, inconsistentVertexReferences, inconsistentNeighborRelations,
