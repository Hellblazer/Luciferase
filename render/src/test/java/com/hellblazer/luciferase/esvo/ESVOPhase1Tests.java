 ...
 * - Ray generation in octree space [0,1]
...
public class ESVOPhase1Tests {
    ...
    @Test
    @DisplayName("Test ray generation in octree space [0,1]")
    void testRayGeneration() {
        // Camera outside octree in +Z, looking toward -Z into the octree
        // generateRay creates rays pointing -Z in camera space
        Vector3f cameraPos = new Vector3f(0.5f, 0.5f, 1.5f); // Outside octree in +Z, centered in X/Y
        Vector3f cameraDir = new Vector3f(0.0f, 0.0f, -1.0f);
        ...
        // Ray should start outside octree (z > 1.0) and point into it (dz < 0)
        assertTrue(ray.origin.z > CoordinateSpace.OCTREE_MAX,
        ...
        // Ray should intersect octree bounds [0,1]
    ...
    void testSingleLevelTraversal() {
        ...
        Vector3f rayOrigin = new Vector3f(-0.5f, 0.25f, 0.25f); // Left of octree, lower quadrant
        ...
        // Hit point should be in octree space [0,1]
    ...
    void testOctantCalculation() {
        Vector3f center = testOctree.getCenter(); // (0.5, 0.5, 0.5)
        ...
        testOctantHit(new Vector3f(0.25f, 0.25f, 0.25f), 0, 1); // Ray through 0, hits 1
        testOctantHit(new Vector3f(0.75f, 0.25f, 0.25f), 1, 1); // Ray through 1, hits 1
        testOctantHit(new Vector3f(0.25f, 0.75f, 0.25f), 2, 3); // Ray through 2, hits 3
        testOctantHit(new Vector3f(0.75f, 0.75f, 0.25f), 3, 3); // Ray through 3, hits 3
        testOctantHit(new Vector3f(0.25f, 0.25f, 0.75f), 4, 5); // Ray through 4, hits 5
        testOctantHit(new Vector3f(0.75f, 0.25f, 0.75f), 5, 5); // Ray through 5, hits 5
        testOctantHit(new Vector3f(0.25f, 0.75f, 0.75f), 6, 7); // Ray through 6, hits 7
        testOctantHit(new Vector3f(0.75f, 0.75f, 0.75f), 7, 7); // Ray through 7, hits 7
    }
    ...
    void testOctantMirroring() {
        ...
        // For [0,1] space: x' = 1.0 - x = 1.0 - (-0.5) = 1.5
        float expectedX = 1.0f - rayOrigin.x;
    ...
    void testRayOctreeIntersectionEdgeCases() {
        ...
        rayOrigin = new Vector3f(0.5f, 0.5f, 0.5f); // Center of octree
        ...
        rayOrigin = new Vector3f(-0.5f, 0.0f, 0.5f); // Aligned with octree edge
    ...
    void testCoordinateSpaceValidation() {
        // Test that traversal properly validates octree coordinate space [0,1]
        Vector3f center = testOctree.getCenter();
        assertEquals(0.5f, center.x, EPSILON, "Octree center X should be 0.5");
        assertEquals(0.5f, center.y, EPSILON, "Octree center Y should be 0.5");
        assertEquals(0.5f, center.z, EPSILON, "Octree center Z should be 0.5");
        ...
        assertEquals(CoordinateSpace.OCTREE_MIN, minBounds.x, EPSILON, "Min X bound should be 0.0");
        assertEquals(CoordinateSpace.OCTREE_MAX, maxBounds.x, EPSILON, "Max X bound should be 1.0");
    }
