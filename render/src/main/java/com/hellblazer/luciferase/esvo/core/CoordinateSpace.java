...
/**
 * Critical Coordinate Space Utilities for ESVO
 * 
 * ESSENTIAL: The octree ALWAYS resides at coordinates [1, 2] regardless of actual object size.
 * This is fundamental to the ESVO algorithm for numerical precision and ray intersection.
 * 
 * Transformation Chain:
 * World Space → Object Space (via objectToWorld matrix)
 * Object Space → Octree Space (via octreeToObject matrix) 
 * Octree Space is ALWAYS [1, 2] regardless of actual size
 */
public final class CoordinateSpace {
    
    // CRITICAL: Octree coordinate bounds - NEVER change these!
    ...
    public static final float OCTREE_CENTER = (OCTREE_MIN + OCTREE_MAX) * 0.5f; // 1.5f
    ...
    /**
     * Transform a point from world space to octree space [1,2]
     * 
     * @param worldPoint Point in world coordinates
     * @param worldToOctreeMatrix Combined transformation matrix
     * @return Point in octree space [1,2]
     */
     ...
     * @return Matrix transforming [1,2] octree space to object space
     */
    public static Matrix4f createOctreeToObjectMatrix(Vector3f objectCenter, float objectSize) {
        Matrix4f matrix = new Matrix4f();
        matrix.setIdentity();
        
        // Scale from [1,2] range to object size
        matrix.setScale(objectSize);
        
        // Translate to place octree center at object center
        // Octree center is at 1.5, so we need to offset by (objectCenter - 1.5 * objectSize)
    ...
    /**
     * Validate that a point is within octree coordinate bounds [1,2]
     * 
     * @param point Point to validate
     * @return true if point is within [1,2] in all dimensions
     */
    ...
    /**
     * Clamp a point to octree coordinate bounds [1,2]
     * 
     ...
     * The octree box is always [1,2] x [1,2] x [1,2]
     * 
    ...
    public static Vector3f applyOctantMirroringToOrigin(Vector3f rayOrigin, int octantMask) {
        ...
        if ((octantMask & 1) == 0) mirrored.x = 3.0f - mirrored.x; // Mirror around center (1.5 * 2 = 3)
        if ((octantMask & 2) == 0) mirrored.y = 3.0f - mirrored.y;
        if ((octantMask & 4) == 0) mirrored.z = 3.0f - mirrored.z;
        return mirrored;
    }
     ...
     * 
     * @param coordinate Coordinate value in [1,2] range
    ...
    public static int getOctreeLevel(float coordinate, int maxLevel) {
        if (coordinate < OCTREE_MIN || coordinate > OCTREE_MAX) {
            throw new IllegalArgumentException("Coordinate must be in [1,2] range");
        }
     ...
     * @return Position in octree coordinate space [1,2]
     */
