package com.hellblazer.luciferase.esvo.reference;
...
public class ESVONodeReference {
    ...
    public int getChildNodeIndexWithOctant(int childIdx, int octantMask) {
        ...
        // Sparse index: count set bits in the valid mask below actualSlot.
        // actualSlot = 7 ^ childShift = 7 ^ (childIdx ^ octantMask)
        int actualSlot = 7 ^ childShift;
        int validMask = getValidMask();
        int bitsBeforeChild = Integer.bitCount(validMask & ((1 << actualSlot) - 1));
