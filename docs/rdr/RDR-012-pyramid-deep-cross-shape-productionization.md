...
post_mortem: docs/rdr/post-mortem/012-pyramid-deep-cross-shape-productionization.md
...
> **Executed scope (2026-05-31, all gate questions resolved).** Gate-Q0 (domain contract): answered by **D0** — the reachable-SFC predicate is specified (§D0.1 C-1..C-5) and `PyramidKeyCodec.encode()` confirmed correct against it by the reframed oracle (D0.2). **D_fix: not triggered** — D0.2 found no live `encode()` reachability gap (the 26569/17208 split is fully the cube-vs-single-root domain difference). Gate-Q1 (deep path fate): **D2 + D3 selected** — deep cross-shape documented infrastructure-only (D2.1) and boundary-pinned (D2.2, `PyramidBoundaryPinningTest`); the deep tet-source t8code parity oracle shipped (D3.1, `T8codeDpyramidTetBoundaryOracleTest`, independently verified against t8code `main@76a5347b`). **D1 (productionize): explicitly deferred, reopen-only** — reopen `Luciferase-9hse` (locate-deep-tet primitive) when a concrete deep-insertion workload appears. The original gate-question text below is retained for the record.
