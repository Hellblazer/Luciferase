...
- This RDR pairs with RDR-011 (SFC linear-id) — both turn on the same SFC reachability semantics.
...
- **Direction D0 — Define & validate the domain contract (prerequisite for all below).** Write down the reachable-SFC predicate, reframe `T8codeDpyramidFaceOracleTest` to assert against Luciferase's reachable set with an explicit single-tree↔cube mapping, and either confirm `encode()` is correct against that contract or pin the gap. Closes ex-P1.
...
3. Add a boundary-pinning test: assert `PyramidIndex` locate/insert never emits keys with `minTetLevel < level` under normal refinement.
