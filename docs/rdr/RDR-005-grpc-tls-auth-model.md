...
> **Implementation note (2026-05-25, refinement — security model unchanged).** Realized at implementation that `common` is a pure leaf depended on by every module and sits *below* `simulation`, so it cannot reach the Delos Fireflies/KEL bridges and would force the heavy Delos KERI stack into the leaf for all consumers. To avoid that, `common` carries only `grpc-api` and holds the auth **mechanism** — `GrpcCredentialFactory`, a `PeerVerifier` SPI, and a generic `PeerAuthInterceptor` (extracts the peer cert from the TLS session, delegates accept/reject to the injected verifier, closes `UNAUTHENTICATED` otherwise). The Delos/Fireflies verification **policy** (`FirefliesPeerVerifier`: parse member `Digest` → KERL signature check → view membership) and the load-bearing forged-cert-rejection spike are implemented where Delos already lives (`simulation`, later `lucien-distributed`). This is dependency inversion, not a change to the two-layer mTLS + cryptographically-verified-identity decision; it keeps the leaf clean and is reconciled with RDR-007.
