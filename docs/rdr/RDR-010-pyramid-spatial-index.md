...
**Honest scope caveat.** The deep-tet arc is complete and validated *as infrastructure* but is **not consumed in production**: `PyramidIndex`'s locate primitive stops at the shallowest tet leaf, so deep tet keys are never inserted. RDR-010's actual need is the shallow hex↔tet boundary, which is fully live. The deep path is reachable only via direct refinement / tests today. **(RDR-012 D2, accepted 2026-05-31: this infrastructure-only status was made the decision, not just a caveat — the shallow-only-live contract is now pinned by `PyramidBoundaryPinningTest`; the pyramid-source face oracle is RDR-012 D0.2 and the deep tet-return oracle is D3.1/`Luciferase-dk12`.)**
...
- **`Tet` primitive** (`lucien/src/main/java/com/hellblazer/luciferase/lucien/tetree/Tet.java`):
  ...
  - `consecutiveIndex()` @ line 1010 — 3 bits/level, O(1) via `Constants.TYPE_CUBE_ID_TO_LOCAL_INDEX`. The pyramid analog adds 3 type-tuple bits/level.
...
**Additional registered deferrals (explicit, not silent):** ~~deep-tet (`l > minTetLevel`) cross-shape face neighbors stay fail-loud guarded (Finding #16) → q3p Phase E / future RDR-scoped tet-tree reconciliation~~ **[SUPERSEDED — see Close section: deep cross-shape was implemented within RDR-010 (cjwr/2l04); no fail-loud guard remains. Productionization vs. infrastructure-only status now tracked by RDR-012.]**; exhaustive cross-shape edge/vertex topology → `Luciferase-0utt`. Both surfaced, not silently dropped.
...
- `Luciferase-d3z3` (§4c consumer-wiring) · `Luciferase-0utt` (exhaustive cross-shape edge/vertex) · ~~deep-tet (`l > minTetLevel`) cross-shape (Finding #16, fail-loud guarded — pending a future tree-divergence RDR)~~ **[SUPERSEDED — deep cross-shape implemented within RDR-010 (cjwr/2l04); see Close section + RDR-012]** · `Luciferase-3y1` (PyramidKeyDecoder dedup) · `Luciferase-7eb` (insertWithSpanning).
- ~~The deep-tet fail-loud guard (Finding #16) remains in place; no silent enablement.~~ **[SUPERSEDED 2026-05-31 — there is no fail-loud guard; deep cross-shape was implemented (cjwr/2l04) but is infrastructure-only, not consumed by live locate. RDR-012 (accepted 2026-05-31) decided D2 (keep infrastructure-only); the shallow-only-live contract is pinned by `PyramidBoundaryPinningTest` (D2.2). The pyramid-source t8code parity oracle shipped as RDR-012 D0.2 (`T8codeDpyramidFaceOracleTest`, ex-remediation-P1 `Luciferase-8xus`, closed); the deep tet-return branch oracle is D3.1 (`Luciferase-dk12`).]**
