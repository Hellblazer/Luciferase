...
- **Sequencing dependency RESOLVED 2026-05-28**: RDR-008 (god-class decomposition, epic `Luciferase-x5i`) closed today. All six phases (P0 `SpatialIndexCore` → P1 `DsocController` → P2 `GhostCoordinator` → P3 `KnnSearcher` → P4 `Culler` → P5 `CollisionEngine` → P6 `EntityLifecycleManager`) shipped. Facade shrunk 5851 → 2916 LOC (50% reduction). Residual ~159 methods, with **17 `protected abstract` methods** (the load-bearing geometry hooks: `addNeighboringNodes`, `calculateSpatialIndex`, `createDefaultSubdivisionStrategy`, `doesFrustumIntersectNode`, `doesNodeIntersectVolume`, `doesPlaneIntersectNode`, `doesRayIntersectNode`, `estimateNodeDistance`, `findNodesIntersectingBounds`, `getCellSizeAtLevel`, `getFrustumTraversalOrder`, `getNodeBounds`, `getPlaneTraversalOrder`, `getRayNodeIntersectionDistance`, `getRayTraversalOrder`, `isNodeContainedInVolume`, `shouldContinueKNNSearch`) + ~32 non-abstract protected helpers. PyramidIndex must implement all 17 abstract methods with pyramid-specific geometry — this is the main implementation surface, not just "wire up collaborators." The post-mortem `docs/rdr/post-mortem/008-decompose-abstractspatialindex.md` documents the realized-vs-predicted delta. **PyramidIndex implementation arc can now proceed** without further wait on RDR-008.
...
3. **Pyramid containment primitive — three sub-options to evaluate at gate.** Pyramids are NOT Kuhn simplices, so the 12-DOP exact containment (RDR-002) does not apply directly. Pyramids also are NOT a permitted variant of the `Spatial.aabt` sealed interface from RDR-001 (which currently permits only `Tet`), so any `aabt`-typed path (KnnSearcher, Culler, CollisionEngine, range queries) needs to resolve pyramid containment through either an `aabt` widening or a tet-decomposition bridge. Options:
   ...
   - **3b — Decompose-and-reuse (RECOMMENDED DEFAULT).** Each pyramid = 6 sub-pyramids + 4 sub-tets at the next level. Recurse one level, then apply tet 12-DOP at the leaves. Reuses proven code; correctness inherited. ~10× containment cost per pyramid at any given level vs. 3a. **Naturally resolves the RDR-001 `aabt permits Tet` constraint** — containment is delegated to the tet-child decomposition, so the `aabt` path sees only Tet instances. No `aabt` widening required.
...
4. **Luciferase's `Forest` API is heterogeneous, but `forest.balancing` is NOT.** `Forest.addTree(AbstractSpatialIndex<Key, ID, Content>)` already accepts heterogeneous trees — the type-parameter-based API is shape-agnostic. **However**, `lucien/src/main/java/com/hellblazer/luciferase/lucien/forest/balancing/fault/TwoOneBalanceChecker.java:127` contains explicit `instanceof MortonKey` branches with a code comment acknowledging `TetreeKey` (and by extension `PyramidKey`) support is missing — quoting the file: `// Additional key types (TetreeKey, etc.) would be handled similarly`. So a pyramid tree added to a Forest that runs 2:1 balance will silently skip balance-violation detection for its neighbors. The Algorithm 5.1 `N_shape(ℓ)` partition-weight hook on `TreeNode.java:76` (`entityCount` only) is one piece; the `TwoOneBalanceChecker` shape-specific neighbor walk is the other. **Direction B's Forest scope is bounded but not zero** — `N_shape(ℓ)` + balance-checker shape-router are both required for correctness in mixed forests.
...
12. **Direction B confirmed at HIGH confidence by synthesis (with one scope-bound correction).** Two uncertainties at RDR drafting are now resolved: (a) Holke 2019 ghost paper IS indexed at `1.14.6` (no longer a future-work caveat); (b) Forest weight pluggability is bounded — though NOT to a "single callback" as the synthesis initially claimed. **Correction (post-gate critique 2026-05-28):** `Forest.balancing.fault.TwoOneBalanceChecker:127` has additional MortonKey-specific hardcoding (TetreeKey already-noted-as-missing in the source comment); pyramid integration in Direction B needs both `N_shape(ℓ)` weight + balance-checker shape-router. Scope is still bounded (2 classes touched in `forest.balancing`, not a structural Forest rewrite). Direction A is the documented-correctness-gap fallback (silent balance-detection skip on non-Morton trees). Direction C remains as a capacity-deferral option; algorithmically unnecessary post-synthesis but viable if implementation opportunity-cost analysis at gate time concludes the work should defer.
...
| `Forest.addTree()` heterogeneous API | `forest` package | Already accepts heterogeneous trees — surprise finding from prior research; NB `forest.balancing.fault.TwoOneBalanceChecker` is NOT shape-generic (see "requires extension") |
...
| `forest.balancing.fault.TwoOneBalanceChecker` | Extend MortonKey-only neighbor walk at `:127` with TetreeKey + PyramidKey branches (or shape-keyed dispatch) | Source comment at `:127` acknowledges TetreeKey support is missing; PyramidKey support similarly absent |
...
| `Spatial.aabt` sealed interface (RDR-001) | No widening required if §3b chosen; containment path delegates to Tet's existing `aabt` impl via pyramid → 6 sub-pyramids + 4 sub-tets decomposition | Decompose-and-reuse is the natural fit; if §3a is chosen instead, `aabt` would need a Pyramid permits-variant |
