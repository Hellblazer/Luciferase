...
6. **FCC/RD intersection: orthogonal (medium confidence).** The open FCC/rhombic-dodecahedral direction (RDR-003; T3 `architecture-luciferase-fcc-*`, flagged "OPEN questions, not resolved decisions") is a different *lattice-overlay* question; two-prism cover completes the existing triangular-prism tiling within the cubic domain. No prior decision ties them. Confirm at gate that pursuing two-prism does not pre-empt an FCC choice.
...
- Does pursuing two-prism pre-empt or compose with the open FCC/RD direction (RDR-003; T3 `architecture-luciferase-fcc-*`)? Finding 6 says orthogonal (medium confidence) — confirm at gate.
- Fallback trigger: at what cost/complexity does Option A yield to Option B (document + fail-fast)? In particular, if the gate chooses the full SFC fix (breaking change), is that still proportionate vs documenting Prism as a half-cube specialist?
