...
- **Scope caveat (infrastructure-only):** the deep-tet path is validated topology but **not consumed in production** — `PyramidIndex`'s locate stops at the shallowest tet leaf, so deep tet keys are never inserted by normal index operation. It is reachable only via direct refinement / tests. Its fate (productionize vs. mark infrastructure-only) is **RDR-012**. The deep face-neighbor path also lacks an independent t8code parity oracle (the `kyz9` oracle bead was closed won't-do) — tracked for remediation. Port baseline: t8code `main@76a5347b` (Feb 2024).
