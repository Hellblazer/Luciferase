...
- **Deep cross-shape is IMPLEMENTED (not guarded).** Deep pyramid-rooted tet `faceNeighborElement` (`l > minTetLevel`) is implemented via the ported `t8_dpyramid_tet_boundary` corner-walk (`Tet.tetBoundary`, cjwr A), deep tet SFC keys (`encode(Tet)`/`elementFromKey` accept `minTetLevel < level`, cjwr B), and full-depth edge/vertex neighbors (`allShapeNeighbors`, 2l04). There is **no fail-loud guard** — the earlier "deferred to a separate RDR" framing was superseded by `Luciferase-4pd` (tet type alignment). Shallow (`l == minTetLevel`) cross-type face neighbors are also supported.
- **Scope caveat (infrastructure-only — RDR-012 D2 decided):** the deep-tet path is validated topology but **not consumed in production** — `PyramidIndex`'s locate stops at the shallowest tet leaf, so deep tet keys are never inserted by normal index operation. It is reachable only via direct refinement / tests. **RDR-012 (accepted 2026-05-31) decided D2: keep it infrastructure-only** (no productionization absent a deep-insertion workload; D1 reopen-only). The shallow-only-live contract is now pinned by `PyramidBoundaryPinningTest` (D2.2) — locate/insert never emits a tet with `0 ≤ minTetLevel < level`. The pyramid-source face path has an independent t8code parity oracle (`T8codeDpyramidFaceOracleTest`, RDR-012 D0.2); the deep tet-return branch oracle is RDR-012 D3.1 (`Luciferase-dk12`). Port baseline: t8code `main@76a5347b` (Feb 2024).
...
- **Scope note**: the shallow pyramid↔tet boundary (`l == minTetLevel`) is the production-live path. Deep-tet cross-shape topology is also implemented (cjwr/2l04) but is infrastructure-only (not consumed by live locate — see RDR-012). The standalone `TETREE_T8CODE_PARTITION_ANALYSIS.md` is retired — RDR-010 is the authoritative record.
