...
import static com.hellblazer.luciferase.geometry.Rotor3f.PrincipalAxis.*;
...
public abstract class MagicMirror extends Application {
    ...
    public static void lookAt(Point3D cameraPosition, Point3D lookAtPos, Camera cam) {
        ...
        cam.getTransforms().addAll(ry, rx,
                                   new Translate(cameraPosition.getX(), cameraPosition.getY(), cameraPosition.getZ()));
    }
        ...
        scene.setOnMouseDragged(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent me) {
                ...
                if (me.isMiddleButtonDown() || (me.isPrimaryButtonDown() && me.isSecondaryButtonDown())) {
                    t.next().translation().setX(
                    t.next().translation().getX() + h.mouseDeltaX * MOUSE_SPEED * modifier * TRACK_SPEED);
                    t.next().translation().setY(
                    t.next().translation().getY() + h.mouseDeltaY * MOUSE_SPEED * modifier * TRACK_SPEED);
