...
/**
 * Unified class for managing tetrahedral cell views using transformations and reference geometry
 * for efficient rendering. This class creates and caches the 6 reference meshes and wireframes
 * (one for each characteristic tetrahedron type S0-S5) and provides transformation calculations
 * to position, scale, and orient instances as needed.
 *
 * @author hal.hildebrand
 */
public class TetreeCellViews {
    ...
    /**
     * Initialize the cell view manager with default edge thickness and material.
     */
    public TetreeCellViews() {
        this(0.01, new PhongMaterial(javafx.scene.paint.Color.BLACK));
    }
    
    /**
     * Initialize the cell view manager with specified edge thickness and material.
     *
     * @param edgeThickness The thickness of the wireframe edges
     * @param edgeMaterial  The material to use for the edges
     */
    public TetreeCellViews(double edgeThickness, Material edgeMaterial) {
        this.edgeThickness = edgeThickness;
        this.edgeMaterial = edgeMaterial;
        
        // Create reference geometry for each tetrahedron type
        for (int type = 0; type < 6; type++) {
            ...
            referenceMeshes[type] = createUnitTetrahedronMesh(vertices);
            
            // Create unit-sized reference wireframe at origin
            referenceWireframes[type] = createUnitTetrahedronWireframe(vertices);
        }
    }
    ...
    /**
     * Create a mesh view for the given tetrahedron.
     *
     * @param tet The tetrahedron to visualize
     * @return A MeshView with appropriate transforms applied
     */
    public MeshView createMeshView(Tet tet) {
        ...
        // Debug output
        System.out.printf("TetreeCellViews.createMeshView: type=%d, anchor=%s, length=%d%n", 
                         tet.type(), tet.anchor(), tet.length());
        System.out.printf("  Transform: scale=(%.2f, %.2f, %.2f) translate=(%.0f, %.0f, %.0f)%n",
                         transform.getMxx(), transform.getMyy(), transform.getMzz(),
                         transform.getTx(), transform.getTy(), transform.getTz());
     ...
     */
    public Group createWireframeView(Tet tet) {
        ...
        System.out.printf("TetreeCellViews.createWireframeView: created wireframe with %d cylinders%n",
                         wireframeView.getChildren().size());
    ...
    /**
     * Get type-specific rotation if needed. This handles the different orientations
     * of types 1-5 relative to type 0.
     *
