...
public class PolyLine extends Group {
    ...
    public static PolyLine ellipse(Point3D center, Point3D a, Point3D b, double radius, int segments,
                                   Material material) {
        ...
        for (int i = 0; i <= segments; i++) {
            points.add(u.multiply(Math.cos(theta)).add(v.multiply(Math.sin(theta))));
            theta += increment;
        }
