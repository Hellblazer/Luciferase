...
/**
 * Transform-based Tetree visualization that uses JavaFX transforms efficiently.
 * Creates only 6 reference meshes (one for each characteristic tetrahedron type S0-S5)
 * and uses transforms to position, scale, and orient instances as needed.
 * 
 * This approach significantly reduces memory usage and improves performance compared
 * to creating individual mesh instances for each tetrahedron.
 * 
...
public class TransformBasedTetreeVisualization<ID extends EntityID, Content> {
    ...
    /**
     * Create the 6 reference meshes for characteristic tetrahedra.
     * These are unit tetrahedra at the origin that will be transformed as needed.
     */
    ...
    private Affine calculateTransform(Tet tet) {
        ...
        if (transformCache.size() < 5) { // Only log first few
            System.out.println("Transform for tet type=" + tet.type() + 
    ...
    /**
     * Get type-specific rotation if needed.
     * This handles the different orientations of S1-S5 relative to S0.
     * 
     * Based on analysis of Constants.SIMPLEX_STANDARD:
     * - S0: Reference orientation
    ...
    public void demonstrateUsage(Tetree<ID, Content> tetree) {
        ...
        tetree.nodes().forEach(node -> {
            ...
            System.out.println("Adding tet: type=" + tet.type() + 
                ", level=" + tet.l() + 
                ", pos=(" + tet.x() + "," + tet.y() + "," + tet.z() + ")");
        ...
        System.out.println("Created " + tetCount + 
                          " tetrahedra using only 6 reference meshes");
