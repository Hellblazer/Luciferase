...
/**
 * Bridges the OpenCL compute renderer with JavaFX display for ESVO.
 *
 * <p>Unlike the OpenGL version, this bridge doesn't need a hidden GLFW window
 * because OpenCL doesn't require an OpenGL context. This makes it simpler and
 * works on macOS where OpenGL 4.3+ is not available.
 *
 * <p>Threading model:
 * <ul>
 ...
 */
public class ESVOOpenCLRenderBridge implements AutoCloseable {

    private static final Logger log = LoggerFactory.getLogger(ESVOOpenCLRenderBridge.class);
    ...
    // OpenCL renderer
    private ESVOOpenCLRenderer clRenderer;
    ...
    private volatile Matrix4f octreeToObject = new Matrix4f();
    ...
    public static boolean isAvailable() {
        return ESVOOpenCLRenderer.isOpenCLAvailable();
    }
    ...
    public CompletableFuture<Void> initialize() {
        ...
        return CompletableFuture.runAsync(() -> {
            try {
                clRenderer = new ESVOOpenCLRenderer(width, height);
    ...
    public CompletableFuture<Void> uploadData(ESVOOctreeData data) {
        ...
        return CompletableFuture.runAsync(() -> {
            ...
            log.debug("Uploaded ESVO data to OpenCL: {} nodes", data.getNodeCount());
        }, renderThread);
    }
     ...
     * Thread-safe, can be called from any thread.
     */
    public void setTransforms(Matrix4f objToWorld, Matrix4f octToObj) {
        this.objectToWorld = new Matrix4f(objToWorld);
        this.octreeToObject = new Matrix4f(octToObj);
    }
    ...
    public void renderAsync(Consumer<WritableImage> callback) {
        ...
        renderThread.execute(() -> {
            ...
            try {
                ...
                var oct2o = new Matrix4f(octreeToObject);

                // Render using OpenCL
                clRenderer.renderFrame(view, proj, o2w, oct2o);
    ...
    public void close() {
        ...
        log.info("ESVOOpenCLRenderBridge disposed");
    }
