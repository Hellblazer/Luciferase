...
public class PolyhedraUtils {
    ...
    public static Map<Integer, Map<Integer, Integer>> addEdgeToCentroidVertices(Polyhedron source, Polyhedron modify) {
        ...
        for (Face face : source.getFaces()) {
            ...
            for (int i = 0; i < edges.length; i++) {
                ...
                Vector3d newVertex = VectorMath.interpolate(edgeMidpt, centroid, 0.3); // 0 < arbitrary scale factor < 1
            ...
            for (int i = 0; i < edges.length; i++) {
                ...
                edgeToVertex.computeIfAbsent(ends[0], a -> new HashMap<>());
    ...
    public static Map<Integer, Map<Integer, int[]>> subdivideEdges(Polyhedron source, Polyhedron modify, int segments) {
        ...
        for (Edge edge : source.getEdges()) {
            ...
            newVertices.computeIfAbsent(ends[0], a -> new HashMap<>());
            newVertices.get(ends[0]).put(ends[1], newIndices);

            newVertices.computeIfAbsent(ends[1], a -> new HashMap<>());
