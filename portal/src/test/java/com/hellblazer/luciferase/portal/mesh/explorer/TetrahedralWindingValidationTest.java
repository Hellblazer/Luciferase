...
/**
 * Validates that all tetrahedra in the visualization have consistent face winding
 * for proper rendering. The key requirement is that the winding order matches the
 * sign of the tetrahedron's volume.
 *
...
public class TetrahedralWindingValidationTest {
    ...
    private void validateFaceNormalConsistency(Point3f[] vertices, int i0, int i1, int i2,
                                             Point3f centroid, boolean expectOutward, String faceName) {
        // Compute face normal
        ...
        Point3f faceCentroid = new Point3f(
            (vertices[i0].x + vertices[i1].x + vertices[i2].x) / 3.0f,
