...
/**
 * Comprehensive unit tests for TransformBasedTetreeVisualization.
 * Tests the transform pipeline from bottom up:
 * 1. Reference mesh creation
...
public class TransformBasedTetreeVisualizationTest {
    ...
    public void testReferenceMeshCreation() throws InterruptedException {
        ...
        Platform.runLater(() -> {
            try {
                // Create a minimal tetree and visualization
                ...
                for (byte level : levels) {
                    ...
                    for (Transform t : meshView.getTransforms()) {
                        if (t instanceof Affine) {
                            Affine affine = (Affine) t;
                            System.out.printf("  Transform: Scale=(%.0f, %.0f, %.0f) Translate=(%.0f, %.0f, %.0f)%n",
                                affine.getMxx(), affine.getMyy(), affine.getMzz(),
                                affine.getTx(), affine.getTy(), affine.getTz());
                            ...
                            assertEquals(0, affine.getTx(), 0.001,
                                "X translation should be 0 at origin");
                            assertEquals(0, affine.getTy(), 0.001,
                                "Y translation should be 0 at origin");
                            assertEquals(0, affine.getTz(), 0.001,
                                "Z translation should be 0 at origin");
                        }
                    }
                }
            ...
            try {
                ...
                for (int[] pos : positions) {
                    ...
                    for (Transform t : meshView.getTransforms()) {
                        if (t instanceof Affine) {
                            Affine affine = (Affine) t;
                            System.out.printf("  Transform: Translate=(%.0f, %.0f, %.0f)%n",
                                affine.getTx(), affine.getTy(), affine.getTz());
                            ...
                            assertEquals(expectedX, affine.getTx(), 0.001,
                                "X translation mismatch");
                            assertEquals(expectedY, affine.getTy(), 0.001,
                                "Y translation mismatch");
                            assertEquals(expectedZ, affine.getTz(), 0.001,
                                "Z translation mismatch");
                        }
                    }
                }
            ...
            try {
                ...
                if (!sceneRoot.getChildren().isEmpty()) {
                    ...
                    if (meshView.getParent().getParent() != null) {
                        System.out.println("  Grandparent transforms: " + meshView.getParent().getParent().getTransforms());
                    }
                    ...
                    assertTrue(minX > 1_000_000, 
                        "X coordinate should be in millions due to scaling: " + minX);
                }
