...
The Entity Management API provides comprehensive methods for managing entity lifecycle, properties, bounds, and spanning behavior in the spatial index. This includes entity creation, updates, queries, and advanced features like multi-node spanning.
...
spatialIndex.insert(customId, position, (byte)10, vehicle);
```
...
spatialIndex.updateEntity(vehicleId, newPosition, (byte)10);
```
...
if (spatialIndex.containsEntity(targetId)) {
    ...
    log.warn("Target entity {} not found", targetId);
}
```
...
log.info("Spatial Index Statistics:");
log.info("  Total Entities: {}", stats.entityCount());
log.info("  Total Nodes: {}", stats.nodeCount());
log.info("  Entity References: {}", stats.totalEntityReferences());
log.info("  Max Tree Depth: {}", stats.maxDepth());
log.info("  Avg Entities/Node: {:.2f}", stats.averageEntitiesPerNode());
log.info("  Entity Spanning Factor: {:.2f}", stats.entitySpanningFactor());
```
...
log.info("Inserted {} new entities", after - before);
```
...
public class LargeEntityManager {
    ...
    public void insertLargeStructure(Structure structure) {
        ...
        spatialIndex.insert(tempId, structure.getCenter(), 
...
public class EntityLifecycleManager {
    ...
    public ID spawn(GameObject object, Point3f position, byte level) {
        ...
        // Store metadata
        metadata.put(entityId, new EntityMetadata(
        ...
        if (object.hasBounds()) {
            spatialIndex.insert(entityId, position, level, 
        ...
        if (object.hasCustomCollision()) {
            spatialIndex.setCollisionShape(entityId, 
