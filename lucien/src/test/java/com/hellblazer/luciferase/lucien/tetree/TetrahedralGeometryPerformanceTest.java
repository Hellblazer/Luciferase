...
/**
 * Performance comparison test between standard and enhanced tetrahedral geometry implementations.
 * Run with: RUN_SPATIAL_INDEX_PERF_TESTS=true
 */
...
public class TetrahedralGeometryPerformanceTest {
    ...
    private List<Long> testTetIndices;
    ...
    private void testStandardImplementation() {
        ...
        System.out.printf("Intersections found: %d (%.1f%%)\n", 
                          intersectionCount, (intersectionCount * 100.0) / TEST_ITERATIONS);
    }
    ...
    private void testBatchOperations() {
        ...
        long tetIndex = testTetIndices.get(0);
        Ray3D[] rayBatch = testRays.toArray(new Ray3D[0]);
    ...
    private void testCacheEffectiveness() {
        ...
        for (int i = 0; i < 1000; i++) {
            Ray3D ray = testRays.get(i % testRays.size());
            long tetIndex = frequentTets[i % frequentTets.length];
            EnhancedTetrahedralGeometry.rayIntersectsTetrahedronCached(ray, tetIndex);
        }
    ...
    private List<Long> generateTestTetIndices(int count) {
        List<Long> indices = new ArrayList<>(count);
    ...
    void testBoundingSphereOptimization() {
        ...
        long tetIndex = testTetIndices.get(0);
        float[] boundingSphere = EnhancedTetrahedralGeometry.getTetrahedronBoundingSphere(tetIndex);
        ...
        System.out.printf("Early rejections: %d (%.1f%%)\n", 
                          earlyRejections, (earlyRejections * 100.0) / missRays.size());
    }
}