...
/**
 * Unit tests for Proximity search functionality
...
public class ProximitySearchTest {
    ...
    void testDistanceRange() {
        ...
        ProximitySearch.DistanceRange range = new ProximitySearch.DistanceRange(
            100.0f, 500.0f, ProximitySearch.ProximityType.CLOSE);
    ...
    void testCubesWithinDistanceRange() {
        ...
        for (int i = 0; i < results.size() - 1; i++) {
            assertTrue(results.get(i).distanceToQuery <= 
                      results.get(i + 1).distanceToQuery);
        }
    ...
    void testCubesInProximityBands() {
        ...
        Map<ProximitySearch.DistanceRange, List<ProximitySearch.ProximityResult<String>>> results = 
            ProximitySearch.cubesInProximityBands(queryPoint, ranges, octree);
        ...
        for (var entry : results.entrySet()) {
            ...
            for (int i = 0; i < bandResults.size() - 1; i++) {
                assertTrue(bandResults.get(i).distanceToQuery <= 
                          bandResults.get(i + 1).distanceToQuery);
            }
        }
    }
    ...
    void testFindNClosestCubes() {
        ...
        List<ProximitySearch.ProximityResult<String>> results = 
            ProximitySearch.findNClosestCubes(queryPoint, n, octree);
    ...
    void testCubesNearAnyPoint() {
        ...
        for (int i = 0; i < results.size() - 1; i++) {
            assertTrue(results.get(i).minDistanceToQuery <= 
                      results.get(i + 1).minDistanceToQuery);
        }
    ...
    void testGetProximityStatistics() {
        ...
        ProximitySearch.ProximityStatistics stats = 
            ProximitySearch.getProximityStatistics(queryPoint, octree);
        ...
        assertEquals(stats.totalCubes, 
                    stats.veryCloseCubes + stats.closeCubes + stats.moderateCubes + stats.farCubes);
    ...
    void testEmptyOctree() {
        ...
        List<ProximitySearch.ProximityResult<String>> results = 
            ProximitySearch.cubesWithinDistanceRange(queryPoint, range, emptyOctree);
        
        assertTrue(results.isEmpty());
        
        List<ProximitySearch.ProximityResult<String>> nClosest = 
            ProximitySearch.findNClosestCubes(queryPoint, 3, emptyOctree);
    ...
    void testNegativeCoordinatesThrowException() {
        ...
        List<Point3f> queryPointsWithNegative = List.of(
            positiveQuery,
            new Point3f(-50.0f, 100.0f, 100.0f)
        );
    ...
    void testProximityResultDataIntegrity() {
        Point3f queryPoint = new Point3f(200.0f, 200.0f, 200.0f);
        ProximitySearch.DistanceRange range = new ProximitySearch.DistanceRange(
            0.0f, 1000000.0f, ProximitySearch.ProximityType.CLOSE);
    ...
    void testDistanceOrdering() {
        ...
        List<ProximitySearch.ProximityResult<String>> nearResults = 
            ProximitySearch.findNClosestCubes(nearQuery, n, octree);
        
        List<ProximitySearch.ProximityResult<String>> farResults = 
            ProximitySearch.findNClosestCubes(farQuery, n, octree);
        ...
        if (!nearResults.isEmpty()) {
            ...
            for (int i = 0; i < nearResults.size() - 1; i++) {
                assertTrue(nearResults.get(i).distanceToQuery <= 
                          nearResults.get(i + 1).distanceToQuery);
            }
        }
        
        if (!farResults.isEmpty()) {
            for (int i = 0; i < farResults.size() - 1; i++) {
                assertTrue(farResults.get(i).distanceToQuery <= 
                          farResults.get(i + 1).distanceToQuery);
            }
        }
    }
    ...
    void testProximityTypeClassification() {
        ...
        List<ProximitySearch.ProximityResult<String>> allResults = 
            ProximitySearch.findNClosestCubes(queryPoint, 100, octree);
