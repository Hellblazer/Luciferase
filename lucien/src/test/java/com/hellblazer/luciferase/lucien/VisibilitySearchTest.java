...
public class VisibilitySearchTest {
    ...
    void testLineOfSightOccluded() {
        ...
        boolean hasOccludingEntity = result.occludingEntities.stream()
            .anyMatch(e -> e.content.startsWith("OccludingEntity"));
        assertTrue(hasOccludingEntity);
        
        // Should be sorted by distance from observer
        for (int i = 0; i < result.occludingEntities.size() - 1; i++) {
            assertTrue(result.occludingEntities.get(i).distanceFromObserver <= 
                      result.occludingEntities.get(i + 1).distanceFromObserver);
        }
    }
    ...
    void testFindVisibleEntities() {
        ...
        boolean hasVisibleEntity = visibleEntities.stream()
            .anyMatch(e -> e.content.startsWith("VisibleEntity"));
        assertTrue(hasVisibleEntity);
        
        // Should NOT include entities outside cone
        boolean hasOutsideConeEntity = visibleEntities.stream()
            .anyMatch(e -> e.content.startsWith("OutsideConeEntity"));
        assertFalse(hasOutsideConeEntity);
        
        // Should NOT include far entities
        boolean hasFarEntity = visibleEntities.stream()
            .anyMatch(e -> e.content.startsWith("FarEntity"));
        assertFalse(hasFarEntity);
        
        // Should be sorted by distance
        for (int i = 0; i < visibleEntities.size() - 1; i++) {
            assertTrue(visibleEntities.get(i).distanceFromObserver <= 
                      visibleEntities.get(i + 1).distanceFromObserver);
        }
    }
    ...
    void testViewingConeFiltering() {
        ...
        for (var narrowEntity : narrowResults) {
            boolean foundInWide = wideResults.stream()
                .anyMatch(w -> w.id.equals(narrowEntity.id));
    ...
    void testOcclusionContributions() {
        ...
        float totalContributions = result.entityOcclusionContributions.values().stream()
    ...
    void testSelfOcclusion() {
        ...
        if (result.occludingEntities.size() == 1 && 
            result.occludingEntities.get(0).content.equals("LargeEntity")) {
            // Self-occlusion case
