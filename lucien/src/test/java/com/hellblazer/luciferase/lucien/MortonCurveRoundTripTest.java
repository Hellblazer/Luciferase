...
/**
 * Comprehensive round-trip tests for MortonCurve encoding/decoding and Constants.toLevel
 * to validate that Morton coordinates correctly encode levels.
 * 
    ...
    void testCalculateMortonIndexRoundTrip() {
        ...
        for (byte level = 0; level <= 21; level++) {
            ...
            if (level == 0) {
                ...
                testPoints = new Point3f[] {
                    new Point3f(0, 0, 0),
                    new Point3f(1000000, 0, 0),
                    new Point3f(0, 1000000, 0),
                    new Point3f(0, 0, 1000000),
                    new Point3f(500000, 500000, 500000)
                };
            } else {
            ...
            for (Point3f point : testPoints) {
                ...
                assertEquals(expectedWorldX, decoded[0], 
                    String.format("X mismatch for point (%.1f,%.1f,%.1f) at level %d", 
                                  point.x, point.y, point.z, level));
                assertEquals(expectedWorldY, decoded[1], 
                    String.format("Y mismatch for point (%.1f,%.1f,%.1f) at level %d", 
                                  point.x, point.y, point.z, level));
                assertEquals(expectedWorldZ, decoded[2], 
                    String.format("Z mismatch for point (%.1f,%.1f,%.1f) at level %d", 
                                  point.x, point.y, point.z, level));
            }
        }
    }
    ...
    void testLevelDeterminationLogic() {
        ...
        for (TestCase tc : testCases) {
            ...
            assertEquals(tc.expectedLevel, actualLevel, 
                String.format("Level mismatch for coordinates (%d,%d,%d), morton=%d", 
                              tc.x, tc.y, tc.z, morton));
        }
    }
                ...
                for (int y = 0; y < 5 * length; y += length) {
                    for (int z = 0; z < 5 * length; z += length) {
                        ...
                        assertFalse(mortonCodes.contains(morton), 
                            String.format("Duplicate Morton code %d for point (%d,%d,%d) at level %d", 
                                          morton, x, y, z, level));
        ...
        for (Point3f adj : adjacent) {
            if (adj.x >= 0 && adj.y >= 0 && adj.z >= 0) {
                ...
                assertNotEquals(mortonOrigin, mortonAdj, 
                    String.format("Adjacent cell at (%.0f,%.0f,%.0f) has same Morton code as origin", 
                                  adj.x, adj.y, adj.z));
            }
        }
    }
