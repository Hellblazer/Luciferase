...
/**
 * Long-running performance test for Tetree profiling. This test exercises various aspects of the Tetree to help
 * identify performance bottlenecks: - Insertion patterns (uniform, clustered, hierarchical) - Query operations (lookup,
 * k-NN, range queries) - Update operations (movement simulation) - Tree balancing and subdivision - Memory usage
 * patterns
 *
...
public class TetreeProfilingTest {
    ...
    void setup() {
        ...
        assumeFalse(CIEnvironmentCheck.isRunningInCI(), "Profiling test is disabled in CI environments");
        // Skip if not flagged
    ...
    private Point3f generateClusteredPosition(int maxCoord) {
        ...
        return new Point3f(Math.max(0, Math.min(maxCoord, x)), Math.max(0, Math.min(maxCoord, y)),
    ...
    private Point3f generateHierarchicalPosition(int maxCoord) {
        ...
        return new Point3f(Math.min(maxCoord - 1, x), Math.min(maxCoord - 1, y), Math.min(maxCoord - 1, z));
    }

    private Point3f generateRandomPosition(int maxCoord) {
        return new Point3f(random.nextFloat() * maxCoord, random.nextFloat() * maxCoord, random.nextFloat() * maxCoord);
    }
    ...
    private void profileConcurrentOperations() {
        ...
        for (int t = 0; t < numThreads; t++) {
            ...
            threads[t] = new Thread(() -> {
                ...
                for (int i = 0; i < 5000; i++) {
                    ...
                    switch (op) {
                        case 0: // Insert
                            Point3f pos = new Point3f(tlr.nextFloat() * 1000, tlr.nextFloat() * 1000,
                                                      tlr.nextFloat() * 1000);
                        ...
                        case 1: // Point lookup
                            Point3f query = new Point3f(tlr.nextFloat() * 1000, tlr.nextFloat() * 1000,
                                                        tlr.nextFloat() * 1000);
                            tetree.lookup(query, (byte) 8);
                            break;

                        case 2: // k-NN search
                            Point3f knnQuery = new Point3f(tlr.nextFloat() * 1000, tlr.nextFloat() * 1000,
                                                           tlr.nextFloat() * 1000);
                            tetree.kNearestNeighbors(knnQuery, 10, Float.MAX_VALUE);
                            break;

                        case 3: // Range query
                            Point3f center = new Point3f(tlr.nextFloat() * 900, tlr.nextFloat() * 900,
                                                         tlr.nextFloat() * 900);
                            // Use cube approximation
                            tetree.entitiesInRegion(new Spatial.Cube(center.x - 50, center.y - 50, center.z - 50, 100));
                            break;
                    }
        ...
        System.out.printf("  Completed %d operations across %d threads in %.2f ms (%.0f ops/sec)%n%n", totalOps.get(),
                          numThreads, elapsed / 1_000_000.0, rate);
    }
    ...
    private void profileFrustumCulling() {
        ...
        for (int i = 0; i < 1000; i++) {
            ...
            Point3f lookAt = new Point3f(random.nextFloat() * 1000, random.nextFloat() * 1000,
                                         random.nextFloat() * 1000);

            // Create frustum
            Frustum3D frustum = Frustum3D.createPerspective(cameraPos, lookAt, new Vector3f(0, 1, 0), // up vector
        ...
        System.out.printf("  Total: 1000 frustum queries in %.2f ms, avg visible: %.1f%n", elapsed / 1_000_000.0,
                          avgVisible);
    }

    private void profileInsertionPattern(String patternName,
                                         java.util.function.Function<Integer, Point3f> positionGenerator) {
        ...
        System.out.printf("  Total: %d entities in %.2f ms (%.0f/sec)%n", batchSize, elapsed / 1_000_000.0, rate);
    }
    ...
    private void profileKNNSearches() {
        ...
        System.out.printf("  Total: %d k-NN searches in %.2f ms (%.0f/sec), avg found: %.1f%n", KNN_SEARCHES,
                          elapsed / 1_000_000.0, rate, avgFound);
    }
    ...
    private void profileMemoryPressure() {
        ...
        System.out.printf("  Total memory growth: %d MB in %.2f ms%n%n", totalUsed, elapsed / 1_000_000.0);
    }
    ...
    private void profilePointLookups() {
        ...
        System.out.printf("  Total: %d lookups in %.2f ms (%.0f/sec), hit rate: %.1f%%%n", QUERY_ITERATIONS,
                          elapsed / 1_000_000.0, rate, (hits * 100.0) / QUERY_ITERATIONS);
    }
    ...
    private void profileRangeQueries() {
        ...
        System.out.printf("  Total: %d range queries in %.2f ms (%.0f/sec), avg found: %.1f%n", RANGE_QUERIES,
                          elapsed / 1_000_000.0, rate, avgFound);
    }
    ...
    private void profileTraversalOrder(String orderName, java.util.function.Supplier<Integer> traversal) {
        ...
        System.out.printf("  Visited %d nodes in %.2f ms (%.0f nodes/sec)%n", nodeCount, elapsed / 1_000_000.0, rate);
    }
    ...
    private void profileTreeTraversal() {
        ...
        profileTraversalOrder("Depth-First", () -> {
            int count = 0;
            var iterator = tetree.iterator(
            com.hellblazer.luciferase.lucien.tetree.TetreeIterator.TraversalOrder.DEPTH_FIRST_PRE);
        ...
        profileTraversalOrder("Breadth-First", () -> {
            int count = 0;
            var iterator = tetree.iterator(
            com.hellblazer.luciferase.lucien.tetree.TetreeIterator.TraversalOrder.BREADTH_FIRST);
            while (iterator.hasNext()) {
                iterator.next();
                count++;
            }
        ...
        profileTraversalOrder("SFC Order", () -> {
            int count = 0;
            var iterator = tetree.iterator(
            com.hellblazer.luciferase.lucien.tetree.TetreeIterator.TraversalOrder.SFC_ORDER);
            while (iterator.hasNext()) {
                iterator.next();
                count++;
            }
    ...
    private void profileUpdateOperations() {
        ...
        for (int iteration = 0; iteration < 10; iteration++) {
            for (LongEntityID id : movingEntities) {
                ...
                if (entity != null) {
                    ...
                    Point3f newPos = new Point3f(oldPos.x + (random.nextFloat() - 0.5f) * 10,
            ...
            System.out.printf("  Iteration %d: %d moves (%.0f moves/sec)%n", iteration + 1, movingEntities.size(),
                              rate);
        }
        ...
        System.out.printf("  Total: %d moves in %.2f ms (%.0f/sec)%n%n", totalMoves, elapsed / 1_000_000.0, rate);
    }
    ...
    private void warmUp() {
        ...
        tetree = new Tetree<>(idGenerator, 50, (byte) 12,
                              new EntitySpanningPolicy(EntitySpanningPolicy.SpanningStrategy.SPAN_TO_OVERLAPPING, true,
                                                       0.1f));
        insertedIds.clear();
