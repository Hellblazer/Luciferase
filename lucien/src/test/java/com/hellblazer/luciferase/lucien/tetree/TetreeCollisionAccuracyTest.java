...
/**
 * Accuracy tests for Tetree collision detection.
 * Validates geometric accuracy, contact points, normals, and penetration depths
 * within the tetrahedral spatial decomposition.
 *
...
public class TetreeCollisionAccuracyTest {
    ...
    void testContactPointAccuracyInTetrahedralSpace() {
        ...
        EntityBounds bounds1 = new EntityBounds(
            new Point3f(195, 195, 195),
            new Point3f(205, 205, 205)
        );
        EntityBounds bounds2 = new EntityBounds(
            new Point3f(203, 195, 195),   // 2 unit overlap
    ...
    void testMultiLevelCollisionAccuracy() {
        ...
        for (byte level : levels) {
            ...
            EntityBounds bounds1 = new EntityBounds(
                new Point3f(95 * scale, 95 * scale, 95 * scale),
                new Point3f(105 * scale, 105 * scale, 105 * scale)
            );
            EntityBounds bounds2 = new EntityBounds(
                new Point3f(102 * scale, 95 * scale, 95 * scale),
            ...
            Optional<SpatialIndex.CollisionPair<LongEntityID, String>> collision = 
                levelTetree.checkCollision(id1, id2);
            assertTrue(collision.isPresent(), 
                      "Collision should be detected at level " + level);
            
            // Verify penetration depth scales correctly
            float expectedPenetration = 3.0f * scale;
            assertEquals(expectedPenetration, collision.get().penetrationDepth(), 
                        EPSILON * scale,
    ...
    void testAnisotropicCollisionAccuracy() {
        ...
        EntityBounds elongatedBounds = new EntityBounds(
            new Point3f(295, 280, 295),
            new Point3f(305, 320, 305)  // 10x40x10
        );
        
        // Wide box along Z axis
        EntityBounds wideBounds = new EntityBounds(
            new Point3f(305, 295, 280),
            new Point3f(315, 305, 320)  // 10x10x40
        );
    ...
    void testCapsuleCollisionAccuracyInTetree() {
        ...
        Optional<SpatialIndex.CollisionPair<LongEntityID, String>> overlap = 
            tetree.checkCollision(id3, id4);
    ...
    void testMultipleSimultaneousCollisionsAccuracy() {
        ...
        EntityBounds centralBounds = new EntityBounds(
            new Point3f(390, 390, 390),
            new Point3f(410, 410, 410)
        );
        ...
        float offset = 15;
        Point3f[] tetVertices = {
            new Point3f(center.x + offset, center.y, center.z),
            new Point3f(center.x - offset/2, center.y + offset * 0.866f, center.z),
            new Point3f(center.x - offset/2, center.y - offset * 0.433f, center.z + offset * 0.75f),
            new Point3f(center.x - offset/2, center.y - offset * 0.433f, center.z - offset * 0.75f)
        };
        ...
        List<SpatialIndex.CollisionPair<LongEntityID, String>> collisions = 
            tetree.findCollisions(centralId);
        
        assertEquals(4, collisions.size(), 
                    "Central entity should collide with all 4 surrounding entities");
        ...
        for (var collision : collisions) {
            ...
            assertEquals(1.0f, collision.contactNormal().length(), EPSILON,
    ...
    void testCollisionNormalConsistency() {
        ...
        EntityBounds bounds1 = new EntityBounds(
            new Point3f(295, 295, 295),
            new Point3f(305, 305, 305)
        );
        EntityBounds bounds2 = new EntityBounds(
            new Point3f(300, 295, 295),
            new Point3f(310, 305, 305)
        );
        ...
        Optional<SpatialIndex.CollisionPair<LongEntityID, String>> collision12 = 
            tetree.checkCollision(id1, id2);
        Optional<SpatialIndex.CollisionPair<LongEntityID, String>> collision21 = 
            tetree.checkCollision(id2, id1);
        ...
        assertTrue(Math.abs(dot) > 0.99f, 
                  "Normals should be either same or opposite direction");
        
        // Penetration depths should be the same
        assertEquals(collision12.get().penetrationDepth(), 
                    collision21.get().penetrationDepth(), 
                    EPSILON,
    ...
    void testHighPrecisionCollisionDetection() {
        ...
        for (int i = 0; i < gridSize; i++) {
            for (int j = 0; j < gridSize; j++) {
                ...
                tetree.setCollisionShape(grid[i][j], 
                    new SphereShape(pos, spacing * 0.6f));
            }
        }
        ...
        for (int i = 0; i < gridSize - 1; i++) {
            for (int j = 0; j < gridSize - 1; j++) {
                ...
                Optional<SpatialIndex.CollisionPair<LongEntityID, String>> hCollision = 
                    tetree.checkCollision(grid[i][j], grid[i+1][j]);
                assertTrue(hCollision.isPresent(), 
                          "Horizontal neighbors should collide at (" + i + "," + j + ")");
                
                // Check vertical neighbor
                Optional<SpatialIndex.CollisionPair<LongEntityID, String>> vCollision = 
                    tetree.checkCollision(grid[i][j], grid[i][j+1]);
                assertTrue(vCollision.isPresent(), 
                          "Vertical neighbors should collide at (" + i + "," + j + ")");
                
                // Verify penetration depth is consistent
                float expectedPenetration = 2 * spacing * 0.6f - spacing;
                assertEquals(expectedPenetration, hCollision.get().penetrationDepth(), 
                            spacing * 0.01f, "Horizontal penetration should be accurate");
                assertEquals(expectedPenetration, vCollision.get().penetrationDepth(), 
                            spacing * 0.01f, "Vertical penetration should be accurate");
            }
        }
    }
    ...
    void testCollisionSeparationVectorsInTetree() {
        ...
        EntityBounds bounds1 = new EntityBounds(
            new Point3f(345, 345, 345),
            new Point3f(355, 355, 355)
        );
        EntityBounds bounds2 = new EntityBounds(
            new Point3f(350, 345, 345),
            new Point3f(360, 355, 355)
        );
        ...
        Optional<SpatialIndex.CollisionPair<LongEntityID, String>> afterSeparation = 
            tetree.checkCollision(id1, id2);
        assertFalse(afterSeparation.isPresent(), 
                   "Entities should not collide after separation");
    }
}