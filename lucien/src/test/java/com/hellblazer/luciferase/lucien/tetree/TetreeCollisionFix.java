 ...
 * 
 * The issue: Entities in different tetrahedra within the same grid cell
 * aren't being checked for collisions.
 * 
 * The solution: When checking collisions, also check all 6 tetrahedra
 * within the same grid cell, not just neighboring grid cells.
 */
public class TetreeCollisionFix {
     ...
     */
    public static Set<BaseTetreeKey<?>> getTetrahedraInSameCell(Tet tet) {
        var indices = new HashSet<BaseTetreeKey<?>>();
     ...
     * 
     * Instead of just checking neighboring grid cells, we also need to check
     * all tetrahedra within the same grid cell.
     */
