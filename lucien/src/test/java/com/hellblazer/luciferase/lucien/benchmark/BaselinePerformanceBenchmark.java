...
/**
 * Baseline performance benchmark for spatial indices. Can be run directly without environment variables.
 *
...
public class BaselinePerformanceBenchmark {
    ...
    public void runBaselineBenchmark() {
        ...
        for (int count : ENTITY_COUNTS) {
            ...
            System.out.printf("  Octree Optimized:  %,8d ms (%.2fx speedup)%n", octreeOptimized,
                              (double) octreeBasic / octreeOptimized);
            System.out.printf("  Tetree Basic:      %,8d ms%n", tetreeBasic);
            System.out.printf("  Tetree Optimized:  %,8d ms (%.2fx speedup)%n", tetreeOptimized,
                              (double) tetreeBasic / tetreeOptimized);
            ...
            if (count <= 50_000) { // Only test queries on smaller datasets
                ...
                System.out.printf("  Octree queries: %,d ms (%.2f ms/query)%n", octreeQueryTime,
                                  octreeQueryTime / 100.0);
                System.out.printf("  Tetree queries: %,d ms (%.2f ms/query)%n", tetreeQueryTime,
                                  tetreeQueryTime / 100.0);
            }
        }
    ...
    private long benchmarkQueries(com.hellblazer.luciferase.lucien.SpatialIndex<?, LongEntityID, String> index,
    ...
    private Octree<LongEntityID, String> createAndPopulateOctree(List<Point3f> positions, List<String> contents,
                                                                 boolean optimized) {
        Octree<LongEntityID, String> octree = new Octree<>(new SequentialLongIDGenerator(), 100, (byte) 20);
        if (optimized) {
            byte optimalLevel = LevelSelector.selectOptimalLevel(positions, 100);
    ...
    private Tetree<LongEntityID, String> createAndPopulateTetree(List<Point3f> positions, List<String> contents,
                                                                 boolean optimized) {
        Tetree<LongEntityID, String> tetree = new Tetree<>(new SequentialLongIDGenerator(), 100, (byte) 20);
        if (optimized) {
            byte optimalLevel = LevelSelector.selectOptimalLevel(positions, 100);
    ...
    private List<Point3f> generateUniformData(int count) {
        ...
        for (int i = 0; i < count; i++) {
            positions.add(new Point3f(random.nextFloat() * 1000, random.nextFloat() * 1000, random.nextFloat() * 1000));
        }
