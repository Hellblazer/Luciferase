...
public class OctreeBulkOptimizationTest {
    ...
    void testBulkInsertWithOptimizations() {
        ...
        System.out.println("===========================================");
        System.out.printf("%-10s | %-15s | %-15s | %-15s | %-10s%n", "Entities", "Sequential (ms)", "Default (ms)",
        ...
        for (int entityCount : entityCounts) {
            ...
            System.out.printf("%-10d | %15.2f | %15.2f | %15.2f | %10.2fx%n", entityCount, sequentialMs, defaultMs,
                              optimizedMs, speedup);
        }
    ...
    void testDifferentSpatialDistributions() {
        ...
        System.out.println("========================================");
        System.out.printf("%-20s | %-15s | %-15s | %-10s%n", "Distribution", "Sequential (ms)", "Bulk (ms)", "Speedup");
        System.out.println("---------------------|-----------------|-----------------|------------");
    ...
    void testMemoryUsage() {
        ...
        System.out.printf("Average entities per node: %.2f%n", (double) entityCount / octree.nodeCount());
    }
    ...
    private List<EntityData<LongEntityID, String>> generateClustered(int count, byte level) {
        ...
        for (int c = 0; c < clusterCount; c++) {
            ...
            Point3f center = new Point3f(random.nextFloat() * 10000, random.nextFloat() * 10000,
                                         random.nextFloat() * 10000);

            // Generate points around cluster
            for (int i = 0; i < entitiesPerCluster; i++) {
                Point3f pos = new Point3f(center.x + (random.nextFloat() - 0.5f) * 100,
    ...
    private void testDistribution(String name, int count, byte level, DistributionGenerator generator) {
        ...
        System.out.printf("%-20s | %15.2f | %15.2f | %10.2fx%n", name, seqMs, bulkMs, speedup);
    }
