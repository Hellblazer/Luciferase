...
/**
 * Performance regression test suite that tracks performance over time and fails if performance degrades beyond
...
public class PerformanceRegressionTest {
        ...
        if (Files.exists(baselinePath)) {
            try (BufferedReader reader = Files.newBufferedReader(baselinePath)) {
                reader.lines().skip(1) // Skip header
                      .map(PerformanceMetric::fromCsv).forEach(metric -> {
    ...
    void saveResults() throws IOException {
        ...
        Files.newBufferedWriter(historyPath, StandardOpenOption.CREATE, StandardOpenOption.APPEND))) {
            if (writeHeader) {
    ...
    void testOptimizationEffectiveness() throws Exception {
        ...
        testOptimizationCombination("No optimizations", testSize, positions, contents, new BulkOperationConfig());
    ...
    private void checkMemoryRegression(PerformanceMetric current) {
        ...
        if (baseline != null && baseline.memoryUsed > 0) {
            ...
            if (regression > ALLOWED_REGRESSION) {
                ...
                "Memory regression detected for %s %s (%d entities): " + "%.1f%% more memory than baseline",
                current.testName, current.indexType, current.entityCount, regression);
    ...
    private void checkRegression(PerformanceMetric current) {
        ...
        if (baseline != null) {
            ...
            if (regression > ALLOWED_REGRESSION) {
                String message = String.format("Performance regression detected for %s %s (%d entities): "
                                               + "%.1f%% slower than baseline (current: %dms, baseline: %dms)",
                                               current.testName, current.indexType, current.entityCount, regression,
                                               current.duration, baseline.duration);
    ...
    private List<String> generateContents(int count) {
        return IntStream.range(0, count).mapToObj(i -> "Entity_" + i).collect(Collectors.toList());
    }
    ...
    private void testMemoryUsage(String indexType, int size, IndexFactory factory) throws Exception {
        ...
        System.out.printf("%-20s %s: %,d entities using %,d KB (%.2f bytes/entity)\n", testName, indexType, size,
                          memUsed / 1024, (double) memUsed / size);
    }

    private void testOptimizationCombination(String name, int size, List<Point3f> positions, List<String> contents,
    ...
    private <Key extends com.hellblazer.luciferase.lucien.SpatialKey<Key>> void testRangeQuery(
    com.hellblazer.luciferase.lucien.SpatialIndex<Key, LongEntityID, String> index, String indexType, int dataSize) {
    ...
    static class PerformanceMetric {
        ...
        PerformanceMetric(String testName, String indexType, int entityCount, long duration, long memoryUsed) {
        ...
        static PerformanceMetric fromCsv(String line) {
            ...
            PerformanceMetric metric = new PerformanceMetric(parts[1], parts[2], Integer.parseInt(parts[3]),
                                                             Long.parseLong(parts[4]), Long.parseLong(parts[6]));
            return metric;
        }

        String toCsv() {
            return String.format("%s,%s,%s,%d,%d,%.0f,%d", timestamp.format(DateTimeFormatter.ISO_LOCAL_DATE_TIME),
