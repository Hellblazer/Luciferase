...
public class ConcurrentSpatialIndexTest {
    ...
    throws InterruptedException {
                ...
                try {
                    ...
                    for (int j = 0; j < OPERATIONS_PER_THREAD; j++) {
                        try {
                            ...
                            index.insert(id, new Point3f(x, y, z), (byte) 10, "Entity-" + threadId + "-" + j);
        ...
        // Wait for completion
        assertTrue(endLatch.await(TEST_TIMEOUT_SECONDS, TimeUnit.SECONDS), "Threads did not complete in time");

        // Verify results
        assertEquals(0, errorCount.get(), "No errors should occur during concurrent inserts");
        assertEquals(THREAD_COUNT * OPERATIONS_PER_THREAD, successCount.get(), "All inserts should succeed");
        ...
        for (int x = 0; x < 10; x++) {
            for (int y = 0; y < 10; y++) {
                for (int z = 0; z < 10; z++) {
                    ...
                    index.insert(id, new Point3f(x * 100, y * 100, z * 100), (byte) 10,
                                 "Grid-" + x + "-" + y + "-" + z);
                }
            }
        }
            ...
            Thread thread = new Thread(() -> {
                try {
                    ...
                    for (int j = 0; j < OPERATIONS_PER_THREAD / 10; j++) { // Fewer k-NN ops as they're expensive
                        try {
                            Point3f queryPoint = new Point3f(random.nextFloat() * 900, random.nextFloat() * 900,
    ...
    throws InterruptedException {
        ...
        for (int i = 0; i < 1000; i++) {
            index.insert(new LongEntityID(i), new Point3f(i * 10, i * 10, i * 10), (byte) 10, "Initial-" + i);
        }
                    ...
                    for (int j = 0; j < OPERATIONS_PER_THREAD; j++) {
                        try {
                            ...
                            if (operation < 40) { // 40% inserts
                                ...
                                index.insert(id, new Point3f(random.nextFloat() * 1000, random.nextFloat() * 1000,
                                                             random.nextFloat() * 1000), (byte) 10, "Mixed-" + id);
                                ...
                                try {
                                    if (index.containsEntity(id)) {
                                        index.updateEntity(id, new Point3f(random.nextFloat() * 1000,
                                                                           random.nextFloat() * 1000,
                                                                           random.nextFloat() * 1000), (byte) 10);
                                    }
        ...
        assertEquals(THREAD_COUNT * OPERATIONS_PER_THREAD, operationCount.get(), "All operations should complete");
