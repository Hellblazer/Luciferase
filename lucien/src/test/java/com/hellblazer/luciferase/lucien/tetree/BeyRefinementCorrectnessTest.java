 ...
 *
 * Bey's tetrahedral refinement has these properties: 1. Volume is conserved (8 children have same total volume as
 * parent) 2. Children tile space without gaps 3. Each child's tm-index correctly encodes its position 4. Corner
 * children (Bey IDs 1-4) have one vertex at a parent vertex 5. Interior children (Bey IDs 5-8) form an octahedron in
 * the middle
 */
public class BeyRefinementCorrectnessTest {
    ...
    void testChildPositionsAreGridAligned() {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            assertTrue(childPositions.add(posKey), String.format("Duplicate child position for child %d", i));
        }
    ...
    void testChildTmIndicesAreUnique() {
        ...
        Set<TetreeKey<?>> childKeys = new HashSet<>();

        for (int i = 0; i < 8; i++) {
            var child = parent.child(i);
            var childKey = child.tmIndex();

            assertTrue(childKeys.add(childKey), String.format("Child %d has duplicate tm-index", i));
    ...
    void testCornerChildrenExist() {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            assertTrue(beyIds.add((int) beyId), String.format("Duplicate Bey ID %d for child %d", beyId, i));

            System.out.printf("Child %d: Bey ID %d, type %d, anchor=(%d,%d,%d)\n", i, beyId, child.type(), child.x(),
                              child.y(), child.z());
        }
    ...
    void testRefinementCoverage() {
        ...
        for (int i = 0; i < numSamples; i++) {
            ...
            if (childCount == 0) {
                ...
                pointsInExactlyOneChild++;
            } else {
                pointsInMultipleChildren++;
            }
        }

        System.out.printf("Sampled %d points:\n", numSamples);
        System.out.printf("  In exactly one child: %d (%.1f%%)\n", pointsInExactlyOneChild,
                          (100.0 * pointsInExactlyOneChild / numSamples));
        ...
        System.out.printf("  In multiple children: %d (%.1f%%)\n", pointsInMultipleChildren,
                          (100.0 * pointsInMultipleChildren / numSamples));
    ...
    void testVolumeConservation() {
        ...
        for (byte level = 0; level < 5; level++) {
            ...
            System.out.printf("Level %d: parent=%.0f, children=%.0f, ratio=%.6f\n", level, parentVolume,
                              totalChildVolume, ratio);

            assertEquals(1.0, ratio, 1e-10, String.format("Volume not conserved at level %d", level));
        }
    }
    ...
    private double computeTetrahedronVolume(Point3i[] vertices) {
        ...
        var det = a.x * (b.y * c.z - b.z * c.y) - a.y * (b.x * c.z - b.z * c.x) + a.z * (b.x * c.y - b.y * c.x);
