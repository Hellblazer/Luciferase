...
public class TetTest {
        ...
        for (int[] testCase : testCases) {
            ...
            if (expectedLevel > 0) {
                ...
                if (parentLevel != expectedLevel - 1) {
                    throw new AssertionError(String.format("Parent level incorrect: expected %d, got %d", 
                        expectedLevel - 1, parentLevel));
                }
                ...
                if (!foundChild) {
                    System.out.printf("  Warning: Parent-child geometric consistency issue for index %d (SFC operations prioritized)%n", index);
                }
            }
            ...
            if (expectedLevel < Constants.getMaxRefinementLevel() - 1) {
                for (byte i = 0; i < 8; i++) {
                    ...
                    if (child.l() != expectedLevel + 1) {
                        throw new AssertionError(String.format("Child level incorrect: expected %d, got %d", 
                            expectedLevel + 1, child.l()));
                    }
    ...
    private void validateCoordinateMapping() {
        ...
        for (byte level = 0; level <= 2; level++) {
            ...
            for (long index = 0; index < Math.min(maxIndex, 64); index++) {
                ...
                if (tet.x() < 0 || tet.x() > maxCoord || 
                    tet.y() < 0 || tet.y() > maxCoord ||
                    tet.z() < 0 || tet.z() > maxCoord) {
                    throw new AssertionError(String.format("Coordinates out of bounds for index %d, level %d: (%d,%d,%d)", 
                        index, level, tet.x(), tet.y(), tet.z()));
                }
                
                // Verify type is valid
                if (tet.type() < 0 || tet.type() > 5) {
                    throw new AssertionError(String.format("Invalid type %d for index %d, level %d", 
                        tet.type(), index, level));
                }
                
                // Test coordinate to index roundtrip
                Tet reconstructed = new Tet(tet.x(), tet.y(), tet.z(), level, tet.type());
                if (reconstructed.index() != index) {
                    throw new AssertionError(String.format("Coordinate roundtrip failed: index %d -> coords (%d,%d,%d) -> index %d", 
                        index, tet.x(), tet.y(), tet.z(), reconstructed.index()));
                }
            }
        }
    ...
    private void validateTypeTransitions() {
        ...
        int[][] c_parenttype_localindex_to_type = {
            {0, 0, 4, 5, 0, 1, 2, 0},
            {1, 1, 2, 3, 0, 1, 5, 1},
            {2, 0, 1, 2, 2, 3, 4, 2},
            {3, 3, 4, 5, 1, 2, 3, 3},
        ...
        for (byte parentType = 0; parentType < 6; parentType++) {
            for (byte localIndex = 0; localIndex < 8; localIndex++) {
                ...
                if (javaChildType != cChildType) {
                    throw new AssertionError(String.format("TYPE_TO_TYPE_OF_CHILD_MORTON mismatch at [%d][%d]: Java=%d, C=%d", 
                        parentType, localIndex, javaChildType, cChildType));
                }
                ...
                if (javaType != cType) {
                    throw new AssertionError(String.format("PARENT_TYPE_LOCAL_INDEX_TO_TYPE mismatch at [%d][%d]: Java=%d, C=%d", 
                        parentType, localIndex, javaType, cType));
                }
            }
        }
        ...
        for (byte level = 1; level <= 2; level++) {
            ...
            for (long index = 0; index < Math.min(maxIndex, 64); index++) {
                ...
                if (calculatedIndex != index) {
                    throw new AssertionError(String.format("Index calculation inconsistent: expected %d, calculated %d", 
                ...
                if (level > 0) {
                    ...
                    if (parentIndex != expectedParentIndex) {
                        throw new AssertionError(String.format("Parent index calculation wrong: child=%d, parent=%d, expected=%d", 
    ...
    private void validateBoundaryConditions() {
        ...
        for (long index : testIndices) {
            if (index >= (1L << (3 * maxLevel))) continue; // Skip if too large
    ...
    public void compareCAndJavaLookupTables() {
        ...
        for (int parentType = 0; parentType < 6; parentType++) {
            for (int localIndex = 0; localIndex < 8; localIndex++) {
                ...
                if (javaValue != cValue) {
                    System.out.printf("  ❌ Mismatch at [%d][%d]: Java=%d, C=%d%n", parentType, localIndex, javaValue, cValue);
                    table5Match = false;
                }
            }
        }
    ...
    public void verifyLookupTables() {
        ...
        assertEquals(6, Constants.CUBE_ID_TYPE_TO_PARENT_TYPE[0].length, "CUBE_ID_TYPE_TO_PARENT_TYPE should have 6 columns");
        assertEquals(6, Constants.TYPE_CUBE_ID_TO_LOCAL_INDEX.length, "TYPE_CUBE_ID_TO_LOCAL_INDEX should have 6 rows");
        assertEquals(8, Constants.TYPE_CUBE_ID_TO_LOCAL_INDEX[0].length, "TYPE_CUBE_ID_TO_LOCAL_INDEX should have 8 columns");
        assertEquals(6, Constants.PARENT_TYPE_LOCAL_INDEX_TO_CUBE_ID.length, "PARENT_TYPE_LOCAL_INDEX_TO_CUBE_ID should have 6 rows");
        assertEquals(8, Constants.PARENT_TYPE_LOCAL_INDEX_TO_CUBE_ID[0].length, "PARENT_TYPE_LOCAL_INDEX_TO_CUBE_ID should have 8 columns");
        assertEquals(6, Constants.PARENT_TYPE_LOCAL_INDEX_TO_TYPE.length, "PARENT_TYPE_LOCAL_INDEX_TO_TYPE should have 6 rows");
        assertEquals(8, Constants.PARENT_TYPE_LOCAL_INDEX_TO_TYPE[0].length, "PARENT_TYPE_LOCAL_INDEX_TO_TYPE should have 8 columns");
        ...
        for (byte type = 0; type < 6; type++) {
            ...
            for (byte localIndex = 0; localIndex < 8; localIndex++) {
                ...
                System.out.printf("    localIndex=%d -> cubeId=%d,type=%d | reverse[%d][%d]=%d%n", 
                    localIndex, cubeId, childType, type, cubeId, reverseLookup);
            }
        }
        ...
        for (int i = 0; i < 6; i++) {
            ...
            int det = a[0] * (b[1] * c[2] - b[2] * c[1]) - 
                      a[1] * (b[0] * c[2] - b[2] * c[0]) + 
                      a[2] * (b[0] * c[1] - b[1] * c[0]);
            
            System.out.printf("    Volume determinant: %d (should be ±1 for unit tetrahedra)%n", det);
            assertTrue(Math.abs(det) == 1, "Tetrahedron " + i + " should have unit volume");
        }
    ...
    void debugSFCIndexMapping() {
        ...
        for (int index = 0; index < 8; index++) {
            ...
            System.out.printf("Index %d -> (%d,%d,%d) type=%d%n", 
                index, reconstructed.x(), reconstructed.y(), reconstructed.z(), reconstructed.type());
        }
        ...
        for (int level = 0; level <= 3; level++) {
            ...
            for (long index = startIndex; index < Math.min(endIndex, startIndex + 8); index++) {
                ...
                if (calculatedLevel != level) {
                    System.out.printf("Level calculation error: index %d, expected level %d, got %d%n", 
                        index, level, calculatedLevel);
                }
                assertEquals(level, calculatedLevel, 
                    "Level calculation failed for index " + index);
            }
        }
        ...
        for (long index = 0; index < 64; index++) {
            try {
                ...
                assertEquals(index, reconstructedIndex, 
                    "SFC roundtrip failed for index " + index);
            } catch (Exception e) {
    ...
    void debugFailingCase() {
        ...
        System.out.printf("tetrahedron(2, 1) = (%d,%d,%d) level=%d type=%d%n", 
        ...
        for (int i = 1; i <= level; i++) {
            ...
            System.out.printf("  PARENT_TYPE_LOCAL_INDEX_TO_CUBE_ID[%d][%d] = %d%n", type, extractedLocalIndex, cid_forward);
    ...
    void debugTetrahedronReconstruction() {
        ...
        for (int i = 1; i <= level; i++) {
            ...
            System.out.printf("    Extract: (index >> %d) & 7 = (%d >> %d) & 7 = %d%n", 
                3 * offsetIndex, index, 3 * offsetIndex, localIndex);
            ...
            System.out.printf("    After coords: [%d,%d,%d] (cubeId %d = %s)%n", 
                coordinates[0], coordinates[1], coordinates[2], cid, Integer.toBinaryString(cid));
        }
        
        System.out.printf("Final result: (%d,%d,%d) level=%d type=%d%n", 
            coordinates[0], coordinates[1], coordinates[2], level, type);
    }
    ...
    void debugCoordinateMappings() {
        ...
        for (int x = 0; x <= h; x += h) {
            for (int y = 0; y <= h; y += h) {
                for (int z = 0; z <= h; z += h) {
                    if (x == 0 && y == 0 && z == 0) continue; // Skip root
                    
                    Tet tet = new Tet(x, y, z, (byte)1, (byte)0);
                    byte cubeId = tet.cubeId((byte)1);
                    System.out.printf("Coords (%d,%d,%d) -> cubeId=%d (binary: %s)%n", 
                        x, y, z, cubeId, Integer.toBinaryString(cubeId));
                }
            }
        }
        ...
        for (byte level = 0; level <= 2; level++) {
            ...
            for (int index = (level == 0 ? 0 : (1 << (3 * (level - 1)))); index < maxIndex; index++) {
                ...
                if (index != reconstructedIndex) {
                    System.out.printf("  FAIL: index %d -> tet(%d,%d,%d,level=%d,type=%d) -> index %d%n",
                        index, originalTet.x(), originalTet.y(), originalTet.z(), originalTet.l(), originalTet.type(), reconstructedIndex);
                }
                
                assertEquals(index, reconstructedIndex, 
                    "Index roundtrip failed: %d -> tet(%d,%d,%d,level=%d,type=%d) -> %d"
                    .formatted(index, originalTet.x(), originalTet.y(), originalTet.z(), originalTet.l(), originalTet.type(), reconstructedIndex));
                
                // Also verify level calculation
                var calculatedLevel = Tet.tetLevelFromIndex(index);
                assertEquals(level, calculatedLevel, 
                    "Level calculation failed for index %d: expected %d, got %d"
                    .formatted(index, level, calculatedLevel));
            }
        }
    }
    ...
    public void baySubdivisionGeometry() {
        ...
        for (byte childIndex = 0; childIndex < 8; childIndex++) {
            ...
            assertEquals(parentTet.l() + 1, childTet.l(), 
                "Child level incorrect for child %d".formatted(childIndex));
        }
    }
}
