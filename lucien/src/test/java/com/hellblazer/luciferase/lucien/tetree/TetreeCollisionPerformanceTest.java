...
/**
 * Performance benchmarks for Tetree collision detection. Tests scalability and performance characteristics under
 * various load conditions. Accounts for tetrahedral space partitioning constraints (positive coordinates only).
 *
...
public class TetreeCollisionPerformanceTest {
    ...
    void testBoundedEntityCollisionPerformance() {
        ...
        System.out.println(
        "  Bounded checks time: " + boundedCheckTime / 1_000_000 + "ms (" + boundedChecks + " pairs)");
    ...
    void testHighDensityCollisionPerformance() {
        ...
        assertTrue(findEntityTime < TimeUnit.MILLISECONDS.toNanos(100),
                   "Single entity collision search should be fast");
    ...
    void testPositiveCoordinateConstraintPerformance() {
        ...
        for (int i = 0; i < entityCount; i++) {
            Point3f position = new Point3f(0.1f + random.nextFloat() * 2000f, // Small to large positive coordinates
                                           0.1f + random.nextFloat() * 2000f, 0.1f + random.nextFloat() * 2000f);
        ...
        assertTrue(findAllTime < TimeUnit.SECONDS.toNanos(2),
                   "Positive coordinate collision detection should be efficient");
    }
    ...
    void testScalabilityBenchmark() {
        ...
        for (int entityCount : entityCounts) {
            ...
            System.out.println(
            "Tetree Scalability - Entities: " + entityCount + ", Insert: " + insertTime / 1_000_000 + "ms"
            + ", FindAll: " + findAllTime / 1_000_000 + "ms" + ", Collisions: " + collisions.size());

            // Performance should not degrade exponentially
            assertTrue(insertTime < TimeUnit.SECONDS.toNanos(8), "Insert should scale reasonably in tetrahedral space");
            assertTrue(findAllTime < TimeUnit.SECONDS.toNanos(5),
                       "FindAll should scale reasonably in tetrahedral space");
        }
    }
    ...
    void testTetrahedralSpatialLocalityPerformance() {
        ...
        assertTrue(findAllTime < TimeUnit.SECONDS.toNanos(2),
                   "Clustered entities should have good collision performance");
    }
    ...
    void testUpdatePerformance() {
        ...
        for (int iteration = 0; iteration < 10; iteration++) {
            for (int i = 0; i < Math.min(entities.size(), 100); i++) {
                Point3f newPos = new Point3f(10f + random.nextFloat() * 990f, // Ensure positive coordinates
                                             10f + random.nextFloat() * 990f, 10f + random.nextFloat() * 990f);
    ...
    void testWorstCaseScenario() {
        ...
        assertTrue(allCollisions.size() >= minAcceptable,
                   "Should find at least 90% of possible collisions. Expected: " + expectedCollisions + ", Found: "
                   + allCollisions.size() + ", Min acceptable: " + minAcceptable);
    }

    private List<LongEntityID> insertRandomBoundedEntities(int count, float worldSize) {
        List<LongEntityID> entities = new ArrayList<>();
        for (int i = 0; i < count; i++) {
            Point3f center = new Point3f(20f + random.nextFloat() * (worldSize - 40f), // Leave room for bounds
    ...
    private List<LongEntityID> insertRandomEntities(int count, float worldSize) {
        List<LongEntityID> entities = new ArrayList<>();
        for (int i = 0; i < count; i++) {
            Point3f position = new Point3f(1f + random.nextFloat() * (worldSize - 1f), // Ensure positive coordinates
    ...
    private List<LongEntityID> insertTetrahedralClusteredEntities(int clusterCount, int entitiesPerCluster) {
        ...
        Point3f[] tetraVertices = { new Point3f(100, 100, 100), new Point3f(300, 100, 100), new Point3f(200, 300, 100),
        ...
        for (int cluster = 0; cluster < Math.min(clusterCount, tetraVertices.length); cluster++) {
            ...
            for (int i = 0; i < entitiesPerCluster; i++) {
                Point3f position = new Point3f(Math.max(1f, clusterCenter.x + (random.nextFloat() - 0.5f) * 30f),
