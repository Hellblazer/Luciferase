...
/**
 * Performance comparison between original Tetree and optimized TetreeOptimized Demonstrates Phase 2 performance
 * improvements
 */
public class OptimizedTetreePerformanceTest {
    ...
    void compareBoundedByPerformance() {
        ...
        var testQueries = new Spatial.Cube[] { new Spatial.Cube(2000.0f, 2000.0f, 2000.0f, 500.0f),   // Small query
        ...
        for (int i = 0; i < testQueries.length; i++) {
            ...
            // Results
            System.out.printf("Original Tetree:  %d results in %.2f μs%n", originalResults.size(),
                              originalTime / 1000.0);
            System.out.printf("Optimized Tetree: %d results in %.2f μs (%.2fx improvement)%n", optimizedResults.size(),
                              optimizedTime / 1000.0, (double) originalTime / optimizedTime);
            System.out.printf("Octree:           %d results in %.2f μs%n", octreeResults.size(), octreeTime / 1000.0);
            System.out.printf("Tetree vs Octree: %.2fx (Original: %.2fx)%n%n", (double) optimizedTime / octreeTime,
                              (double) originalTime / octreeTime);
        }
    }
    ...
    void testCachingEffectiveness() {
        ...
        System.out.printf("Warm cache:  %d results in %.2f μs (%.2fx improvement)%n", warmResults.size(),
                          warmTime / 1000.0, (double) coldTime / warmTime);
        System.out.printf("Hot cache:   %d results in %.2f μs (%.2fx improvement)%n", hotResults.size(),
                          hotTime / 1000.0, (double) coldTime / hotTime);
    ...
    private void testLargeVolumeQueries(String testName) {
        ...
        System.out.printf("Optimized Tetree: %.2f μs (%.2fx improvement)%n", optimizedTotal / (numQueries * 1000.0),
                          (double) originalTotal / optimizedTotal);
