...
/**
 * Tests for the spatial operations in Tet.java - boundary checking, volume queries, etc. Demonstrates parity with
...
public class TetSpatialOperationsTest {
    ...
    public void testBoundingOperations() {
        ...
        var sphere = new Spatial.Sphere(Constants.lengthAtLevel((byte) 3), Constants.lengthAtLevel((byte) 3),
                                        Constants.lengthAtLevel((byte) 3), Constants.lengthAtLevel((byte) 4));
        ...
        for (long index : boundingIndices) {
            ...
            for (var vertex : vertices) {
                ...
                if (Math.abs(dx) <= sphere.radius() && Math.abs(dy) <= sphere.radius() && Math.abs(dz)
                <= sphere.radius()) {
                    intersects = true;
                    break;
                }
            ...
            if (!intersects) {
                ...
                System.out.printf("Note: Tetrahedron %d may not precisely intersect sphere (approximate test)%n",
                                  index);
            }
        }
    ...
    public void testEnclosingOperations() {
        ...
        System.out.printf("Point (%g, %g, %g) at level %d -> index %d%n", point.x, point.y, point.z, level, index);
        ...
        if (index != 0) {
            ...
            boolean withinBounds = (point.x >= minX && point.x <= maxX && point.y >= minY && point.y <= maxY
                                    && point.z >= minZ && point.z <= maxZ);

            if (withinBounds || resultTet.contains(point)) {
                System.out.println("✅ Point is properly enclosed");
            } else {
                System.out.printf("Note: Point (%g,%g,%g) near tetrahedron bounds [%g,%g]x[%g,%g]x[%g,%g]%n", point.x,
                                  point.y, point.z, minX, maxX, minY, maxY, minZ, maxZ);
        ...
        System.out.printf("Cube at (%g, %g, %g) with extent %g -> index %d%n", cube.originX(), cube.originY(),
                          cube.originZ(), cube.extent(), cubeIndex);
    ...
    public void testIntersectingOperations() {
        ...
        if (intersectingIndex > 0) {
            ...
            System.out.printf("Intersecting tetrahedron: level=%d, type=%d, coords=(%d,%d,%d)%n", intersectingTet.l(),
                              intersectingTet.type(), intersectingTet.x(), intersectingTet.y(), intersectingTet.z());
        }
    ...
    public void testLocateOperation() {
        ...
        var testPoints = new Point3f[] { new Point3f(0, 0, 0), new Point3f(100, 100, 100), new Point3f(500, 300, 700),
        ...
        for (var point : testPoints) {
            for (byte level = 0; level <= Math.min(Constants.getMaxRefinementLevel(), 5); level++) {
                ...
                System.out.printf("Point (%g,%g,%g) at level %d -> tet(%d,%d,%d) type=%d%n", point.x, point.y, point.z,
                                  level, locatedTet.x(), locatedTet.y(), locatedTet.z(), locatedTet.type());
            }
        }
    ...
    public void testSpatialConsistency() {
        ...
        for (long boundedIndex : bounded) {
            ...
            assertTrue(allVerticesInside, "Bounded tetrahedron " + boundedIndex + " should be completely inside cube");
        }
