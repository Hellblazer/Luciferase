...
/**
 * Comprehensive benchmarks for bulk operation optimizations.
 * Compares performance before and after optimization.
 * 
 * Tests measure:
 * - Insertion time reduction
...
public class BulkOperationBenchmark {
    ...
    enum Distribution {
        ...
        CLUSTERED("Clustered"),
        SURFACE_ALIGNED("Surface Aligned"),
    ...
    static class BenchmarkResult {
        ...
        public String toString() {
            return String.format("%-40s: %,10d entities in %,8d ms (%,10.0f entities/sec), Memory: %,d KB",
                testName, entityCount, duration, throughput, memoryUsed / 1024);
        }
    }
    ...
    private void benchmarkIterativeInsertion(String indexType, Distribution dist, 
    ...
    private void benchmarkBasicBulkInsertion(String indexType, Distribution dist, 
                                            List<Point3f> positions, List<String> contents) {
    ...
    private void benchmarkOptimizedBulkInsertion(String indexType, Distribution dist, 
                                                List<Point3f> positions, List<String> contents) {
    ...
    private void benchmarkStackBasedBulkInsertion(String indexType, Distribution dist, 
                                                 List<Point3f> positions, List<String> contents) {
    ...
    private void benchmarkIterativeInsertionTetree(String indexType, Distribution dist, 
                                                  List<Point3f> positions, List<String> contents) {
        String testName = String.format("%s Iterative - %s", indexType, dist.displayName);
    ...
    private void benchmarkBasicBulkInsertionTetree(String indexType, Distribution dist, 
                                                   List<Point3f> positions, List<String> contents) {
        String testName = String.format("%s Basic Bulk - %s", indexType, dist.displayName);
    ...
    private void benchmarkOptimizedBulkInsertionTetree(String indexType, Distribution dist, 
                                                       List<Point3f> positions, List<String> contents) {
        String testName = String.format("%s Optimized Bulk - %s", indexType, dist.displayName);
    ...
    private void benchmarkStackBasedBulkInsertionTetree(String indexType, Distribution dist, 
                                                        List<Point3f> positions, List<String> contents) {
        String testName = String.format("%s Stack-Based Bulk - %s", indexType, dist.displayName);
        ...
        switch (dist) {
            ...
            case CLUSTERED:
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    positions.add(new Point3f(
                        baseX + rand.nextFloat() * 20,
                        baseY + rand.nextFloat() * 20,
                        baseZ + rand.nextFloat() * 20
                    ));
                }
                break;
                        ...
                        if (result.testName.contains(dist.displayName)) {
                            ...
                            if (baseline != null) {
                                ...
                                System.out.printf("%-50s: %.2fx faster, %.1f%% memory reduction\n",
                                    result.testName, improvement, memReduction);
                            }
                            break;
                        }
                    }
