 ...
 *
 * NOTE: The full tetrahedral SFC optimization is not yet implemented due to memory concerns with large level values.
 * Currently using a simpler approach that checks existing nodes for intersection.
 */
public class TetreeSpatialRangeOptimizationTest {
    ...
    void testEdgeCases() {
        // Test empty tree
        com.hellblazer.luciferase.lucien.Spatial.Cube region = new com.hellblazer.luciferase.lucien.Spatial.Cube(0, 0,
                                                                                                                 0,
                                                                                                                 100);
        ...
        byte level = 15;
        // Use a point that's properly inside a tetrahedron
        tetree.insert(new Point3f(32, 32, 32), level, "single");
        ...
        // At level 15, the tetrahedron containing (32,32,32) is in cell (0,0,0) to (64,64,64)
        var missingRegion = new Spatial.Cube(200, 200, 200, 100);
        ...
        // Use a small region centered on the entity position
        com.hellblazer.luciferase.lucien.Spatial.Cube smallRegion = new com.hellblazer.luciferase.lucien.Spatial.Cube(
        30, 30, 30, 5);
        ...
        com.hellblazer.luciferase.lucien.Spatial.Cube edgeRegion = new com.hellblazer.luciferase.lucien.Spatial.Cube(0,
                                                                                                                     0,
                                                                                                                     0,
                                                                                                                     1);
    ...
    void testOptimizedSpatialRangeCorrectness() {
        ...
        com.hellblazer.luciferase.lucien.Spatial.Cube region = new com.hellblazer.luciferase.lucien.Spatial.Cube(200,
                                                                                                                 200,
                                                                                                                 200,
                                                                                                                 400);
        ...
        for (int i = 0; i < allIds.size(); i++) {
            ...
            if (pos.x >= 200 && pos.x <= 600 && pos.y >= 200 && pos.y <= 600 && pos.z >= 200 && pos.z <= 600) {
    ...
    void testPerformanceImprovement() {
        ...
        com.hellblazer.luciferase.lucien.Spatial.Cube region = new com.hellblazer.luciferase.lucien.Spatial.Cube(500,
                                                                                                                 500,
                                                                                                                 500,
                                                                                                                 1000);
        ...
        for (int i = 0; i < allIds.size(); i++) {
            Point3f pos = allPositions.get(i);
            if (pos.x >= 500 && pos.x <= 1500 && pos.y >= 500 && pos.y <= 1500 && pos.z >= 500 && pos.z <= 1500) {
                bruteForceResults.add(allIds.get(i));
            }
        }
        ...
        // Due to proper tetrahedral geometry, the optimized method may find fewer entities
        // than a simple AABB check. This is correct behavior.
        // We verify that:
        ...
        for (LongEntityID id : optimizedResults) {
            ...
            assertTrue(idx >= 0, "Returned ID should exist");
            Point3f pos = allPositions.get(idx);
            assertTrue(pos.x >= 500 && pos.x <= 1500, "X coordinate should be in bounds");
        ...
        System.out.println(
        "Note: With " + tetree.nodeCount() + " nodes, " + (optimizedTime < bruteTime ? "optimized is faster"
