 ...
 * Note: Tetree performance may differ from Octree due to tetrahedral geometry complexity.
 *
...
public class TetreeRayPerformanceTest {
    ...
    void testPositiveCoordinateConstraint() {
        ...
        for (int i = 0; i < numEntities; i++) {
            Point3f pos = new Point3f(
                50.0f + random.nextFloat() * 950.0f, // 50-1000 range (positive)
        ...
        for (int i = 0; i < rays.length; i++) {
            Point3f origin = new Point3f(
                10.0f + random.nextFloat() * 100.0f, // 10-110 range
                10.0f + random.nextFloat() * 100.0f,
                10.0f + random.nextFloat() * 100.0f
            );
            Vector3f direction = new Vector3f(
                random.nextFloat(), // Positive direction components
                random.nextFloat(),
                random.nextFloat()
            );
            direction.normalize();
    ...
    void testTetrahedralBoundaryPerformance() {
        ...
        for (int i = 0; i < numEntities; i++) {
            ...
            Point3f pos = new Point3f(
                boundary + (random.nextFloat() - 0.5f) * 10.0f, // ±5 units around boundary
    ...
    void testRayLengthPerformance() {
        ...
        for (float length : rayLengths) {
            ...
            assertTrue(durationMs < 3000, "Tetree ray intersection should complete reasonably fast regardless of ray length");
        }
    }
    ...
    void testMultiLevelPerformance() {
        ...
        System.out.printf("Tetree Multi-level: %d entities across %d levels, 20 rays, %d ms%n", 
                         totalEntities, levels.length, durationMs);
    ...
    private void insertRandomEntities(int count, float maxCoordinate) {
        for (int i = 0; i < count; i++) {
            Point3f pos = new Point3f(
                10.0f + random.nextFloat() * (maxCoordinate - 10.0f), // Ensure positive coordinates
    ...
    private void insertRandomBoundedEntities(int count, float maxCoordinate, float maxBoundsSize) {
        for (int i = 0; i < count; i++) {
            Point3f pos = new Point3f(
                50.0f + random.nextFloat() * (maxCoordinate - 100.0f), // Leave room for bounds
            ...
            bounds = new EntityBounds(
                new Point3f(Math.max(1.0f, bounds.getMin().x), Math.max(1.0f, bounds.getMin().y), Math.max(1.0f, bounds.getMin().z)),
                new Point3f(Math.max(2.0f, bounds.getMax().x), Math.max(2.0f, bounds.getMax().y), Math.max(2.0f, bounds.getMax().z))
            );
    ...
    private Ray3D[] createRandomRays(int count, float maxCoordinate, float maxDistance) {
        ...
        for (int i = 0; i < count; i++) {
            Point3f origin = new Point3f(
                10.0f + random.nextFloat() * (maxCoordinate / 2), // Positive origin
