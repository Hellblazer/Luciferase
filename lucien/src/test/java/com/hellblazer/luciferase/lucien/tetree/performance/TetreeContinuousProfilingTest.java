 ...
 * This test maintains a steady state of operations to help identify:
 * - Memory leaks
 * - Performance degradation over time
 * - GC pressure
 * - Cache behavior
 * 
 * Run with: -Xmx4g -XX:+UseG1GC -XX:+PrintGCDetails
 * Attach profiler while running to analyze behavior
 * 
...
public class TetreeContinuousProfilingTest {
    ...
    private void performPointLookup() {
        ...
        stats.lookups++;
        if (!results.isEmpty()) stats.lookupHits++;
    }
    ...
    private void reportStatistics(long cycles, long totalTimeMs) {
        ...
        System.out.printf("Cycles: %d (%.1f/sec)%n", cycles, 
                         cycles * 1000.0 / totalTimeMs);
        
        // Operation counts and rates
        System.out.println("\nOperation Counts:");
        System.out.printf("  Inserts: %d (%.0f/sec)%n", stats.inserts, 
                         stats.inserts * 1000.0 / totalTimeMs);
        System.out.printf("  Removes: %d (%.0f/sec)%n", stats.removes,
                         stats.removes * 1000.0 / totalTimeMs);
        System.out.printf("  Updates: %d (%.0f/sec)%n", stats.updates,
                         stats.updates * 1000.0 / totalTimeMs);
        ...
        System.out.printf("  Range: %.1f μs%n", stats.rangeQueries > 0 ? stats.rangeTime / stats.rangeQueries / 1000.0 : 0);
        System.out.printf("  Frustum: %.1f μs%n", stats.frustumCulls > 0 ? stats.frustumTime / stats.frustumCulls / 1000.0 : 0);
