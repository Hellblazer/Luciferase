...
/**
 * Unit tests for TetreeIterator.
 * Validates different traversal orders and level restrictions.
 * 
...
public class TetreeIteratorTest {
    ...
    public void testDepthFirstPreOrder() {
        ...
        assertEquals(tetree.getSortedSpatialIndices().size(), indices.size(), 
    ...
    public void testBreadthFirstOrder() {
        ...
        assertEquals(tetree.getSortedSpatialIndices().size(), nodeCount,
                     "Should visit all nodes");
    }
    ...
    public void testSFCOrder() {
        ...
        for (int i = 1; i < indices.size(); i++) {
            assertTrue(indices.get(i) > indices.get(i-1), 
                "Indices should be in ascending order for SFC traversal");
        }
        
        // Verify we got all nodes
        assertEquals(tetree.getSortedSpatialIndices().size(), indices.size(),
            "Should visit all nodes in SFC order");
    }
