...
/**
 * Unit tests for TetreeConnectivity tables. Validates the precomputed lookup tables match expected Bey refinement
 * properties.
 *
...
public class TetreeConnectivityTest {
    ...
    public void testChildTypes() {
        ...
        // Test specific known relationships — t8code dtet convention (Tet type k IS t8code type k).
        // Type 0 parent: corners (0-3) are type 0; interior (4-7) are t8code type_of_child {4,5,2,1}.
    ...
    public void testChildVertexMapping() {
        ...
        for (int child = 0; child < 8; child++) {
            ...
            for (int vertex = 0; vertex < 4; vertex++) {
                ...
                assertTrue(parentRef >= 0 && parentRef <= 10, "Invalid parent reference point " + parentRef);
            }
        }
        ...
        for (int child = 1; child <= 4; child++) {
            ...
            assertTrue(hasParentVertex, "Corner child " + child + " should have at least one parent vertex");
        }
    }
        ...
        for (byte parentType = 0; parentType < 6; parentType++) {
            for (int face = 0; face < 4; face++) {
                ...
                for (byte child : children) {
                    assertTrue(child >= 0 && child < 8, "Invalid child index " + child);
                }
            }
        }
        ...
        for (byte parentType = 0; parentType < 6; parentType++) {
            for (int childIndex = 0; childIndex < 8; childIndex++) {
                for (int parentFace = 0; parentFace < 4; parentFace++) {
                    ...
                    assertTrue(childFace == -1 || (childFace >= 0 && childFace < 4), "Invalid child face " + childFace);
    ...
    public void testFaceCorners() {
        ...
        for (byte tetType = 0; tetType < 6; tetType++) {
            for (int face = 0; face < 4; face++) {
                ...
                for (byte corner : corners) {
                    assertTrue(corner >= 0 && corner < 4, "Invalid corner index " + corner);
                }
        ...
        for (byte tetType = 0; tetType < 6; tetType++) {
            ...
            for (int face = 0; face < 4; face++) {
                ...
                for (byte corner : corners) {
                    assertNotEquals(face, corner, "Face " + face + " should not contain vertex " + face);
                }
            }
        }
    }
    ...
    public void testFaceNeighborTypes() {
        ...
        for (byte tetType = 0; tetType < 6; tetType++) {
            for (int face = 0; face < 4; face++) {
                ...
                assertTrue(neighborType >= 0 && neighborType < 6, "Invalid neighbor type " + neighborType);
            }
        }
        ...
        for (byte typeA = 0; typeA < 6; typeA++) {
            for (int faceA = 0; faceA < 4; faceA++) {
                ...
                assertTrue(foundReverse, "Should find reverse mapping from type " + typeB + " back to type " + typeA);
            }
        }
    }
    ...
    public void testSiblingRelationships() {
        ...
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                assertTrue(TetreeConnectivity.areSiblings(i, j), "All children of same parent should be siblings");
            }
        }
    }
