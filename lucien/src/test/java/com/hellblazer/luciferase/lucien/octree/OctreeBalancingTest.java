...
public class OctreeBalancingTest {
    ...
    public void setUp() {
        octree = new Octree<>(
            ...
            5,      // Low max entities to trigger splits
            (byte) 10  // max depth
        );
    }
    ...
    public void testNodeSplitting() {
        ...
        Point3f[] positions = new Point3f[] {
            new Point3f(baseCoord, baseCoord, baseCoord),                    // Child 0
            new Point3f(baseCoord + cellSizeLevel6, baseCoord, baseCoord),   // Child 1 (+X)
            new Point3f(baseCoord, baseCoord + cellSizeLevel6, baseCoord),   // Child 2 (+Y)
            new Point3f(baseCoord + cellSizeLevel6, baseCoord + cellSizeLevel6, baseCoord), // Child 3 (+X+Y)
            new Point3f(baseCoord, baseCoord, baseCoord + cellSizeLevel6),   // Child 4 (+Z)
            new Point3f(baseCoord + cellSizeLevel6, baseCoord, baseCoord + cellSizeLevel6)  // Child 5 (+X+Z)
        };
        ...
        for (var node : octree.nodes().toList()) {
            ...
            if (nodeLevel == 5) nodesAtLevel5++;
            if (nodeLevel == 6) nodesAtLevel6++;
        }
        ...
        for (LongEntityID id : entityIds) {
            Point3f pos = octree.getEntityPosition(id);
            var found = octree.kNearestNeighbors(pos, 10, Float.MAX_VALUE); // Search for more neighbors with unlimited distance
    ...
    public void testNodeMerging() {
        ...
        assertTrue(finalStats.nodeCount() <= initialNodes, 
                  "Node count should not increase after removing entities");
        
        // Remaining entities should still be found
        for (int i = 6; i < 8; i++) {
            assertTrue(octree.containsEntity(entityIds.get(i)), 
                      "Remaining entity should exist: " + entityIds.get(i));
        }
    }
    ...
    public void testManualSplitOperation() {
        ...
        TreeBalancingStrategy<LongEntityID> strategy = new DefaultBalancingStrategy<>(
            0.1,    // Very low merge threshold
        ...
        if (result.nodesSplit() > 0) {
            assertTrue(octree.getStats().nodeCount() > 1, 
                      "Should have more nodes after split");
        }
    }
    ...
    public void testChildParentRelationships() {
        ...
        for (var node : nodes) {
            ...
            for (Long childIndex : children) {
                assertTrue(octree.hasNode(childIndex), 
                          "Child node should exist: " + childIndex);
            }
        }
    }
    ...
    public void testMergeWithEmptyNodes() {
        ...
        assertTrue(finalNodeCount < initialNodeCount, 
                  "Should have fewer nodes after removing all entities");
