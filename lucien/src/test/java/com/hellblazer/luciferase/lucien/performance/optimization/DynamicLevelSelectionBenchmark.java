...
public class DynamicLevelSelectionBenchmark {
    ...
    public void benchmarkOctreeDynamicLevelSelection() {
        ...
        for (DataDistribution distribution : DataDistribution.values()) {
            ...
            System.out.println("Size,DynamicLevel,DynamicTime(ms),BestFixedLevel,BestFixedTime(ms),Speedup,SelectedLevel");
            ...
            for (int size : TEST_SIZES) {
                ...
                System.out.printf("%d,%d,%d,%d,%d,%.2fx,L%d%n",
                    size, dynamicTime, dynamicTime, bestFixedLevel, bestFixedTime, speedup, selectedLevel);
            }
        }
    }
    ...
    public void benchmarkAdaptiveSubdivision() {
        ...
        // Test Octree
        {
            ...
            BulkOperationConfig adaptiveConfig = BulkOperationConfig.highPerformance()
                .withAdaptiveSubdivision(true);
            ...
            BulkOperationConfig fixedConfig = BulkOperationConfig.highPerformance()
                .withAdaptiveSubdivision(false);
            ...
            System.out.printf("Octree,%dms,%dms,%d,%d,%.1f%%%n",
                adaptiveTime, fixedTime, adaptiveNodes, fixedNodes, reduction);
        }
        ...
        // Test Tetree
        {
            ...
            double reduction = (1.0 - (double)adaptiveNodes / fixedNodes) * 100;
            System.out.printf("Tetree,%dms,%dms,%d,%d,%.1f%%%n",
                adaptiveTime, fixedTime, adaptiveNodes, fixedNodes, reduction);
        }
    }
    ...
    private List<Point3f> generateData(DataDistribution distribution, int count) {
        ...
        switch (distribution) {
            ...
            case CLUSTERED:
                ...
                for (int i = 0; i < count; i++) {
                    positions.add(new Point3f(
                        centerX + (rand.nextFloat() - 0.5f) * 100,
                        centerY + (rand.nextFloat() - 0.5f) * 100,
            ...
            case MULTI_CLUSTER:
                ...
                for (int c = 0; c < clustersCount; c++) {
                    ...
                    for (int i = 0; i < pointsPerCluster; i++) {
                        positions.add(new Point3f(
                            cx + (rand.nextFloat() - 0.5f) * 50,
                            cy + (rand.nextFloat() - 0.5f) * 50,
