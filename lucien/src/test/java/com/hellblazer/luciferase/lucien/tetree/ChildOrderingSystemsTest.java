...
/**
 * Test demonstrating the three different child ordering systems in Tetree:
 * 1. Bey Order (used by child() method)
 * 2. TM-Index Order (used by childTM() method)
 * 3. Standard/Morton Order (used by childStandard() method)
 */
public class ChildOrderingSystemsTest {
    ...
    public void demonstrateThreeOrderingSystems() {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            // Format output
            System.out.printf("%5d | Type %d at %-6s | Type %d at %-6s | Type %d at %-6s | ",
                i,
                beyChild.type(), formatCoords(beyChild),
                tmChild.type(), formatCoords(tmChild),
                standardChild.type(), formatCoords(standardChild));
