...
/**
 * Unit tests for TetreeNeighborFinder.
 * Validates face neighbor finding algorithms.
 * 
...
public class TetreeNeighborFinderTest {
    ...
    public void testFindFaceNeighbor() {
        ...
        for (int face = 0; face < 4; face++) {
            ...
            if (neighbor != null) {
                ...
                assertEquals(tet.l(), neighbor.l(), 
                    "Neighbor should be at same level");
                
                // Verify neighbor is adjacent
                assertTrue(areAdjacent(tet, neighbor), 
    ...
    public void testAreNeighbors() {
        ...
        if (neighbor != null) {
            // Should be neighbors
            assertTrue(neighborFinder.areNeighbors(tet1, neighbor),
                "Should detect neighbors correctly");
            
            // Should be symmetric
            assertTrue(neighborFinder.areNeighbors(neighbor, tet1),
        ...
        assertFalse(neighborFinder.areNeighbors(tet1, farTet),
            "Far tetrahedra should not be neighbors");
    }
    ...
    public void testFindSharedFace() {
        ...
        for (int face = 0; face < 4; face++) {
            ...
            if (neighbor != null) {
                ...
                assertEquals(face, sharedFace,
                    "Should find correct shared face");
                
                // Find shared face from neighbor's perspective
                int reverseSharedFace = neighborFinder.findSharedFace(neighbor, tet);
                assertTrue(reverseSharedFace >= 0,
                    "Should find shared face from neighbor's perspective");
            }
        }
    }
    ...
    public void testFindNeighborsAtLevel() {
        ...
        assertFalse(sameLevelNeighbors.isEmpty(),
            "Should find neighbors at same level");
        ...
        for (Tet neighbor : coarserNeighbors) {
            assertEquals(2, neighbor.l(),
                "Coarser neighbors should be at requested level");
        }
    }
    ...
    public void testFindNeighborsWithinDistance() {
        ...
        assertEquals(1, distance0.size(),
            "Distance 0 should return only the starting tetrahedron");
        ...
        assertTrue(distance1.size() > 1,
            "Distance 1 should include immediate neighbors");
        assertTrue(distance1.contains(tet),
            "Should include starting tetrahedron");
        
        // Distance 2 should include more tetrahedra
        List<Tet> distance2 = neighborFinder.findNeighborsWithinDistance(tet, 2);
        assertTrue(distance2.size() > distance1.size(),
            "Distance 2 should include more tetrahedra than distance 1");
        
        // All distance 1 tets should be in distance 2
        for (Tet d1 : distance1) {
            assertTrue(distance2.contains(d1),
                "Distance 2 should include all distance 1 tetrahedra");
        }
    }
            ...
            for (int face = 0; face < 4; face++) {
                ...
                if (neighbor != null) {
                    ...
                    assertEquals(expectedType, neighbor.type(),
                        "Neighbor type should match connectivity table for type " + 
                        type + " face " + face);
                }
            }
        }
    ...
    public void testInvalidFaceIndex() {
        ...
        assertThrows(IllegalArgumentException.class, 
            () -> neighborFinder.findFaceNeighbor(tet, -1),
            "Should throw for negative face index");
        
        assertThrows(IllegalArgumentException.class, 
            () -> neighborFinder.findFaceNeighbor(tet, 4),
    ...
    private boolean areAdjacent(Tet tet1, Tet tet2) {
        ...
        return (dx == cellSize && dy == 0 && dz == 0) ||
               (dx == 0 && dy == cellSize && dz == 0) ||
               (dx == 0 && dy == 0 && dz == cellSize) ||
               (dx == cellSize && dy == cellSize && dz == 0) ||
               (dx == cellSize && dy == 0 && dz == cellSize) ||
               (dx == 0 && dy == cellSize && dz == cellSize) ||
               (dx == cellSize && dy == cellSize && dz == cellSize);
    }
}