...
/**
 * Integration tests for Octree collision detection with the full collision system. Tests the complete pipeline
 * including physics, listeners, and filters.
 *
...
public class OctreeCollisionIntegrationTest {
    ...
    void testCollisionDiagnostics() {
        ...
        assertTrue(stats.collisionsDetected() >= 9, "Should detect at least 9 collisions in chain of 10 entities");
    }

    @Test
    void testCollisionFilterIntegration() {
        // Create multiple entities
        Point3f[] positions = { new Point3f(100, 100, 100), new Point3f(100.05f, 100, 100), new Point3f(200, 200, 200),
        ...
        CollisionFilter<LongEntityID, String> filter = collision -> collision.involves(ids[0]);

        CollisionSystem<LongEntityID, String> filteredSystem = new CollisionSystem<>(octree, new CollisionResolver(),
                                                                                     filter);
        ...
        assertTrue(processed.get(0).collision().involves(ids[0]), "Processed collision should involve entity 0");
    }
    ...
    void testCollisionRegionIntegration() {
        ...
        Point3f[] positions = { new Point3f(50, 50, 50), new Point3f(50.05f, 50, 50), new Point3f(150, 150, 150),
                                new Point3f(150.05f, 150, 150), new Point3f(250, 250, 250), new Point3f(250.05f, 250,
                                                                                                        250) };
        ...
        List<SpatialIndex.CollisionPair<LongEntityID, String>> regionCollisions = octree.findCollisionsInRegion(region);
        ...
        List<SpatialIndex.CollisionPair<LongEntityID, String>> regionCollisions2 = octree.findCollisionsInRegion(
        region2);
    ...
    void testContinuousCollisionDetection() {
        ...
        EntityBounds bounds2 = new EntityBounds(new Point3f(195, 95, 95), new Point3f(205, 105, 105));
    ...
    void testFullCollisionPipeline() {
        ...
        EntityBounds bounds1 = new EntityBounds(new Point3f(95, 95, 95), new Point3f(105, 105, 105));
        EntityBounds bounds2 = new EntityBounds(new Point3f(100, 95, 95), new Point3f(110, 105, 105));
    ...
    void testKinematicEntityCollisions() {
        ...
        EntityBounds kinematicBounds = new EntityBounds(new Point3f(95, 95, 95), new Point3f(105, 105, 105));
        ...
        if (!processed.isEmpty()) {
            ...
            if (kinematicIsFirst) {
                assertEquals(0, response.impulse1().length(), 0.001f, "Kinematic entity should receive no impulse");
                assertTrue(response.impulse2().length() > 0, "Dynamic entity should receive impulse from kinematic");
            } else {
                assertTrue(response.impulse1().length() > 0, "Dynamic entity should receive impulse from kinematic");
                assertEquals(0, response.impulse2().length(), 0.001f, "Kinematic entity should receive no impulse");
            }
        }
    }
    ...
    void testMultiListenerPriority() {
        ...
        CollisionListener<LongEntityID, String> listener1 = (collision, response) -> {
            listener1Count.incrementAndGet();
            return true;
        };

        // Second listener prevents processing
        CollisionListener<LongEntityID, String> listener2 = (collision, response) -> {
        ...
        assertEquals(0, processed.size(), "Collision should not be processed due to listener veto");
    }
    ...
    void testStaticVsDynamicCollisions() {
        ...
        EntityBounds staticBounds = new EntityBounds(new Point3f(95, 95, 95), new Point3f(105, 105, 105));
        ...
        if (processed.get(0).collision().entityId1().equals(staticId)) {
            assertEquals(0, response.impulse1().length(), 0.001f, "Static entity should receive no impulse");
            assertTrue(response.impulse2().length() > 0, "Dynamic entity should receive impulse");
        } else {
            assertTrue(response.impulse1().length() > 0, "Dynamic entity should receive impulse");
            assertEquals(0, response.impulse2().length(), 0.001f, "Static entity should receive no impulse");
        }
    }
}
