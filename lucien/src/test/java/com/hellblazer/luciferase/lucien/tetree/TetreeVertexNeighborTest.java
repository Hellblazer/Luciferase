...
public class TetreeVertexNeighborTest {
    ...
    void testVertexNeighborComprehensive() {
        ...
        var center = new Point3f(400, 300, 400);
        ...
        var face1Neighbor = tetree.findFaceNeighbor(centerKey, 1);
        if (face1Neighbor != null) {
            face1.add(face1Neighbor);
        }

        var face2Neighbor = tetree.findFaceNeighbor(centerKey, 2);
        if (face2Neighbor != null) {
            face2.add(face2Neighbor);
        }

        var face3Neighbor = tetree.findFaceNeighbor(centerKey, 3);
        if (face3Neighbor != null) {
            face3.add(face3Neighbor);
        }

        // Get vertex neighbors
        var vertex0Neighbors = tetree.findVertexNeighbors(centerKey, 0); // faces 1,2,3
        var vertex1Neighbors = tetree.findVertexNeighbors(centerKey, 1); // faces 0,2,3
        var vertex2Neighbors = tetree.findVertexNeighbors(centerKey, 2); // faces 0,1,3
    ...
    void testVertexNeighborIndexValidation() {
        ...
        var p1 = new Point3f(512, 512, 512);
    ...
    void testVertexNeighborsBasic() {
        ...
        for (int vertex = 0; vertex < 4; vertex++) {
            var vertexNeighbors = tetree.findVertexNeighbors(tetKey, vertex);
            assertNotNull(vertexNeighbors, "Vertex neighbors should not be null for vertex " + vertex);

            // Should not include itself
            assertFalse(vertexNeighbors.contains(tetKey), "Vertex neighbors should not include the tetrahedron itself");
        }
    }
    ...
    void testVertexNeighborsBoundary() {
        ...
        var boundary1 = new Point3f(10, 10, 10);      // Near origin
        var boundary2 = new Point3f(1014, 1014, 1014); // Near max
        var center = new Point3f(512, 512, 512);       // Center
        ...
        var boundaryTet1 = tetree.locateTetrahedron(boundary1, (byte) 2);
        var boundaryTet2 = tetree.locateTetrahedron(boundary2, (byte) 2);

        // Boundary tetrahedra should have fewer vertex neighbors
        var boundaryKey1 = boundaryTet1.tmIndex();
        var boundaryKey2 = boundaryTet2.tmIndex();
        ...
        var centerTet = tetree.locateTetrahedron(center, (byte) 2);
        var centerKey = centerTet.tmIndex();
    ...
    void testVertexNeighborsDenseGrid() {
        ...
        // Pick a central tetrahedron
        var center = new Point3f(384, 384, 384); // Middle of the grid
        var centerTet = tetree.locateTetrahedron(center, (byte) 4);
        var centerKey = centerTet.tmIndex();

        // Count total vertex neighbors
        int totalVertexNeighbors = 0;
        for (int vertex = 0; vertex < 4; vertex++) {
            var neighbors = tetree.findVertexNeighbors(centerKey, vertex);
    ...
    void testVertexNeighborsEdgeConsistency() {
        ...
        var p1 = new Point3f(400, 400, 400);
        var p2 = new Point3f(600, 400, 400);
        var p3 = new Point3f(400, 600, 400);
        var p4 = new Point3f(400, 400, 600);
        ...
        var edge0Neighbors = tetree.findEdgeNeighbors(tetKey, 0);
        var edge1Neighbors = tetree.findEdgeNeighbors(tetKey, 1);
        var edge2Neighbors = tetree.findEdgeNeighbors(tetKey, 2);
        ...
        if (!vertex0Neighbors.isEmpty() && !expectedFromEdges.isEmpty()) {
            var vertexSet = new HashSet<>(vertex0Neighbors);
    ...
    void testVertexNeighborsSymmetry() {
        ...
        var tetKey1 = tet1.tmIndex();
        var tetKey2 = tet2.tmIndex();

        for (int v1 = 0; v1 < 4; v1++) {
            var neighbors1 = tetree.findVertexNeighbors(tetKey1, v1);
            ...
            if (neighbors1.contains(tetKey2)) {
                ...
                for (int v2 = 0; v2 < 4; v2++) {
                    var neighbors2 = tetree.findVertexNeighbors(tetKey2, v2);
