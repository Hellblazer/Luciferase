...
/**
 * Benchmarks for parallel processing optimizations including: - Parallel bulk insertion performance - Lock contention
 * analysis - Work-stealing vs fixed thread pool comparison - Scalability across different core counts - Spatial
 * partitioning effectiveness
 *
 * Run with: RUN_SPATIAL_INDEX_PERF_TESTS=true mvn test
 *
 * UPDATE: ParallelBulkOperations has been optimized to use true batch operations with coarse-grained locking. Test
 * sizes have been restored to their original values to demonstrate the improved performance and scalability.
 *
...
public class ParallelProcessingBenchmark {
    ...
    void benchmarkWorkStealingVsFixedPool() {
        ...
        for (int size : testSizes) {
            ...
            // Get baseline
            var baseline = baselineDurations.computeIfAbsent(size, k -> benchmarkSingleThreaded(positions, contents));
    ...
    private void benchmarkFineLocking(List<Point3f> positions, List<String> contents) {
        ...
        var config = new ParallelBulkOperations.ParallelConfig().withThreadCount(AVAILABLE_CORES).withBatchSize(100);

        var result = runParallelInsertion(positions, contents, config);
    ...
    private void benchmarkFixedThreadPool(List<Point3f> positions, List<String> contents, long baseline) {
        ...
        var config = ParallelBulkOperations.highPerformanceConfig().withThreadCount(AVAILABLE_CORES).withWorkStealing(
        false);
        ...
        var benchResult = new ParallelBenchmarkResult(testName, AVAILABLE_CORES, avgDuration, positions.size(),
                                                      baseline, 0);
    ...
    private void benchmarkParallelInsertion(int threadCount, List<Point3f> positions, List<String> contents,
                                            long baseline) {
        var testName = String.format("Parallel insertion", threadCount);

        // Configure parallel operations
        var config = new ParallelBulkOperations.ParallelConfig().withThreadCount(threadCount)
                                                                .withBatchSize(1000)
        ...
        var benchResult = new ParallelBenchmarkResult(testName, threadCount, avgDuration, positions.size(), baseline,
                                                      avgLockTime);
        results.add(benchResult);
        System.out.println(benchResult);
    }
    ...
    private void benchmarkRegionBasedLocking(List<Point3f> positions, List<String> contents) {
        ...
        var config = ParallelBulkOperations.largeDatasetConfig().withThreadCount(AVAILABLE_CORES);
    ...
    private long benchmarkSingleThreaded(List<Point3f> positions, List<String> contents) {
        ...
        var result = new ParallelBenchmarkResult("Single-threaded baseline", 1, avgDuration, positions.size(),
                                                 avgDuration, 0);
        results.add(result);
    ...
    private List<Point3f> generateClusteredTestData(int count) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            positions.add(
            new Point3f(baseX + rand.nextFloat() * 30, baseY + rand.nextFloat() * 30, baseZ + rand.nextFloat() * 30));
        }
        return positions;
    }
        ...
        for (int x = 0; x < gridSize && idx < count; x++) {
            for (int y = 0; y < gridSize && idx < count; y++) {
                for (int z = 0; z < gridSize && idx < count; z++) {
                    positions.add(new Point3f(x * 1000.0f / gridSize, y * 1000.0f / gridSize, z * 1000.0f / gridSize));
                    idx++;
                }
            }
        }
        ...
        for (var entry : bySize.entrySet()) {
            ...
            for (ParallelBenchmarkResult result : entry.getValue()) {
                if (!result.testName.contains("baseline")) {
                    System.out.printf("%7d  %7.2fx  %9.1f%%\n", result.threadCount, result.speedup,
    ...
    private ParallelBulkOperations.ParallelOperationResult<LongEntityID> runParallelInsertion(List<Point3f> positions,
    ...
    private void testSpatialPartitioning(String distribution, List<Point3f> positions) {
        ...
        var config = ParallelBulkOperations.highPerformanceConfig().withThreadCount(AVAILABLE_CORES);
    ...
    static class ParallelBenchmarkResult {
        ...
        ParallelBenchmarkResult(String testName, int threadCount, long duration, int entityCount, long baselineDuration,
                                long lockContentionTime) {
        ...
        public String toString() {
            return String.format("%-40s: %2d threads, %,10d entities in %,6d ms "
                                 + "(%.2fx speedup, %.1f%% efficiency, %,d ms lock contention)", testName, threadCount,
                                 entityCount, duration, speedup, efficiency * 100, lockContentionTime);
        }
    }
}
