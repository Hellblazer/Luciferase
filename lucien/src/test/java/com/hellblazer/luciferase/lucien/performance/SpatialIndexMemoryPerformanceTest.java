 ...
 */
public abstract class SpatialIndexMemoryPerformanceTest<Key extends com.hellblazer.luciferase.lucien.SpatialKey<Key>, ID extends com.hellblazer.luciferase.lucien.entity.EntityID, Content>
extends AbstractSpatialIndexPerformanceTest<Key, ID, Content> {
    ...
    void testDenseVsSparseMemoryEfficiency() {
        ...
        System.out.printf("Memory efficiency - Dense: %.2f MB, Sparse: %.2f MB, Ratio: %.2fx%n",
                          denseMemory / (1024.0 * 1024.0), sparseMemory / (1024.0 * 1024.0), efficiencyRatio);
    ...
    void testEmptyNodeOverhead() {
        ...
        for (int x = 0; x <= 1; x++) {
            for (int y = 0; y <= 1; y++) {
                for (int z = 0; z <= 1; z++) {
                    ...
                    index.insert(id, new javax.vecmath.Point3f(x == 0 ? DEFAULT_BOUNDS.minX() : DEFAULT_BOUNDS.maxX(),
                                                               y == 0 ? DEFAULT_BOUNDS.minY() : DEFAULT_BOUNDS.maxY(),
                                                               z == 0 ? DEFAULT_BOUNDS.minZ() : DEFAULT_BOUNDS.maxZ()),
                                 DEFAULT_LEVEL, createTestContent(entityCount));
                    entityCount++;
                }
            }
        }
        ...
        var metrics = new PerformanceMetrics("empty_node_overhead", entityCount, 0, usedMemory, null);
    ...
    void testMemoryFragmentation() {
        ...
        var metrics = new PerformanceMetrics("memory_fragmentation", operations, 0, finalMemory - initialMemory, null);
        performanceResults.add(metrics);

        System.out.printf("Memory fragmentation after %d operations: %.2fx increase (%.2f MB -> %.2f MB)%n", operations,
                          fragmentationRatio, initialMemory / (1024.0 * 1024.0), finalMemory / (1024.0 * 1024.0));

        // Memory shouldn't grow more than 2x due to fragmentation
        assertTrue(fragmentationRatio < 2.0, "Memory fragmentation should be limited (less than 2x growth)");
    }
    ...
    void testMemoryUsageScaling() {
        ...
        for (int size : testSizes) {
            ...
            var metrics = new PerformanceMetrics("memory_scaling", size, 0, // Not timing-based
                                                 usedMemory, null);
            performanceResults.add(metrics);

            System.out.printf("Memory usage for %d entities: %.2f MB (%.2f bytes/entity)%n", size,
                              usedMemory / (1024.0 * 1024.0), memoryPerEntity);
        }
    ...
    void testMemoryWithDifferentDistributions() {
        ...
        for (var distribution : SpatialDistribution.values()) {
            ...
            var metrics = new PerformanceMetrics("memory_distribution_" + distribution.name().toLowerCase(), size, 0,
                                                 usedMemory, null);
            performanceResults.add(metrics);

            System.out.printf("Memory usage with %s distribution: %.2f MB (%.2f bytes/entity)%n", distribution,
                              usedMemory / (1024.0 * 1024.0), (double) usedMemory / size);
        }
    }
