...
/**
 * Unit tests for Stream API integration in Tetree. Tests the functional programming patterns and stream operations.
 *
...
public class TetreeStreamAPITest {
    ...
    void testGetLeafNodes() {
        ...
        for (int i = 0; i < 8; i++) {
            float offset = 200;
            Point3f p = new Point3f(512 + (i % 2 - 0.5f) * offset, 512 + ((i / 2) % 2 - 0.5f) * offset,
        ...
        assertEquals(leafStreamCount, leafNodes.size(), "Leaf node list size should match leaf stream count");
    }
    ...
    void testGetNodeCountByLevel() {
        ...
        for (Map.Entry<Byte, Integer> entry : nodeCounts.entrySet()) {
            ...
            assertEquals(streamCount, count, "Node count for level " + level + " should match stream count");
        }
        ...
        assertEquals(totalFromStream, totalFromMap, "Total node count from map should match stream count");
    }
    ...
    void testLevelStream() {
        ...
        for (byte level = 0; level <= 5; level++) {
            ...
            assertTrue(allAtCorrectLevel, "All nodes in level stream should be at level " + testLevel);
        }
    ...
    void testVisitLevel() {
        ...
        assertEquals(levelStreamCount, visitCount[0], "Visit count should match level stream count");
