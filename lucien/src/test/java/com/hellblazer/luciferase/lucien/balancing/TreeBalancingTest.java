...
public class TreeBalancingTest {
    ...
    @BeforeEach
    public void setUp() {
        octree = new Octree<>(new SequentialLongIDGenerator(), 20,     // max entities per node
                              (byte) 10  // max depth
        );
    }
    ...
    public void testAutoBalancing() {
        ...
        TreeBalancingStrategy<LongEntityID> aggressiveStrategy = new DefaultBalancingStrategy<>(0.3,
                                                                                                // 30% merge factor
                                                                                                0.7,
                                                                                                // 70% split factor
                                                                                                0.2,
        ...
        for (int i = 0; i < 200; i++) {
            Point3f pos = new Point3f(rand.nextFloat() * 100, rand.nextFloat() * 100, rand.nextFloat() * 100);
    ...
    public void testBalancingStrategyConfiguration() {
        ...
        // Set custom strategy
        TreeBalancingStrategy<LongEntityID> customStrategy = new DefaultBalancingStrategy<>(0.2,    // 20% merge factor
                                                                                            0.8,    // 80% split factor
                                                                                            0.25,
    ...
    public void testDefaultBalancingStrategy() {
        ...
        assertTrue(splitThreshold > 0 && splitThreshold < 100, "Split threshold should be between 0 and max entities");
        ...
        // Test should split
        assertTrue(strategy.shouldSplit(95, (byte) 5, 100), "Should split when above threshold");
        assertFalse(strategy.shouldSplit(50, (byte) 5, 100), "Should not split when below threshold");

        // Test should merge
        assertTrue(strategy.shouldMerge(5, (byte) 5, new int[] { 3, 4, 2 }), "Should merge when entities are low");
    ...
    public void testEntityDistribution() {
        ...
        for (var childEntities : distribution) {
            assertTrue(childEntities.size() >= 1 && childEntities.size() <= 3, "Should have roughly even distribution");
        }
    }
    ...
    private void createImbalancedTree() {
        ...
        Random rand = new Random(42);
        for (int i = 0; i < 20; i++) {
            Point3f pos = new Point3f(500 + rand.nextFloat() * 500, 500 + rand.nextFloat() * 500,
    ...
    private void insertClusteredEntities(int totalEntities, int clusters) {
        ...
        for (int c = 0; c < clusters; c++) {
            ...
            for (int i = 0; i < entitiesPerCluster; i++) {
                Point3f pos = new Point3f(centerX + rand.nextFloat() * 50, centerY + rand.nextFloat() * 50,
