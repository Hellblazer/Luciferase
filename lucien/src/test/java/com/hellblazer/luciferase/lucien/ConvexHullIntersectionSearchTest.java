...
/**
 * Unit tests for Convex Hull intersection search functionality All test coordinates use positive values only
 *
...
public class ConvexHullIntersectionSearchTest {
    ...
    void testBatchConvexHullIntersections() {
        ...
        Vector3f[] axes = { new Vector3f(1.0f, 0.0f, 0.0f), new Vector3f(0.0f, 1.0f, 0.0f), new Vector3f(0.0f, 0.0f,
                                                                                                         1.0f) };

        ConvexHullIntersectionSearch.ConvexHull hull1 = ConvexHullIntersectionSearch.ConvexHull.createOrientedBoundingBox(
        new Point3f(150.0f, 150.0f, 150.0f), axes, new float[] { 100.0f, 100.0f, 100.0f });

        ConvexHullIntersectionSearch.ConvexHull hull2 = ConvexHullIntersectionSearch.ConvexHull.createOrientedBoundingBox(
        ...
        Map<ConvexHullIntersectionSearch.ConvexHull, List<ConvexHullIntersectionSearch.ConvexHullIntersection<String>>> batchResults = ConvexHullIntersectionSearch.batchConvexHullIntersections(
        hulls, octree, referencePoint);
        ...
        for (var entry : batchResults.entrySet()) {
            List<ConvexHullIntersectionSearch.ConvexHullIntersection<String>> individual = ConvexHullIntersectionSearch.convexHullIntersectedAll(
            entry.getKey(), octree, referencePoint);
    ...
    void testConvexHullDistanceCalculation() {
        ...
        ConvexHullIntersectionSearch.ConvexHull hull = ConvexHullIntersectionSearch.ConvexHull.createOrientedBoundingBox(
        center, axes, extents);
    ...
    void testConvexHullFromVertices() {
        ...
        List<Point3f> vertices = List.of(new Point3f(100.0f, 100.0f, 100.0f), new Point3f(300.0f, 100.0f, 100.0f),
                                         new Point3f(200.0f, 300.0f, 100.0f), new Point3f(200.0f, 200.0f, 300.0f));

        ConvexHullIntersectionSearch.ConvexHull hull = ConvexHullIntersectionSearch.ConvexHull.fromVertices(vertices);
    ...
    void testConvexHullIntersectedAll() {
        ...
        Vector3f[] axes = { new Vector3f(1.0f, 0.0f, 0.0f), new Vector3f(0.0f, 1.0f, 0.0f), new Vector3f(0.0f, 0.0f,
                                                                                                         1.0f) };
        float[] extents = { 150.0f, 150.0f,
                            150.0f }; // Large enough to encompass cubes but not violate positive coordinates
        ...
        List<ConvexHullIntersectionSearch.ConvexHullIntersection<String>> intersections = ConvexHullIntersectionSearch.convexHullIntersectedAll(
        hull, octree, referencePoint);
    ...
    void testConvexHullIntersectedFirst() {
        ...
        ConvexHullIntersectionSearch.ConvexHullIntersection<String> first = ConvexHullIntersectionSearch.convexHullIntersectedFirst(
        hull, octree, referencePoint);

        if (first != null) {
            // Should be the closest intersection
            List<ConvexHullIntersectionSearch.ConvexHullIntersection<String>> all = ConvexHullIntersectionSearch.convexHullIntersectedAll(
            hull, octree, referencePoint);
    ...
    void testCubesCompletelyInside() {
        ...
        List<ConvexHullIntersectionSearch.ConvexHullIntersection<String>> inside = ConvexHullIntersectionSearch.cubesCompletelyInside(
        hull, octree, referencePoint);
        ...
        for (int i = 0; i < inside.size() - 1; i++) {
            assertTrue(inside.get(i).distanceToReferencePoint <= inside.get(i + 1).distanceToReferencePoint);
        }
    }
    ...
    void testCubesPartiallyIntersecting() {
        ...
        List<ConvexHullIntersectionSearch.ConvexHullIntersection<String>> intersecting = ConvexHullIntersectionSearch.cubesPartiallyIntersecting(
        hull, octree, referencePoint);

        // All results should be INTERSECTING
        for (var intersection : intersecting) {
            assertEquals(ConvexHullIntersectionSearch.IntersectionType.INTERSECTING, intersection.intersectionType);
        }

        // Results should be sorted by distance
        for (int i = 0; i < intersecting.size() - 1; i++) {
            assertTrue(
            intersecting.get(i).distanceToReferencePoint <= intersecting.get(i + 1).distanceToReferencePoint);
        }
    }
    ...
    void testDistanceOrdering() {
        ...
        List<ConvexHullIntersectionSearch.ConvexHullIntersection<String>> nearResults = ConvexHullIntersectionSearch.convexHullIntersectedAll(
        hull, octree, nearReference);

        List<ConvexHullIntersectionSearch.ConvexHullIntersection<String>> farResults = ConvexHullIntersectionSearch.convexHullIntersectedAll(
        hull, octree, farReference);
    ...
    void testEmptyOctree() {
        ...
        ConvexHullIntersectionSearch.IntersectionStatistics stats = ConvexHullIntersectionSearch.getConvexHullIntersectionStatistics(
        hull, emptyOctree);
    ...
    void testHasAnyIntersection() {
        ...
        ConvexHullIntersectionSearch.ConvexHull largeHull = ConvexHullIntersectionSearch.ConvexHull.createOrientedBoundingBox(
        center, axes, extents);
        ...
        Point3f referencePoint = new Point3f(100.0f, 100.0f, 100.0f);
        List<ConvexHullIntersectionSearch.ConvexHullIntersection<String>> intersections = ConvexHullIntersectionSearch.convexHullIntersectedAll(
        largeHull, octree, referencePoint);
        ...
        ConvexHullIntersectionSearch.ConvexHull smallHull = ConvexHullIntersectionSearch.ConvexHull.createOrientedBoundingBox(
        farCenter, axes, smallExtents);
    ...
    void testInvalidConvexHullCreation() {
        ...
        assertThrows(IllegalArgumentException.class, () -> {
            ConvexHullIntersectionSearch.ConvexHull.fromVertices(
            List.of(new Point3f(100.0f, 100.0f, 100.0f), new Point3f(200.0f, 100.0f, 100.0f)));
        });

        // Test wrong number of axes
        assertThrows(IllegalArgumentException.class, () -> {
            ConvexHullIntersectionSearch.ConvexHull.createOrientedBoundingBox(new Point3f(200.0f, 200.0f, 200.0f),
                                                                              new Vector3f[] {
        ...
        assertThrows(IllegalArgumentException.class, () -> {
            ConvexHullIntersectionSearch.ConvexHull.createOrientedBoundingBox(new Point3f(200.0f, 200.0f, 200.0f), axes,
                                                                              new float[] { -100.0f, 100.0f, 100.0f });
        });
    }
    ...
    void testLargeAndSmallConvexHulls() {
        ...
        List<ConvexHullIntersectionSearch.ConvexHullIntersection<String>> smallResults = ConvexHullIntersectionSearch.convexHullIntersectedAll(
        smallHull, octree, referencePoint);

        // Very large hull - should intersect most or all cubes
        ConvexHullIntersectionSearch.ConvexHull largeHull = ConvexHullIntersectionSearch.ConvexHull.createOrientedBoundingBox(
        new Point3f(300.0f, 300.0f, 300.0f), axes, new float[] { 200.0f, 200.0f, 200.0f });

        List<ConvexHullIntersectionSearch.ConvexHullIntersection<String>> largeResults = ConvexHullIntersectionSearch.convexHullIntersectedAll(
        largeHull, octree, referencePoint);
    ...
    void testNegativeCoordinatesThrowException() {
        ...
        List<Point3f> verticesWithNegative = List.of(new Point3f(100.0f, 100.0f, 100.0f),
        ...
        ConvexHullIntersectionSearch.ConvexHull validHull = ConvexHullIntersectionSearch.ConvexHull.createOrientedBoundingBox(
        positiveCenter, axes, extents);
