...
/**
 * Accuracy tests for Octree collision detection.
...
public class OctreeCollisionAccuracyTest {
    ...
    void testContactPointAccuracyForAABBCollision() {
        ...
        EntityBounds bounds1 = new EntityBounds(
            ...
            new Point3f(105, 105, 105)  // 10x10x10 box
        );
        EntityBounds bounds2 = new EntityBounds(
            new Point3f(103, 95, 95),   // Overlaps by 2 units on X axis
    ...
    void testContactNormalAccuracy() {
        ...
        EntityBounds bounds2 = new EntityBounds(
            new Point3f(104, 95, 95),   // 1 unit overlap on X
    ...
    void testPenetrationDepthAccuracy() {
        ...
        EntityBounds bounds1 = new EntityBounds(
            new Point3f(95, 95, 95),
            new Point3f(105, 105, 105)  // Right edge at x=105
        );
        EntityBounds bounds2 = new EntityBounds(
            new Point3f(102, 95, 95),   // Left edge at x=102, so 3 units overlap
    ...
    void testSphereCollisionAccuracy() {
        ...
        assertTrue(projection > 0 && projection < 3.0f, 
        ...
        assertEquals(expectedPenetration, pair.penetrationDepth(), EPSILON,
                    "Penetration depth should be 1 unit");
    }
    ...
    void testMixedShapeCollisionAccuracy() {
        ...
        EntityBounds boxBounds = new EntityBounds(
            new Point3f(95, 95, 95),
            new Point3f(105, 105, 105)
        );
    ...
    void testMultipleCollisionAccuracy() {
        ...
        Point3f[] surroundingCenters = {
            new Point3f(108, 100, 100), // +X
        ...
        for (int i = 0; i < 6; i++) {
            EntityBounds bounds = new EntityBounds(
                new Point3f(surroundingCenters[i].x - 5, 
                           surroundingCenters[i].y - 5,
                           surroundingCenters[i].z - 5),
                new Point3f(surroundingCenters[i].x + 5,
                           surroundingCenters[i].y + 5,
                           surroundingCenters[i].z + 5)
            );
        ...
        for (var collision : allCollisions) {
            ...
            int alignedAxis = 0;
            if (Math.abs(normal.x) > 0.9f) alignedAxis++;
            if (Math.abs(normal.y) > 0.9f) alignedAxis++;
            if (Math.abs(normal.z) > 0.9f) alignedAxis++;
            ...
            assertEquals(2.0f, collision.penetrationDepth(), EPSILON,
                        "All penetration depths should be 2 units");
        }
    }
    ...
    void testNumericalScaleInvariance() {
        ...
        for (float scale : scales) {
            ...
            EntityBounds bounds2 = new EntityBounds(
                new Point3f(103 * scale, 95 * scale, 95 * scale),
            ...
            Optional<SpatialIndex.CollisionPair<LongEntityID, String>> collision = scaledOctree.checkCollision(id1, id2);
