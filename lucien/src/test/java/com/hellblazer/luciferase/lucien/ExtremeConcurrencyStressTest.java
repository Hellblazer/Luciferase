...
/**
 * Extreme concurrency stress test for Luciferase spatial index implementations.
 * Tests with 50+ threads performing mixed operations to expose any remaining
 * concurrency issues after the ConcurrentSkipListMap implementation.
 *
...
public class ExtremeConcurrencyStressTest {
    ...
    private final Random masterRandom = new Random(42); // Deterministic seed for reproducibility
    ...
    void tearDown() {
        ...
        System.out.println("Operations/second: " + 
            (totalOperations.get() * 1000.0 / Math.max(1, getTestDuration())));
    ...
    private <Key extends SpatialKey<Key>> void runExtremeStressTest(
        ...
        System.out.println("\nStarting extreme stress test with " + threadCount + " threads for " + 
            durationMs + "ms on " + spatialIndex.getClass().getSimpleName());
                    ...
                    while (running.get()) {
                        ...
                        if (currentState != null && !currentState.deleted) {
                            ...
                            var newPos = new Point3f(
                                Math.max(0.1f, oldPos.x + (random.nextFloat() - 0.5f) * delta),
            ...
            executor.submit(() -> {
                try {
                    ...
                    while (running.get()) {
                        ...
                        var pos1 = new Point3f(
                            boundary - 1f + random.nextFloat() * 2f,
    ...
    private <Key extends SpatialKey<Key>> void runMixedOperations(
            SpatialIndex<Key, LongEntityID, String> spatialIndex,
            int threadId,
            AtomicBoolean running) {
        
        Random random = new Random(threadId * 1000 + masterRandom.nextInt());
    ...
    private <Key extends SpatialKey<Key>> void performInsert(
            SpatialIndex<Key, LongEntityID, String> spatialIndex,
            Random random,
            List<LongEntityID> threadEntityIds) {
    ...
    private <Key extends SpatialKey<Key>> void performQuery(
            SpatialIndex<Key, LongEntityID, String> spatialIndex,
            Random random) {
    ...
    private <Key extends SpatialKey<Key>> void performUpdate(
            SpatialIndex<Key, LongEntityID, String> spatialIndex,
            Random random,
            List<LongEntityID> threadEntityIds) {
    ...
    private <Key extends SpatialKey<Key>> void performDelete(
            SpatialIndex<Key, LongEntityID, String> spatialIndex,
            Random random,
            List<LongEntityID> threadEntityIds) {
    ...
    private <Key extends SpatialKey<Key>> void verifyIndexIntegrity(
            ...
            if (!state.deleted) {
                ...
                if (indexData == null) {
                    ...
                    if (indexPos == null || !positionsEqual(state.position, indexPos)) {
                        incorrectPositions++;
                        System.err.println("Position mismatch for " + entry.getKey() + 
                            ": expected " + state.position + ", got " + indexPos);
                    }
                }
            }
        }
        ...
        System.out.println("Integrity check passed. Expected entities: " + expectedCount + 
            ", Index reports: " + stats.entityCount());
    }
