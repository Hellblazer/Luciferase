...
public class LazyEvaluationProfilingTest {
    ...
    void measureOverhead() {
        ...
        System.out.printf("Direct TetreeKey creation: %.2f ms (%.2f ns/key)%n", 
            directTime / 1_000_000.0, directTime / (double) positions.size());
        System.out.printf("Lazy key creation (unresolved): %.2f ms (%.2f ns/key)%n", 
            lazyTime / 1_000_000.0, lazyTime / (double) positions.size());
        System.out.printf("Lazy key + resolution: %.2f ms (%.2f ns/key)%n", 
            resolvedTime / 1_000_000.0, resolvedTime / (double) positions.size());
    ...
    void testSelectiveLazyEvaluation() {
        ...
        System.out.printf("\nSingle insert speedup (never/always): %.2fx%n", 
            (double) alwaysLazyTime / neverLazyTime);
        System.out.printf("Bulk speedup (never single/lazy bulk): %.2fx%n", 
            (double) neverLazyTime / bulkLazyTime);
    }
}