...
/**
 * Utility methods for generating valid test data for Tetree tests. Ensures all generated Tet instances have properly
 * quantized coordinates and correct types based on standard refinement.
 */
public class TetTestUtils {

    /**
     * Generate all 8 corner Tets at a given level. These are the Tets at the corners of the unit cube scaled to the
     * level, with types computed from standard refinement.
     *
    ...
    /**
     * Generate a Tet that would be produced by refining from root. This traces the refinement path from root to ensure
     * valid type.
     *
    ...
    /**
     * Generate a random valid Tet at the specified level. This creates a Tet that follows standard refinement from type
     * 0 root.
     *
     * @param level  the target level
    ...
    /**
     * Generate a valid Tet at a specific grid position. This creates a Tet with the correct type based on standard
     * refinement.
     *
    ...
    /**
     * Generate valid test data for TM-index tests. Returns an array of Object[] suitable for parameterized tests. Each
     * entry contains: x, y, z, level, type (all guaranteed to be valid).
     *
    ...
    public static Object[][] validTmIndexTestData() {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            testData.add(new Object[] { child.x(), child.y(), child.z(), child.l(), child.type() });
        }
        ...
        for (byte level = 3; level <= 10; level++) {
            for (int i = 0; i < 3; i++) {
                ...
                testData.add(new Object[] { tet.x(), tet.y(), tet.z(), tet.l(), tet.type() });
            }
        }
