...
/**
 * Unit tests for AABB intersection search functionality All test coordinates use positive values only
 *
...
public class AABBIntersectionSearchTest {
    ...
    void testAABBCreation() {
        ...
        AABBIntersectionSearch.AABB aabb = new AABBIntersectionSearch.AABB(100.0f, 100.0f, 100.0f, 300.0f, 300.0f,
                                                                           300.0f);
    ...
    void testAABBEqualsAndHashCode() {
        AABBIntersectionSearch.AABB aabb1 = new AABBIntersectionSearch.AABB(100.0f, 100.0f, 100.0f, 200.0f, 200.0f,
                                                                            200.0f);
        AABBIntersectionSearch.AABB aabb2 = new AABBIntersectionSearch.AABB(100.0f, 100.0f, 100.0f, 200.0f, 200.0f,
                                                                            200.0f);
        AABBIntersectionSearch.AABB aabb3 = new AABBIntersectionSearch.AABB(100.0f, 100.0f, 100.0f, 300.0f, 200.0f,
                                                                            200.0f);
    ...
    void testAABBFromCenterAndHalfExtents() {
        ...
        AABBIntersectionSearch.AABB aabb = AABBIntersectionSearch.AABB.fromCenterAndHalfExtents(center, 50.0f, 75.0f,
                                                                                                100.0f);
    ...
    void testAABBIntersectedAll() {
        ...
        AABBIntersectionSearch.AABB aabb = new AABBIntersectionSearch.AABB(150.0f, 150.0f, 150.0f, 350.0f, 350.0f,
                                                                           350.0f);
        Point3f referencePoint = new Point3f(100.0f, 100.0f, 100.0f);

        List<AABBIntersectionSearch.AABBIntersection<String>> intersections = AABBIntersectionSearch.aabbIntersectedAll(
        aabb, octree, referencePoint);
        ...
        for (var intersection : intersections) {
            ...
            assertNotEquals(AABBIntersectionSearch.IntersectionType.COMPLETELY_OUTSIDE, intersection.intersectionType);
        }
    }

    @Test
    void testAABBIntersectedFirst() {
        AABBIntersectionSearch.AABB aabb = new AABBIntersectionSearch.AABB(120.0f, 120.0f, 120.0f, 280.0f, 280.0f,
                                                                           280.0f);
        Point3f referencePoint = new Point3f(75.0f, 75.0f, 75.0f);

        AABBIntersectionSearch.AABBIntersection<String> firstIntersection = AABBIntersectionSearch.aabbIntersectedFirst(
        aabb, octree, referencePoint);

        if (firstIntersection != null) {
            // Should be the closest intersection
            List<AABBIntersectionSearch.AABBIntersection<String>> allIntersections = AABBIntersectionSearch.aabbIntersectedAll(
            aabb, octree, referencePoint);
    ...
    void testBatchAABBIntersections() {
        ...
        AABBIntersectionSearch.AABB aabb1 = new AABBIntersectionSearch.AABB(150.0f, 150.0f, 150.0f, 250.0f, 250.0f,
                                                                            250.0f);
        AABBIntersectionSearch.AABB aabb2 = new AABBIntersectionSearch.AABB(250.0f, 250.0f, 250.0f, 350.0f, 350.0f,
                                                                            350.0f);

        List<AABBIntersectionSearch.AABB> aabbs = List.of(aabb1, aabb2);

        Map<AABBIntersectionSearch.AABB, List<AABBIntersectionSearch.AABBIntersection<String>>> results = AABBIntersectionSearch.batchAABBIntersections(
        aabbs, octree, referencePoint);
    ...
    void testCountAABBIntersections() {
        AABBIntersectionSearch.AABB aabb = new AABBIntersectionSearch.AABB(200.0f, 200.0f, 200.0f, 400.0f, 400.0f,
                                                                           400.0f);
        Point3f referencePoint = new Point3f(100.0f, 100.0f, 100.0f);
    ...
    void testCubesCompletelyInside() {
        ...
        List<AABBIntersectionSearch.AABBIntersection<String>> insideCubes = AABBIntersectionSearch.cubesCompletelyInside(
        aabb, octree, referencePoint);

        // All results should be COMPLETELY_INSIDE
        for (var intersection : insideCubes) {
            assertEquals(AABBIntersectionSearch.IntersectionType.COMPLETELY_INSIDE, intersection.intersectionType);
        }
    ...
    void testCubesContainingAABB() {
        ...
        AABBIntersectionSearch.AABB smallAABB = new AABBIntersectionSearch.AABB(200.0f, 200.0f, 200.0f, 210.0f, 210.0f,
                                                                                210.0f);
        Point3f referencePoint = new Point3f(100.0f, 100.0f, 100.0f);

        List<AABBIntersectionSearch.AABBIntersection<String>> containingCubes = AABBIntersectionSearch.cubesContainingAABB(
        smallAABB, octree, referencePoint);

        // All results should be CONTAINS_AABB
        for (var intersection : containingCubes) {
            assertEquals(AABBIntersectionSearch.IntersectionType.CONTAINS_AABB, intersection.intersectionType);
        }

        // Results should be sorted by distance from reference point
        for (int i = 0; i < containingCubes.size() - 1; i++) {
            assertTrue(
            containingCubes.get(i).distanceToReferencePoint <= containingCubes.get(i + 1).distanceToReferencePoint);
        }
    }
    ...
    void testCubesPartiallyIntersecting() {
        ...
        AABBIntersectionSearch.AABB aabb = new AABBIntersectionSearch.AABB(180.0f, 180.0f, 180.0f, 320.0f, 320.0f,
                                                                           320.0f);
        Point3f referencePoint = new Point3f(100.0f, 100.0f, 100.0f);

        List<AABBIntersectionSearch.AABBIntersection<String>> intersectingCubes = AABBIntersectionSearch.cubesPartiallyIntersecting(
        aabb, octree, referencePoint);

        // All results should be INTERSECTING
        for (var intersection : intersectingCubes) {
            assertEquals(AABBIntersectionSearch.IntersectionType.INTERSECTING, intersection.intersectionType);
        }
    ...
    void testDistanceOrdering() {
        AABBIntersectionSearch.AABB aabb = new AABBIntersectionSearch.AABB(150.0f, 150.0f, 150.0f, 450.0f, 450.0f,
                                                                           450.0f);
        Point3f nearReference = new Point3f(100.0f, 100.0f, 100.0f);
        Point3f farReference = new Point3f(600.0f, 600.0f, 600.0f);

        List<AABBIntersectionSearch.AABBIntersection<String>> nearIntersections = AABBIntersectionSearch.aabbIntersectedAll(
        aabb, octree, nearReference);

        List<AABBIntersectionSearch.AABBIntersection<String>> farIntersections = AABBIntersectionSearch.aabbIntersectedAll(
        aabb, octree, farReference);
    ...
    void testEmptyOctree() {
        ...
        AABBIntersectionSearch.AABB aabb = new AABBIntersectionSearch.AABB(150.0f, 150.0f, 150.0f, 250.0f, 250.0f,
                                                                           250.0f);
        Point3f referencePoint = new Point3f(100.0f, 100.0f, 100.0f);

        List<AABBIntersectionSearch.AABBIntersection<String>> intersections = AABBIntersectionSearch.aabbIntersectedAll(
        aabb, emptyOctree, referencePoint);

        assertTrue(intersections.isEmpty());

        AABBIntersectionSearch.AABBIntersection<String> first = AABBIntersectionSearch.aabbIntersectedFirst(aabb,
                                                                                                            emptyOctree,
                                                                                                            referencePoint);
        ...
        AABBIntersectionSearch.IntersectionStatistics stats = AABBIntersectionSearch.getAABBIntersectionStatistics(aabb,
                                                                                                                   emptyOctree);
    ...
    void testGetAABBIntersectionStatistics() {
        ...
        assertEquals(stats.getIntersectedPercentage(),
                     stats.getInsidePercentage() + stats.getIntersectingPercentage() + stats.getContainingPercentage(),
                     0.001);
    ...
    void testHasAnyIntersection() {
        ...
        AABBIntersectionSearch.AABB intersectingAABB = new AABBIntersectionSearch.AABB(150.0f, 150.0f, 150.0f, 350.0f,
                                                                                       350.0f, 350.0f);

        // Small AABB in an area with no data
        AABBIntersectionSearch.AABB nonIntersectingAABB = new AABBIntersectionSearch.AABB(1000.0f, 1000.0f, 1000.0f,
    ...
    void testNegativeCoordinatesThrowException() {
        ...
        AABBIntersectionSearch.AABB validAABB = new AABBIntersectionSearch.AABB(150.0f, 150.0f, 150.0f, 250.0f, 250.0f,
                                                                                250.0f);
    ...
    void testSmallAndLargeAABBs() {
        ...
        AABBIntersectionSearch.AABB smallAABB = new AABBIntersectionSearch.AABB(200.0f, 200.0f, 200.0f, 201.0f, 201.0f,
                                                                                201.0f);

        List<AABBIntersectionSearch.AABBIntersection<String>> smallIntersections = AABBIntersectionSearch.aabbIntersectedAll(
        smallAABB, octree, referencePoint);

        // Very large AABB - should intersect most or all cubes
        AABBIntersectionSearch.AABB largeAABB = new AABBIntersectionSearch.AABB(50.0f, 50.0f, 50.0f, 1000000.0f,
                                                                                1000000.0f, 1000000.0f);

        List<AABBIntersectionSearch.AABBIntersection<String>> largeIntersections = AABBIntersectionSearch.aabbIntersectedAll(
        largeAABB, octree, referencePoint);
