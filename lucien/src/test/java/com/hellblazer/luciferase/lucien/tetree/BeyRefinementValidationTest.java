...
public class BeyRefinementValidationTest {
    ...
    void testParentChildRoundTrip() {
        ...
        for (byte parentType = 0; parentType < 6; parentType++) {
            ...
            for (int i = 0; i < 8; i++) {
                ...
                assertEquals(parent.x(), computedParent.x(), 
                    "Parent X should match");
                assertEquals(parent.y(), computedParent.y(), 
                    "Parent Y should match");
                assertEquals(parent.z(), computedParent.z(), 
                    "Parent Z should match");
                assertEquals(parent.l(), computedParent.l(), 
                    "Parent level should match");
                assertEquals(parent.type(), computedParent.type(), 
    ...
    void testChildTypesFollowT8Code() {
        ...
        for (var testCase : testCases) {
            ...
            System.out.printf("Parent type %d, child %d -> type %d (expected %d)\n",
                parentType, childIndex, child.type(), expectedType);
            
            assertEquals(expectedType, child.type(),
                String.format("Child %d of parent type %d should have type %d",
                    childIndex, parentType, expectedType));
        }
    }
    ...
    void testBeyRefinementProperties() {
        ...
        for (int i = 0; i < 8; i++) {
            assertEquals(parent.l() + 1, children[i].l(),
                "Child should be at parent level + 1");
        }
        
        // Children should have distinct tm-indices
        var indices = new java.util.HashSet<BaseTetreeKey<?>>();
        for (int i = 0; i < 8; i++) {
            var tmIndex = children[i].tmIndex();
            assertTrue(indices.add(tmIndex),
                "Each child should have a unique tm-index");
        }
