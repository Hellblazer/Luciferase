 ...
 *
 * This demonstrates how pure tetrahedral coordinate operations could enhance our current Bey red refinement tetrahedral
...
public class TetrahedralCoordinateTest {
    ...
    void compareTetrahedralVsCartesianSpatialRelationships() {
        ...
        Point3f[] tetrahedronVertices = { new Point3f(0.0f, 0.0f, 0.0f), new Point3f(100.0f, 0.0f, 0.0f), new Point3f(
        50.0f, 86.6f, 0.0f),  // sqrt(3)/2 * 100 ≈ 86.6
        ...
        for (int i = 0; i < tetrahedronVertices.length; i++) {
            ...
            assertTrue(tet.type() >= 0 && tet.type() <= 5, "Tetrahedron type should be valid for vertex " + i);
        }
    ...
    /**
     * Convert Cartesian coordinates to tetrahedral coordinates Inspired by the coordinate transformation in the gist
     */
    ...
    /**
     * Calculate distance in tetrahedral space Demonstrates tetrahedral-native distance calculation
     */
    ...
    /**
     * Calculate taxicab distance in tetrahedral space Alternative distance metric from the gist
     */
