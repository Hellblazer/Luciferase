...
public class LevelSelectorTest {
    ...
    public void testShouldUseMortonSortClusteredData() {
        ...
        for (int cluster = 0; cluster < 10; cluster++) {
            ...
            for (int i = 0; i < 200; i++) {
                positions.add(new Point3f(
                    centerX + rand.nextFloat() * 10,
                    centerY + rand.nextFloat() * 10,
    ...
    public void testGetAdaptiveSubdivisionThreshold() {
        ...
        for (byte level = 0; level <= 10; level++) {
            assertEquals(baseThreshold, 
                        LevelSelector.getAdaptiveSubdivisionThreshold(level, baseThreshold),
                        "Levels 0-10 should return base threshold");
        }
    ...
    public void testOptimalLevelConsistency() {
        ...
        for (int i = 0; i < 5000; i++) {
            positions1.add(new Point3f(
                rand1.nextFloat() * 500,
                rand1.nextFloat() * 500,
                rand1.nextFloat() * 500
            ));
            positions2.add(new Point3f(
                rand2.nextFloat() * 500,
                rand2.nextFloat() * 500,
                rand2.nextFloat() * 500
            ));
