...
/**
 * Comprehensive performance benchmark for Tetree vs Octree operations
...
public class TetreePerformanceBenchmark {
    ...
    private void populateWithTestData(int count) {
        ...
        System.out.printf("Contents: Tetree %d, Octree %d%n", 
            getTetreeContentCount(), getOctreeContentCount());
    }
    ...
    private void benchmarkBoundedByQuery(String queryType, Spatial.Cube query) {
        ...
        System.out.printf("BoundedBy %s: Tetree %d results in %.2f μs, Octree %d results in %.2f μs (%.2fx)%n",
            queryType, tetreeResults.size(), tetreeTime / 1000.0, 
            octreeResults.size(), octreeTime / 1000.0, 
            (double) tetreeTime / octreeTime);
    }
    ...
    private void benchmarkEnclosingQuery(String queryType) {
        ThreadLocalRandom random = ThreadLocalRandom.current();
        Point3f queryPoint = new Point3f(random.nextFloat() * 5000, 
        ...
        System.out.printf("Enclosing %s: Tetree %.2f μs, Octree %.2f μs (%.2fx)%n",
            queryType, tetreeTime / 1000.0, octreeTime / 1000.0,
            (double) tetreeTime / octreeTime);
    }
    ...
    void analyzeMemoryUsage() {
        ...
        System.out.printf("Memory used for %d entries: %.2f MB%n", 
            LARGE_DATASET, memoryUsed / (1024.0 * 1024.0));
        System.out.printf("Memory per entry: %.2f KB%n", 
            memoryUsed / (LARGE_DATASET * 1024.0));
    ...
    private void analyzeQueryMemoryUsage() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            System.out.printf("Query %d: Allocated %.2f KB (Tetree: %d, Octree: %d results)%n",
                i + 1, (after - before) / 1024.0, 
                tetreeResults.size(), octreeResults.size());
        }
    }
