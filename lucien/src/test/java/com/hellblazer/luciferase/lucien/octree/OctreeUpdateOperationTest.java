...
public class OctreeUpdateOperationTest {
    ...
    void testBulkUpdates() {
        ...
        for (int i = 0; i < entityCount; i++) {
            Point3f newPos = new Point3f(positions[i].x + 1000, positions[i].y + 1000, positions[i].z + 1000);
        ...
        // Verify all updates
        for (int i = 0; i < entityCount; i++) {
            Point3f expectedPos = new Point3f(positions[i].x + 1000, positions[i].y + 1000, positions[i].z + 1000);
            assertEquals(expectedPos, octree.getEntityPosition(ids[i]));
        }

        System.out.printf("Updated %d entities in %.2f ms (%.2f μs/op)%n", entityCount, updateTime / 1_000_000.0,
                          updateTime / 1000.0 / entityCount);
    ...
    void testUpdateNonExistentEntity() {
        ...
        assertThrows(IllegalArgumentException.class, () -> octree.updateEntity(fakeId, newPos, (byte) 10),
