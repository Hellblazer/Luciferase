...
/**
 * Unit tests for Frustum culling search functionality All test coordinates use positive values only
 *
...
public class FrustumCullingSearchTest {
    ...
    void testBatchFrustumCulling() {
        ...
        Frustum3D frustum1 = Frustum3D.createPerspective(cameraPos, lookAt1, up, (float) Math.PI / 4.0f, 1.0f, 50.0f,
                                                         400.0f);
        Frustum3D frustum2 = Frustum3D.createPerspective(cameraPos, lookAt2, up, (float) Math.PI / 4.0f, 1.0f, 50.0f,
                                                         400.0f);

        List<Frustum3D> frustums = List.of(frustum1, frustum2);

        Map<Frustum3D, List<FrustumCullingSearch.FrustumIntersection<String>>> results = FrustumCullingSearch.batchFrustumCulling(
        frustums, octree, cameraPos);
        ...
        for (var entry : results.entrySet()) {
            ...
            for (int i = 0; i < intersections.size() - 1; i++) {
                assertTrue(intersections.get(i).distanceToCamera <= intersections.get(i + 1).distanceToCamera);
            }
        }
    }
    ...
    void testCountFrustumIntersections() {
        ...
        Frustum3D frustum = Frustum3D.createPerspective(cameraPos, lookAt, up, (float) Math.PI / 4.0f, 1.0f, 75.0f,
                                                        500.0f);
        ...
        List<FrustumCullingSearch.FrustumIntersection<String>> intersections = FrustumCullingSearch.frustumCulledAll(
        frustum, octree, cameraPos);
    ...
    void testCubesCompletelyInside() {
        ...
        Frustum3D frustum = Frustum3D.createPerspective(cameraPos, lookAt, up, (float) Math.PI / 2.0f, 1.0f, 25.0f,
                                                        600.0f);

        List<FrustumCullingSearch.FrustumIntersection<String>> insideCubes = FrustumCullingSearch.cubesCompletelyInside(
        frustum, octree, cameraPos);
        ...
        for (int i = 0; i < insideCubes.size() - 1; i++) {
            assertTrue(insideCubes.get(i).distanceToCamera <= insideCubes.get(i + 1).distanceToCamera);
        }
    }
    ...
    void testCubesPartiallyIntersecting() {
        ...
        Frustum3D frustum = Frustum3D.createPerspective(cameraPos, lookAt, up, (float) Math.PI / 6.0f, 1.0f, 50.0f,
                                                        400.0f);

        List<FrustumCullingSearch.FrustumIntersection<String>> intersectingCubes = FrustumCullingSearch.cubesPartiallyIntersecting(
        frustum, octree, cameraPos);
        ...
        for (int i = 0; i < intersectingCubes.size() - 1; i++) {
            assertTrue(intersectingCubes.get(i).distanceToCamera <= intersectingCubes.get(i + 1).distanceToCamera);
        }
    }
    ...
    void testDistanceOrdering() {
        ...
        Frustum3D frustum = Frustum3D.createPerspective(nearCamera, lookAt, up, (float) Math.PI / 2.0f, 1.0f, 25.0f,
                                                        800.0f);

        List<FrustumCullingSearch.FrustumIntersection<String>> nearIntersections = FrustumCullingSearch.frustumCulledAll(
        frustum, octree, nearCamera);

        List<FrustumCullingSearch.FrustumIntersection<String>> farIntersections = FrustumCullingSearch.frustumCulledAll(
        frustum, octree, farCamera);
        ...
        if (!nearIntersections.isEmpty()) {
            ...
            for (int i = 0; i < nearIntersections.size() - 1; i++) {
                assertTrue(nearIntersections.get(i).distanceToCamera <= nearIntersections.get(i + 1).distanceToCamera);
            }
        }

        if (!farIntersections.isEmpty()) {
            for (int i = 0; i < farIntersections.size() - 1; i++) {
                assertTrue(farIntersections.get(i).distanceToCamera <= farIntersections.get(i + 1).distanceToCamera);
            }
        }
    }
    ...
    void testEmptyOctree() {
        ...
        Frustum3D frustum = Frustum3D.createPerspective(cameraPos, lookAt, up, (float) Math.PI / 4.0f, 1.0f, 50.0f,
                                                        400.0f);

        List<FrustumCullingSearch.FrustumIntersection<String>> intersections = FrustumCullingSearch.frustumCulledAll(
        frustum, emptyOctree, cameraPos);

        assertTrue(intersections.isEmpty());

        FrustumCullingSearch.FrustumIntersection<String> first = FrustumCullingSearch.frustumCulledFirst(frustum,
                                                                                                         emptyOctree,
                                                                                                         cameraPos);
        ...
        FrustumCullingSearch.CullingStatistics stats = FrustumCullingSearch.getFrustumCullingStatistics(frustum,
                                                                                                        emptyOctree);
    ...
    void testFrustumCulledAll() {
        ...
        Frustum3D frustum = Frustum3D.createPerspective(cameraPos, lookAt, up, (float) Math.PI / 3.0f, 1.0f, 25.0f,
                                                        400.0f);
    ...
    void testFrustumCulledFirst() {
        ...
        FrustumCullingSearch.FrustumIntersection<String> firstIntersection = FrustumCullingSearch.frustumCulledFirst(
        frustum, octree, cameraPos);

        if (firstIntersection != null) {
            // Should be the closest intersection
            List<FrustumCullingSearch.FrustumIntersection<String>> allIntersections = FrustumCullingSearch.frustumCulledAll(
            frustum, octree, cameraPos);

            assertFalse(allIntersections.isEmpty());
            assertEquals(allIntersections.get(0).content, firstIntersection.content);
            assertEquals(allIntersections.get(0).distanceToCamera, firstIntersection.distanceToCamera, 0.001f);
        }
    }
    ...
    void testFrustumIntersectionDataIntegrity() {
        ...
        Frustum3D frustum = Frustum3D.createPerspective(cameraPos, lookAt, up, (float) Math.PI / 3.0f, 1.0f, 50.0f,
                                                        400.0f);
    ...
    void testGetFrustumCullingStatistics() {
        ...
        assertEquals(stats.getVisiblePercentage(), stats.getInsidePercentage() + stats.getIntersectingPercentage(),
                     0.001);
    ...
    void testHasAnyIntersection() {
        ...
        Frustum3D intersectingFrustum = Frustum3D.createPerspective(cameraPos, lookAt, up, (float) Math.PI / 2.0f, 1.0f,
                                                                    25.0f, 600.0f);

        // Very narrow frustum looking in a direction with no data
        Frustum3D nonIntersectingFrustum = Frustum3D.createPerspective(new Point3f(100.0f, 100.0f, 100.0f),
                                                                       new Point3f(1000.0f, 1000.0f, 100.0f), up,
    ...
    void testNegativeCameraPositionThrowsException() {
        ...
        Frustum3D frustum = Frustum3D.createPerspective(new Point3f(100.0f, 100.0f, 100.0f), lookAt, up,
    ...
    void testOrthographicFrustumCulling() {
        ...
        Frustum3D orthogonalFrustum = Frustum3D.createOrthographic(cameraPos, lookAt, up, 50.0f, 150.0f, 50.0f, 150.0f,
                                                                   50.0f, 400.0f);

        List<FrustumCullingSearch.FrustumIntersection<String>> intersections = FrustumCullingSearch.frustumCulledAll(
        orthogonalFrustum, octree, cameraPos);
