...
/**
 * Comprehensive test suite for TetRayTracingSearch
 * Tests ray-tetrahedron intersection search functionality using Möller-Trumbore algorithm
 * 
...
public class TetRayTracingSearchTest {
    ...
    void testRayDirections() {
        ...
        Vector3f[] directions = {
            new Vector3f(1.0f, 0.0f, 0.0f),  // X-axis
    ...
    void testConsistencyBetweenMethods() {
        ...
        if (!allIntersections.isEmpty()) {
            ...
            assertEquals(allIntersections.get(0).distance, firstIntersection.distance, 1e-6f,
                        "Distance should match");
        } else {
    ...
    void testZeroLengthRay() {
        ...
        assertEquals(1.0f, ray.direction().length(), 1e-6f,
                    "Ray direction should be normalized to unit length");
