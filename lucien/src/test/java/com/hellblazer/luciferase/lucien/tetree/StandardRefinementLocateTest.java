...
public class StandardRefinementLocateTest {
    ...
    /**
     * Simple containment check for grid-aligned tetrahedra. Since childStandard creates grid-aligned children, we can
     * use a simple bounding box check for this prototype.
     */
    private static boolean containsPoint(Tet tet, float px, float py, float pz) {
        int cellSize = Constants.lengthAtLevel(tet.l());
        return px >= tet.x() && px <= tet.x() + cellSize && py >= tet.y() && py <= tet.y() + cellSize && pz >= tet.z()
        && pz <= tet.z() + cellSize;
    }

    /**
     * Locate a tetrahedron containing a point using standard refinement hierarchy. This method follows the refinement
     * path from root to find the correct tetrahedron.
     */
    ...
    public void testStandardRefinementLocate() {
        ...
        int[][] testPoints = { { 0, 0, 0 }, { cellSize, 0, 0 }, { 0, cellSize, 0 }, { cellSize, cellSize, 0 },
                               { 0, 0, cellSize }, { cellSize, 0, cellSize }, { 0, cellSize, cellSize },
        ...
        for (int i = 0; i < testPoints.length; i++) {
            ...
            boolean match = standard.type() == expected.type() && standard.x() == expected.x()
            && standard.y() == expected.y() && standard.z() == expected.z();

            System.out.printf("%5d | (%6.0f,%6.0f,%6.0f) | Type %d at (%7d,%7d,%7d) | Type %d at %-15s | %s\n", i, x, y,
                              z, standard.type(), standard.x(), standard.y(), standard.z(), expected.type(),
                              String.format("(%d,%d,%d)", expected.x(), expected.y(), expected.z()),
                              match ? "YES" : "NO");
        }
        ...
        System.out.printf("\nTesting point (%.0f, %.0f, %.0f) at different levels:%n", testX, testY, testZ);
        ...
        for (byte testLevel = 0; testLevel <= 5; testLevel++) {
            ...
            System.out.printf("%5d | Type %d at (%d, %d, %d)\n", testLevel, result.type(), result.x(), result.y(),
                              result.z());
        }
    }
}
