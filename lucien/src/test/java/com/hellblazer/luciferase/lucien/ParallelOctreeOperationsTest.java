...
/**
 * Unit tests for parallel octree operations
...
public class ParallelOctreeOperationsTest {
    ...
    void testParallelConfigCreation() {
        ParallelOctreeOperations.ParallelConfig defaultConfig = 
            ParallelOctreeOperations.ParallelConfig.defaultConfig();
        
        assertEquals(1000, defaultConfig.parallelismThreshold);
        assertNull(defaultConfig.customThreadPool);
        
        ParallelOctreeOperations.ParallelConfig customConfig = 
            ParallelOctreeOperations.ParallelConfig.withCustomPool(500, 8);
    ...
    void testBoundedByParallelSmallDataset() {
        ...
        List<Octree.Hexahedron<String>> sequentialResults = 
            smallOctree.boundedBy(searchVolume).toList();
        
        List<Octree.Hexahedron<String>> parallelResults = 
            ParallelOctreeOperations.boundedByParallel(searchVolume, smallOctree, parallelConfig);
        
        assertEquals(sequentialResults.size(), parallelResults.size());
        
        // Results should contain the same elements (order may differ)
        Set<String> sequentialContents = sequentialResults.stream()
            .map(h -> h.cell()).collect(Collectors.toSet());
        Set<String> parallelContents = parallelResults.stream()
            .map(h -> h.cell()).collect(Collectors.toSet());
    ...
    void testBoundedByParallelLargeDataset() {
        ...
        List<Octree.Hexahedron<String>> sequentialResults = 
            largeOctree.boundedBy(searchVolume).toList();
        
        List<Octree.Hexahedron<String>> parallelResults = 
            ParallelOctreeOperations.boundedByParallel(searchVolume, largeOctree, parallelConfig);
    ...
    void testBoundingParallelCorrectness() {
        ...
        List<Octree.Hexahedron<String>> sequentialResults = 
            largeOctree.bounding(searchVolume).toList();
        
        List<Octree.Hexahedron<String>> parallelResults = 
            ParallelOctreeOperations.boundingParallel(searchVolume, largeOctree, parallelConfig);
    ...
    void testKNearestNeighborsParallelCorrectness() {
        ...
        List<KNearestNeighborSearch.KNNCandidate<String>> sequentialResults = 
            KNearestNeighborSearch.findKNearestNeighbors(queryPoint, k, largeOctree);
        
        List<KNearestNeighborSearch.KNNCandidate<String>> parallelResults = 
            ParallelOctreeOperations.kNearestNeighborsParallel(queryPoint, k, largeOctree, parallelConfig);
    ...
    void testRayIntersectedAllParallelCorrectness() {
        ...
        List<RayTracingSearch.RayIntersection<String>> sequentialResults = 
            RayTracingSearch.rayIntersectedAll(ray, largeOctree);
        
        List<RayTracingSearch.RayIntersection<String>> parallelResults = 
            ParallelOctreeOperations.rayIntersectedAllParallel(ray, largeOctree, parallelConfig);
    ...
    void testBatchKNearestNeighbors() {
        List<Point3f> queryPoints = Arrays.asList(
            new Point3f(100000.0f, 100000.0f, 32.0f),
        ...
        Map<Point3f, List<KNearestNeighborSearch.KNNCandidate<String>>> batchResults = 
            ParallelOctreeOperations.batchKNearestNeighbors(queryPoints, k, largeOctree, parallelConfig);
    ...
    void testSmallBatchUsesSequentialProcessing() {
        ...
        List<Point3f> smallBatch = Arrays.asList(
            new Point3f(100000.0f, 100000.0f, 32.0f)
        );
        int k = 2;
        
        Map<Point3f, List<KNearestNeighborSearch.KNNCandidate<String>>> batchResults = 
            ParallelOctreeOperations.batchKNearestNeighbors(smallBatch, k, smallOctree, parallelConfig);
    ...
    void testParallelOperationsWithCustomThreadPool() {
        ...
        ParallelOctreeOperations.ParallelConfig customConfig = 
            ParallelOctreeOperations.ParallelConfig.withCustomPool(5, 2);
        
        Point3f queryPoint = new Point3f(500000.0f, 500000.0f, 32.0f);
        
        List<KNearestNeighborSearch.KNNCandidate<String>> results = 
            ParallelOctreeOperations.kNearestNeighborsParallel(queryPoint, 3, largeOctree, customConfig);
    ...
    void testParallelOperationsWithCommonPool() {
        ...
        ParallelOctreeOperations.ParallelConfig commonPoolConfig = 
            new ParallelOctreeOperations.ParallelConfig(5, null);
        
        Point3f queryPoint = new Point3f(500000.0f, 500000.0f, 32.0f);
        
        List<KNearestNeighborSearch.KNNCandidate<String>> results = 
            ParallelOctreeOperations.kNearestNeighborsParallel(queryPoint, 3, largeOctree, commonPoolConfig);
    ...
    void testEmptyOctreeParallelOperations() {
        ...
        List<Octree.Hexahedron<String>> results = 
            ParallelOctreeOperations.boundedByParallel(searchVolume, emptyOctree, parallelConfig);
    ...
    void testThresholdBehavior() {
        ...
        List<KNearestNeighborSearch.KNNCandidate<String>> smallResults = 
            ParallelOctreeOperations.kNearestNeighborsParallel(queryPoint, 2, smallOctree, parallelConfig);
        ...
        List<KNearestNeighborSearch.KNNCandidate<String>> largeResults = 
            ParallelOctreeOperations.kNearestNeighborsParallel(queryPoint, 5, largeOctree, parallelConfig);
