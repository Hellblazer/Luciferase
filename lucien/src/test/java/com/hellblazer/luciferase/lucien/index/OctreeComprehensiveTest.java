...
/**
 * Comprehensive tests for Octree implementation using Morton curve encoding.
 * Tests spatial indexing, cubic space-filling curve properties, and geometric consistency.
 * 
...
public class OctreeComprehensiveTest {
    ...
    void testCubeLocationAndGeometry() {
        ...
        for (byte level = 5; level <= 15; level++) {
            ...
            assertTrue(cube.originX() % length == 0, "X should be quantized to grid");
            assertTrue(cube.originY() % length == 0, "Y should be quantized to grid");
            assertTrue(cube.originZ() % length == 0, "Z should be quantized to grid");
    ...
    void testMultipleInsertionsAndSpatialDistribution() {
        ...
        Point3f[] points = {
            new Point3f(length * 1.5f, length * 1.5f, length * 1.5f),
            new Point3f(length * 2.5f, length * 2.5f, length * 2.5f),
            new Point3f(length * 3.5f, length * 3.5f, length * 3.5f),
            new Point3f(length * 4.5f, length * 5.5f, length * 6.5f),
    ...
    void testMortonCurveOrderingProperties() {
        ...
        Point3f[] nearbyPoints = {
            new Point3f(1001.0f, 1000.0f, 1000.0f), // +X
