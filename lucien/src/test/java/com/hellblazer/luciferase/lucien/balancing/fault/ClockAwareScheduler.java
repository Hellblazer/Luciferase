...
/**
 * Scheduler that uses injected Clock for deterministic timing.
 *
 * <p>Unlike ScheduledExecutorService which uses wall-clock time,
 * this scheduler delegates timing decisions to an injected Clock.
 * This enables deterministic testing via TestClock.
 *
 * <p><b>Usage Pattern</b>:
 * <pre>
 * // Production: uses system time
 * var scheduler = new ClockAwareScheduler(Clock.system(), 1000, task);
 * scheduler.start();
 *
 * // Testing: manual tick advancement
 * var testClock = new TestClock(0);
 * var scheduler = new ClockAwareScheduler(testClock, 1000, task);
 * scheduler.start();
 *
...
public class ClockAwareScheduler {
     ...
     *
     * @param clock the clock for timing decisions
     * @param intervalMs execution interval in milliseconds
     * @param task the task to execute periodically
     * @throws NullPointerException if clock or task is null
     * @throws IllegalArgumentException if intervalMs is not positive
     */
    public ClockAwareScheduler(Clock clock, long intervalMs, Runnable task) {
        ...
        this.lastExecutionTime = clock.currentTimeMillis();
    }
     ...
     * TestClock.advance() for deterministic testing.
     *
     ...
     * Start scheduling.
     *
     * <p>Initializes lastExecutionTime to current clock time.
     * Subsequent tick() calls will execute task when interval elapses.
     */
    public void start() {
        running = true;
        lastExecutionTime = clock.currentTimeMillis();
    }
