...
public class OctreeBulkPerformanceTest {
    ...
    void testBulkInsertPerformanceComparison() {
        ...
        new BulkOperationConfig().withDeferredSubdivision(true).withPreSortByMorton(false));
        ...
        System.out.printf("Sequential insertion:      %8.2f ms (%6.2f μs/op)%n", sequentialMs,
                          sequentialMs * 1000 / entityCount);
        System.out.printf("Default bulk:              %8.2f ms (%6.2f μs/op) - %.2fx speedup%n", defaultMs,
                          defaultMs * 1000 / entityCount, sequentialMs / defaultMs);
        System.out.printf("Bulk without Morton sort:  %8.2f ms (%6.2f μs/op) - %.2fx speedup%n", noSortMs,
                          noSortMs * 1000 / entityCount, sequentialMs / noSortMs);
        System.out.printf("Optimized bulk:            %8.2f ms (%6.2f μs/op) - %.2fx speedup%n", optimizedMs,
                          optimizedMs * 1000 / entityCount, sequentialMs / optimizedMs);
        ...
        if (currentSpeedup < targetSpeedup) {
            System.out.printf("Current: %.2fx, Target: %.2fx, Gap: %.2fx%n", currentSpeedup, targetSpeedup,
                              targetSpeedup - currentSpeedup);
    ...
    void testInsertBatchVsInsertAll() {
        ...
        BulkOperationConfig configWithIds = new BulkOperationConfig().withDeferredSubdivision(true).withPreSortByMorton(
        true).withStackBasedBuilder(true).withStackBuilderThreshold(5000);
        ...
        if (!batchIds.isEmpty()) {
            ...
            System.out.println(
            "Note: ID tracking was disabled for performance, but entities were successfully inserted");
        }
