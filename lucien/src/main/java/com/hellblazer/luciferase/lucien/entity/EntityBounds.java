...
/**
 * Represents the bounding box of an entity for spanning calculations.
 * Entities with bounds can span multiple octree nodes.
 * 
...
public class EntityBounds {
    ...
    public boolean intersectsCube(float cubeX, float cubeY, float cubeZ, float cubeSize) {
        ...
        return !(max.x < cubeX || min.x > cubeMaxX ||
                 max.y < cubeY || min.y > cubeMaxY ||
                 max.z < cubeZ || min.z > cubeMaxZ);
    }
    ...
    public boolean containedInCube(float cubeX, float cubeY, float cubeZ, float cubeSize) {
        ...
        return min.x >= cubeX && max.x <= cubeMaxX &&
               min.y >= cubeY && max.y <= cubeMaxY &&
               min.z >= cubeZ && max.z <= cubeMaxZ;
    }
    
    @Override
    public String toString() {
        return String.format("EntityBounds[min=(%.2f,%.2f,%.2f), max=(%.2f,%.2f,%.2f)]",
