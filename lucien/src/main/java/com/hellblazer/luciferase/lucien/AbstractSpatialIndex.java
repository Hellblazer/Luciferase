...
/**
 * Abstract base class for spatial index implementations. Provides common functionality
 * for entity management, configuration, and basic spatial operations while allowing
 * concrete implementations to specialize the spatial decomposition strategy.
 *
...
public abstract class AbstractSpatialIndex<ID extends EntityID, Content, NodeType extends SpatialNodeStorage<ID>> 
     ...
     */
    protected AbstractSpatialIndex(EntityIDGenerator<ID> idGenerator, int maxEntitiesPerNode, 
                                   byte maxDepth, EntitySpanningPolicy spanningPolicy) {
     ...
     */
    protected abstract Stream<Map.Entry<Long, NodeType>> spatialRangeQuery(VolumeBounds bounds, boolean includeIntersecting);
    ...
    public int nodeCount() {
        return (int) getSpatialIndex().values().stream()
    ...
    public Stream<SpatialNode<ID>> nodes() {
        return getSpatialIndex().entrySet().stream()
    ...
    public Stream<SpatialNode<ID>> boundedBy(Spatial volume) {
        ...
        return spatialRangeQuery(bounds, false)
