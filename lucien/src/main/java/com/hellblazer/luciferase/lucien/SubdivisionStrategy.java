...
/**
 * Abstract base class for subdivision strategies that determine how nodes should be subdivided
 * when they reach capacity. Based on sophisticated control flow strategies from high-performance
 * C++ spatial index implementations.
 * 
 * Different strategies can be used to optimize for different use cases:
 * - Dense point clouds: aggressive subdivision
 * - Large entities: selective subdivision with spanning
 * - Mixed workloads: adaptive strategies
 *
...
public abstract class SubdivisionStrategy<ID extends EntityID, Content> {
    ...
    public enum ControlFlow {
        /**
         * Keep entity in parent node without subdivision.
         * Used when subdivision would not improve performance.
         */
        INSERT_IN_PARENT,
        
        /**
         * Split entity across multiple child nodes.
         * Used for large entities that span multiple spatial regions.
         */
        SPLIT_TO_CHILDREN,
        
        /**
         * Create only the necessary child nodes.
         * Optimizes memory by creating children on-demand.
         */
        CREATE_SINGLE_CHILD,
        
        /**
         * Perform complete rebalancing of the subtree.
         * Used when tree becomes unbalanced.
         */
        FULL_REBALANCING,
        
        /**
         * Defer subdivision until bulk operation completes.
         * Optimizes bulk insertions.
         */
        DEFER_SUBDIVISION,
        
        /**
         * Force immediate subdivision regardless of other factors.
         * Used when node is critically overloaded.
         */
        FORCE_SUBDIVISION
    }
     ...
     */
    public static class SubdivisionContext<ID extends EntityID> {
        ...
        public SubdivisionContext(long nodeIndex, byte nodeLevel, int currentNodeSize,
                                 int maxEntitiesPerNode, boolean isBulkOperation,
                                 EntityBounds newEntityBounds, List<ID> existingEntities,
                                 byte maxDepth) {
    ...
    public static class SubdivisionResult {
        ...
        public final Set<Long> targetNodes;
        public final boolean shouldSpanEntity;
        public final String reason;
        
        public SubdivisionResult(ControlFlow decision, Set<Long> targetNodes, 
                               boolean shouldSpanEntity, String reason) {
        ...
        public static SubdivisionResult splitToChildren(Set<Long> targetNodes, String reason) {
            return new SubdivisionResult(ControlFlow.SPLIT_TO_CHILDREN, targetNodes, true, reason);
        }
        
        public static SubdivisionResult createSingleChild(Long targetNode, String reason) {
            return new SubdivisionResult(ControlFlow.CREATE_SINGLE_CHILD, 
                                       Set.of(targetNode), false, reason);
        }
     ...
     */
    public abstract SubdivisionResult determineStrategy(SubdivisionContext<ID> context);
    
    /**
     * Calculate which child nodes an entity should belong to after subdivision
     */
    public abstract Set<Long> calculateTargetNodes(long parentIndex, byte parentLevel, 
     ...
     */
    protected double estimateSubdivisionBenefit(SubdivisionContext<ID> context) {
     ...
     */
    protected abstract double estimateEntitySizeFactor(SubdivisionContext<ID> context);
    
    /**
     * Check if an entity should span multiple nodes
     */
    protected boolean shouldSpanEntity(SubdivisionContext<ID> context, double nodeSizeEstimate) {
        if (context.newEntityBounds == null) {
            return false;
        }
        
        double entitySize = Math.max(
            Math.max(context.newEntityBounds.getMaxX() - context.newEntityBounds.getMinX(),
    ...
    public <T extends SubdivisionStrategy<ID, Content>> T withSpanningThreshold(double threshold) {
        this.spanningThreshold = threshold;
        return (T) this;
    }
    
    @SuppressWarnings("unchecked")
    public <T extends SubdivisionStrategy<ID, Content>> T withLoadFactor(double factor) {
        this.loadFactor = factor;
        return (T) this;
    }
    
    @SuppressWarnings("unchecked")
    public <T extends SubdivisionStrategy<ID, Content>> T withMinEntitiesForSplit(int minEntities) {
        this.minEntitiesForSplit = minEntities;
        return (T) this;
    }
    
    @SuppressWarnings("unchecked")
    public <T extends SubdivisionStrategy<ID, Content>> T withAdaptiveMode(boolean adaptive) {
        this.adaptiveMode = adaptive;
        return (T) this;
    }
    
    // Getters
    public double getSpanningThreshold() { return spanningThreshold; }
    public double getLoadFactor() { return loadFactor; }
    public int getMinEntitiesForSplit() { return minEntitiesForSplit; }
