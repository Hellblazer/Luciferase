...
/**
 * Optimized ray traversal for Tetree using space-filling curve properties.
 * Replaces brute-force traversal with efficient neighbor-based algorithm.
 * Based on t8code's ray traversal approach.
 * 
...
public class TetreeSFCRayTraversal<ID extends EntityID, Content> {
    ...
    /**
     * Traverse the tetree along a ray using SFC properties for efficiency.
     * Returns tetrahedra indices in the order they are intersected by the ray.
     * 
    ...
    private boolean rayIntersectsTetrahedron(Ray3D ray, Tet tet) {
        ...
        return rayIntersectsFace(ray, verts[0], verts[1], verts[2]) ||
               rayIntersectsFace(ray, verts[0], verts[1], verts[3]) ||
               rayIntersectsFace(ray, verts[0], verts[2], verts[3]) ||
               rayIntersectsFace(ray, verts[1], verts[2], verts[3]) ||
               tet.contains(ray.origin());
    }
     ...
     */
    private void addIntersectedNeighbors(Tet current, Ray3D ray, 
                                       Queue<Tet> toProcess, Set<Long> visited) {
        // Check all 4 faces
     ...
     */
    private void addIntersectedChildren(Tet current, Ray3D ray, 
                                      Queue<Tet> toProcess, Set<Long> visited) {
        for (int i = 0; i < 8; i++) {
            try {
    ...
    /**
     * Quick test if ray could possibly intersect a tetrahedron.
     ...
     * Ray-AABB intersection test.
     */
    private boolean rayIntersectsAABB(Ray3D ray, float minX, float minY, float minZ,
