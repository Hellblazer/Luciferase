...
/**
 * Estimates the number of nodes that will be created for a given entity count and distribution.
 * Based on the C++ implementation's EstimateNodeNumber algorithm.
 *
...
public class NodeEstimator {
    ...
    public static class SpatialDistribution {
        ...
        public float getClusteringFactor() { return clusteringFactor; }
        public int getClusterCount() { return clusterCount; }
        public float getSurfaceThickness() { return surfaceThickness; }
        
        // Predefined common distributions
        public static final SpatialDistribution UNIFORM = new SpatialDistribution(Type.UNIFORM);
        public static final SpatialDistribution CLUSTERED_MODERATE = new SpatialDistribution(Type.CLUSTERED, 0.5f, 10, 0.1f);
        public static final SpatialDistribution CLUSTERED_HIGH = new SpatialDistribution(Type.CLUSTERED, 0.8f, 20, 0.1f);
        public static final SpatialDistribution SURFACE_THIN = new SpatialDistribution(Type.SURFACE_ALIGNED, 0.3f, 1, 0.01f);
    }
     ...
     */
    public static int estimateFromSamples(int totalEntityCount, int sampleSize, 
                                         int uniqueCellsInSample, int maxEntitiesPerNode) {
        if (sampleSize <= 0 || uniqueCellsInSample <= 0) {
            return estimateNodeCount(totalEntityCount, maxEntitiesPerNode, (byte)21, 
