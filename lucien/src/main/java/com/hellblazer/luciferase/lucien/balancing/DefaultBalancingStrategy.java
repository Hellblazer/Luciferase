...
public class DefaultBalancingStrategy<ID extends EntityID> implements TreeBalancingStrategy<ID> {
     ...
     */
    public DefaultBalancingStrategy(double mergeFactor, double splitFactor, 
                                  double imbalanceThreshold, long minRebalancingInterval) {
    ...
    public boolean shouldRebalanceTree(TreeBalancingStats stats) {
        ...
        double emptyPercentage = stats.totalNodes() > 0 ? 
            (double) stats.emptyNodes() / stats.totalNodes() : 0;
    ...
    /**
     * Helper to calculate max entities for a given level.
     * Higher levels (closer to root) can hold more entities.
     */
