...
/**
 * Strategy interface for tree balancing operations. Defines when and how nodes should be split, merged, or rebalanced.
 *
...
public interface TreeBalancingStrategy<ID extends EntityID> {
    ...
    /**
     * Calculate the merge threshold for a given level. Nodes with fewer entities than this threshold are candidates for
     * merging.
     *
    ...
    /**
     * Calculate the split threshold for a given level. Nodes with more entities than this threshold should be split.
     *
     * @param nodeLevel          the level of the node
     * @param maxEntitiesPerNode maximum allowed entities per node
     ...
     */
    record TreeBalancingStats(int totalNodes, int underpopulatedNodes, int overpopulatedNodes, int emptyNodes,
                              int maxDepth, double averageEntityLoad, double loadVariance) {
        /**
         * Calculate the load imbalance factor (0.0 = perfectly balanced, 1.0 = highly imbalanced)
         */
        public double imbalanceFactor() {
            if (averageEntityLoad == 0) {
                return 0;
            }
