 ...
 * 
 * This class provides low-level bit manipulation utilities for working with
 * tetrahedral space-filling curve indices and coordinates. All methods are
 * static for maximum performance.
 * 
...
public final class TetreeBits {
     ...
     * - Bits 8-28:  X coordinate (21 bits)
     * - Bits 29-49: Y coordinate (21 bits)
     * - Bits 50-70: Z coordinate (21 bits) - Note: uses 71 bits total, need adjustment
     * 
    ...
    public static long packTet(Tet tet) {
        ...
        packed |= ((long)(tet.type() & TYPE_MASK)) << TYPE_SHIFT;
    ...
    /**
     * Extract the refinement level from a space-filling curve index.
     * This is more efficient than Tet.tetLevelFromIndex for repeated operations.
     * 
    ...
    /**
     * Extract the type of a tetrahedron from its SFC index and level.
     * This requires partial decoding of the SFC index.
     * 
     * @param sfcIndex the space-filling curve index
    ...
    /**
     * Calculate the parent coordinate from a child coordinate at a given level.
     * Uses bit manipulation to clear the appropriate low-order bit.
     * 
    ...
    /**
     * Fast comparison of two tetrahedra using their SFC indices.
     * Returns negative if tet1 < tet2, zero if equal, positive if tet1 > tet2.
     * 
    ...
    /**
     * Compute the child ID (0-7) of a tetrahedron within its parent.
     * 
     * @param tet the tetrahedron
     * @return child ID in Bey's ordering (0-7)
     */
    public static byte computeChildId(Tet tet) {
        if (tet.l() == 0) {
            throw new IllegalArgumentException("Root tetrahedron is not a child");
        }
        
        // Get the parent tetrahedron
        Tet parent = tet.parent();
        
        // Search through parent's children to find which one matches this tet
        for (byte childId = 0; childId < 8; childId++) {
            byte childType = TetreeConnectivity.getChildType(parent.type(), childId);
            if (childType == tet.type()) {
                // Check if coordinates match
                Tet candidateChild = parent.child(childId);
                if (candidateChild.x() == tet.x() && 
                    candidateChild.y() == tet.y() && 
                    candidateChild.z() == tet.z()) {
                    return childId;
                }
            }
        }
        
        throw new IllegalStateException("Failed to compute child ID");
    }
    ...
    /**
     * Compute the XOR of two tetrahedra's coordinates.
     * Useful for finding the highest differing bit (depth of common ancestor).
     * 
    ...
    public static long coordinateXor(Tet tet1, Tet tet2) {
        ...
        result |= ((long)(xorY & COORD_MASK)) << COORD_BITS;
        result |= ((long)(xorZ & COORD_MASK)) << (2 * COORD_BITS);
        
        return result;
    }
    
    /**
     * Find the level of the lowest common ancestor of two tetrahedra.
     * Based on t8code's t8_dtri_nearest_common_ancestor algorithm.
     * 
     * @param tet1 first tetrahedron
     * @param tet2 second tetrahedron
    ...
    public static byte lowestCommonAncestorLevel(Tet tet1, Tet tet2) {
        ...
        byte c_level = (byte)Math.min(Constants.getMaxRefinementLevel() - maxlevel,
                                      Math.min(tet1.l(), tet2.l()));
    ...
    /**
     * Compute a hash code for a tetrahedron that preserves spatial locality.
     * Tetrahedra that are close in space will have similar hash codes.
     * 
     * @param tet the tetrahedron
