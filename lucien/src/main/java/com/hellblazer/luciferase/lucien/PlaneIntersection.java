...
/**
 * Represents the result of a plane intersection query with an entity in the spatial index.
 * Contains the entity information, content, distance from plane, closest point on entity to plane,
 ...
 */
public record PlaneIntersection<ID extends EntityID, Content>(
    ID entityId,
    Content content,
    ...
    /**
     * Compare plane intersections by absolute distance from plane.
     * Entities closer to the plane are considered "smaller" and will appear first in sorted collections.
     *
    ...
    public boolean isOnPositiveSide() {
        return intersectionType == IntersectionType.POSITIVE_SIDE 
    ...
    public boolean isOnNegativeSide() {
        return intersectionType == IntersectionType.NEGATIVE_SIDE 
            || intersectionType == IntersectionType.INTERSECTING 
            || intersectionType == IntersectionType.ON_PLANE;
    }
    ...
    public String toString() {
        return String.format("PlaneIntersection[entity=%s, distance=%.3f, type=%s, point=%s]",
                           entityId, distanceFromPlane, intersectionType, closestPoint);
    }
}