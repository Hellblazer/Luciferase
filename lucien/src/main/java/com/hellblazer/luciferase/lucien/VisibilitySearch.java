...
/**
 * Multi-entity aware visibility search for Octree. Performs line-of-sight tests, occlusion queries, and visibility
 * analysis with entity awareness.
 *
...
public class VisibilitySearch {
     ...
     */
    private static float calculateEntityOcclusionRatio(Point3f entityPos, EntityBounds bounds, Ray3D ray,
    ...
    Point3f observer, float maxViewDistance, Octree<ID, Content> octree) {
        ...
        for (Map.Entry<ID, Point3f> entry : entitiesWithPositions.entrySet()) {
            ...
            // Test visibility
            EntityLineOfSightResult<ID, Content> losResult = testLineOfSight(observer, entityPos, octree, 0.01f);
        ...
        float averageVisibilityRatio = (visibleEntities + partiallyOccludedEntities) > 0 ? totalVisibilityRatio / (
        visibleEntities + partiallyOccludedEntities) : 0.0f;

        // Find most significant occluders
        List<ID> topOccluders = occluderCounts.entrySet().stream().sorted(
        Map.Entry.<ID, Integer>comparingByValue().reversed()).limit(5).map(Map.Entry::getKey).collect(
        Collectors.toList());

        return new EntityVisibilityStatistics(totalEntities, visibleEntities, partiallyOccludedEntities,
     ...
     */
    private static VisibilityType classifyEntityVisibility(Point3f entityPos, EntityBounds bounds, Point3f observer,
                                                           Point3f target, float intersectionDistance,
                                                           float totalDistance) {
    ...
    ID targetEntityId, List<Point3f> candidatePositions, Octree<ID, Content> octree) {
        ...
        for (Point3f candidate : candidatePositions) {
            EntityLineOfSightResult<ID, Content> losResult = testLineOfSight(candidate, targetPos, octree, 0.1f);
            ...
            boolean selfOccluding = losResult.occludingEntities.stream().anyMatch(e -> e.id.equals(targetEntityId));

            EntityVantagePoint<ID> vantagePoint = new EntityVantagePoint<>(candidate, targetEntityId, distance,
                                                                           losResult.hasLineOfSight || selfOccluding,
                                                                           losResult.totalOcclusionRatio,
                                                                           visibilityScore,
    ...
    Point3f observer, Point3f target, Octree<ID, Content> octree) {
    ...
    Point3f observer, Vector3f viewDirection, float viewAngle, float maxViewDistance, Octree<ID, Content> octree) {
        ...
        for (Map.Entry<ID, Point3f> entry : entitiesWithPositions.entrySet()) {
            ...
            Vector3f toEntity = new Vector3f(entityPos.x - observer.x, entityPos.y - observer.y,
            ...
            if (angle <= viewAngle) {
                ...
                if (losResult.hasLineOfSight || isEntitySelfVisible(entityId, losResult)) {
                    ...
                    if (content != null) {
                        ...
                        EntityVisibilityResult<ID, Content> result = new EntityVisibilityResult<>(entityId, content,
                                                                                                  entityPos,
     ...
     */
    private static <ID extends EntityID, Content> boolean isEntitySelfVisible(ID entityId,
                                                                              EntityLineOfSightResult<ID, Content> losResult) {
        return losResult.occludingEntities.size() == 1 && losResult.occludingEntities.get(0).id.equals(entityId);
    }
     ...
     */
    public static <ID extends EntityID, Content> EntityLineOfSightResult<ID, Content> testLineOfSight(Point3f observer,
                                                                                                      Point3f target,
                                                                                                      Octree<ID, Content> octree,
        ...
        for (Map.Entry<ID, Point3f> entry : entitiesWithPositions.entrySet()) {
            ...
            RayIntersection intersection = bounds != null ? testRayBoundsIntersection(sightRay, bounds)
                                                          : testRayPointIntersection(sightRay, entityPos,
                                                                                     occlusionThreshold);
            ...
            if (intersection.intersects && intersection.distance < totalDistance) {
                ...
                float occlusionRatio = calculateEntityOcclusionRatio(entityPos, bounds, sightRay, intersection.distance,
                                                                     totalDistance);

                if (occlusionRatio > 0) {
                    VisibilityType visType = classifyEntityVisibility(entityPos, bounds, observer, target,
                                                                      intersection.distance, totalDistance);

                    if (visType == VisibilityType.PARTIALLY_OCCLUDING || visType == VisibilityType.FULLY_OCCLUDING) {
                        ...
                        if (content != null) {
                            ...
                            float occluderThickness = bounds != null ? calculateBoundsThicknessAlongRay(bounds,
                                                                                                        sightRay)
                                                                     : occlusionThreshold;
        ...
        return new EntityLineOfSightResult<>(hasLineOfSight, occludingEntities, totalOcclusionRatio,
    ...
    private static RayIntersection testRayPointIntersection(Ray3D ray, Point3f point, float radius) {
        Vector3f toPoint = new Vector3f(point.x - ray.origin().x, point.y - ray.origin().y, point.z - ray.origin().z);
    ...
    public static class EntityVisibilityResult<ID extends EntityID, Content> {
        ...
        public EntityVisibilityResult(ID id, Content content, Point3f position, float distanceFromObserver,
                                      float distanceFromTarget, VisibilityType visibilityType, float occlusionRatio) {
            this.id = id;
    ...
    public static class EntityVisibilityStatistics {
        ...
        public EntityVisibilityStatistics(long totalEntities, long visibleEntities, long partiallyOccludedEntities,
                                          long fullyOccludedEntities, long entitiesOutOfRange,
    ...
    public static class EntityVantagePoint<ID extends EntityID> {
        ...
        public EntityVantagePoint(Point3f position, ID targetEntityId, float distanceToTarget, boolean hasLineOfSight,
                                  float occlusionRatio, float visibilityScore, int occluderCount) {
            this.position = position;
