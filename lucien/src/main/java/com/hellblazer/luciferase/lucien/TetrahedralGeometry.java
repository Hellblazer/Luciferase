...
/**
 * Specialized geometric predicates and operations for tetrahedral spatial structures.
 * Provides precise geometric computations for ray-tetrahedron intersection, 
 * frustum-tetrahedron intersection, and other tetrahedral-specific operations.
 * 
...
public class TetrahedralGeometry {
     ...
     */
    public static record Ray3D(Point3f origin, Vector3f direction, float maxDistance) {
        public Ray3D {
     ...
     */
    public static record Plane3D(Point3f point, Vector3f normal) {
        public Plane3D {
    ...
    public static class Frustum3D {
        ...
        public Frustum3D(Point3f position, Vector3f forward, Vector3f up, Vector3f right,
            ...
            Point3f farCenter = new Point3f(
                position.x + far * forward.x,
                position.y + far * forward.y,
                position.z + far * forward.z
            );
            
            // Near and far planes
            planes[0] = new Plane3D(new Point3f(position.x + near * forward.x, 
                                               position.y + near * forward.y, 
                                               position.z + near * forward.z), 
            ...
            leftNormal.cross(up, new Vector3f(forward.x + right.x * halfWidth / far, 
                                            forward.y + right.y * halfWidth / far, 
                                            forward.z + right.z * halfWidth / far));
            leftNormal.normalize();
            planes[2] = new Plane3D(position, leftNormal);
            
            Vector3f rightNormal = new Vector3f();
            rightNormal.cross(new Vector3f(forward.x - right.x * halfWidth / far, 
                                         forward.y - right.y * halfWidth / far, 
            ...
            topNormal.cross(right, new Vector3f(forward.x + up.x * halfHeight / far, 
                                              forward.y + up.y * halfHeight / far, 
                                              forward.z + up.z * halfHeight / far));
            topNormal.normalize();
            planes[4] = new Plane3D(position, topNormal);
            
            Vector3f bottomNormal = new Vector3f();
            bottomNormal.cross(new Vector3f(forward.x - up.x * halfHeight / far, 
                                          forward.y - up.y * halfHeight / far, 
    ...
    public static RayTetrahedronIntersection rayIntersectsTetrahedron(Ray3D ray, long tetIndex) {
        ...
        if (closestIntersection != null) {
            return new RayTetrahedronIntersection(true, closestDistance, closestIntersection, closestNormal, closestFace);
        } else {
    ...
    private static boolean tetrahedraShareSpace(Point3i[] vertices1, Point3i[] vertices2) {
        ...
        // AABB overlap test
        return !(max1X < min2X || min1X > max2X ||
                 max1Y < min2Y || min1Y > max2Y ||
                 max1Z < min2Z || min1Z > max2Z);
    }
}