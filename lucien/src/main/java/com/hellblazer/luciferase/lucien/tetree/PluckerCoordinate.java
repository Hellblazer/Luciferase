...
/**
 * Plücker coordinate representation for lines and rays in 3D space.
 * Based on the algorithm from Platis & Theoharis (2003) "Fast Ray-Tetrahedron Intersection Using Plücker Coordinates".
 * 
 * Plücker coordinates represent a ray r as πᵣ = {L : L × P} = {Uᵣ : Vᵣ} where:
 * - U is the direction vector 
 * - V is the direction × point cross product
 * 
 * The key property is the permuted inner product: πᵣ ⊙ πₛ = Uᵣ · Vₛ + Uₛ · Vᵣ
 * which determines spatial orientation between two lines.
 *
...
public class PluckerCoordinate {
    ...
    /**
     * Compute the permuted inner product between two Plücker coordinates.
     * This is the fundamental operation for Plücker-based intersection tests.
     * 
     * Formula: πᵣ ⊙ πₛ = Uᵣ · Vₛ + Uₛ · Vᵣ
     * 
     * Geometric interpretation:
     * - > 0: s goes counterclockwise around r
     * - < 0: s goes clockwise around r  
     * - = 0: s intersects or is parallel to r
     * 
    ...
    /**
     * Get the sign of the permuted inner product (-1, 0, or 1).
     * Used in the Plücker ray-tetrahedron intersection algorithm.
     * 
     * @param other The other Plücker coordinate
    ...
    public String toString() {
        return String.format("PluckerCoord{U=(%f,%f,%f), V=(%f,%f,%f)}", 
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof PluckerCoordinate other)) return false;
        
        final float EPSILON = 1e-6f;
        return Math.abs(U.x - other.U.x) < EPSILON && Math.abs(U.y - other.U.y) < EPSILON && 
               Math.abs(U.z - other.U.z) < EPSILON && Math.abs(V.x - other.V.x) < EPSILON && 
               Math.abs(V.y - other.V.y) < EPSILON && Math.abs(V.z - other.V.z) < EPSILON;
    }
