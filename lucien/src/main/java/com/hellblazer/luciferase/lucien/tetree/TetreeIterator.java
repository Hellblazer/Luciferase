...
/**
 * Iterator for traversing a Tetree using space-filling curve properties.
 * Supports multiple traversal orders and level-restricted iteration.
 * Based on t8code's tree traversal algorithms.
 * 
...
public class TetreeIterator<ID extends EntityID, Content> implements Iterator<TetreeNodeImpl<ID>> {
    ...
    public TetreeIterator(Tetree<ID, Content> tree, TraversalOrder order, 
                         byte minLevel, byte maxLevel, boolean includeEmpty) {
        this.tree = tree;
        this.order = order;
        ...
        this.stack = (order == TraversalOrder.DEPTH_FIRST_PRE || 
                     order == TraversalOrder.DEPTH_FIRST_POST) ? new ArrayDeque<>() : null;
    ...
    /**
     * Skip the entire subtree rooted at the current node.
     * Only valid for depth-first traversals.
     */
    public void skipSubtree() {
        if (order != TraversalOrder.DEPTH_FIRST_PRE && 
            order != TraversalOrder.DEPTH_FIRST_POST) {
            throw new UnsupportedOperationException(
                "skipSubtree only supported for depth-first traversals");
        }
        skipSubtree = true;
    }
     ...
     */
    public static <ID extends EntityID, Content> TetreeIterator<ID, Content> depthFirstPre(
            Tetree<ID, Content> tree) {
     ...
     */
    public static <ID extends EntityID, Content> TetreeIterator<ID, Content> breadthFirst(
            Tetree<ID, Content> tree) {
     ...
     */
    public static <ID extends EntityID, Content> TetreeIterator<ID, Content> sfcOrder(
            Tetree<ID, Content> tree) {
