...
/**
 * Optimized tmIndex implementation that processes bits during parent walk for better CPU cache utilization.
 *
...
public class TetOptimized {
     ...
     */
    private static TetreeKey<?> buildTmIndexFromChain(Tet tet, Tet[] parentChain) {
        ...
        if (tet.l() <= 10) {
            ...
            result = new ExtendedTetreeKey(tet.l(), lowBits, highBits);
        }
    ...
    /**
     * Optimized tmIndex that processes bits during parent walk to improve cache locality. Instead of: 1. Walk parent
     * chain to collect types 2. Build bit pattern from types
     *
     * This does: 1. Walk parent chain AND build bits incrementally
     *
     * This keeps data in CPU cache while processing.
     */
    public static TetreeKey<?> tmIndexOptimized(Tet tet) {
     ...
     */
    public static TetreeKey<?> tmIndexOptimizedV2(Tet tet) {
        if (tet.l() == 0) {
            return Tet.ROOT_TET;
        }
    ...
    /**
     * Production-ready optimized tmIndex that uses better parent walk ordering. This version minimizes cache misses by
     * processing the parent chain in memory order.
     */
    public static TetreeKey<?> tmIndexOptimizedV3(Tet tet) {
        // Fast path for cached result
        var cached = TetreeLevelCache.getCachedTetreeKey(tet.x(), tet.y(), tet.z(), tet.l(), tet.type());
        if (cached != null) {
            return cached;
        ...
        for (int level = tet.l() - 1; level >= 0; level--) {
            ...
            if (level > 0) {
                TetreeLevelCache.cacheParent(parent.x(), parent.y(), parent.z(), parent.l(), parent.type(),
                                             parentChain[level - 1]);
            }
        }
