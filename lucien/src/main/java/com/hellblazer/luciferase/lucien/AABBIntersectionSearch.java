...
/**
 * Axis-Aligned Bounding Box (AABB) intersection search implementation for Octree
 * Finds octree cubes that intersect with 3D AABBs, with results ordered by distance from reference point
...
public class AABBIntersectionSearch {
    ...
    public static class AABBIntersection<Content> {
        ...
        public AABBIntersection(long index, Content content, Spatial.Cube cube, 
    ...
    public static <Content> List<AABBIntersection<Content>> aabbIntersectedAll(
            AABB aabb, Octree<Content> octree, Point3f referencePoint) {
        ...
        List<AABBIntersection<Content>> intersections = new ArrayList<>();
        ...
        for (Map.Entry<Long, Content> entry : map.entrySet()) {
            ...
            if (intersectionType != IntersectionType.COMPLETELY_OUTSIDE) {
                ...
                AABBIntersection<Content> intersection = new AABBIntersection<>(
                    entry.getKey(), 
                    entry.getValue(), 
                    cube, 
                    distance,
     ...
     * @throws IllegalArgumentException if reference point has negative coordinates
     */
    public static <Content> List<AABBIntersection<Content>> cubesCompletelyInside(
            AABB aabb, Octree<Content> octree, Point3f referencePoint) {
        
        return aabbIntersectedAll(aabb, octree, referencePoint).stream()
     ...
     * @throws IllegalArgumentException if reference point has negative coordinates
     */
    public static <Content> List<AABBIntersection<Content>> cubesPartiallyIntersecting(
            AABB aabb, Octree<Content> octree, Point3f referencePoint) {
     ...
     * @throws IllegalArgumentException if reference point has negative coordinates
     */
    public static <Content> List<AABBIntersection<Content>> cubesContainingAABB(
            AABB aabb, Octree<Content> octree, Point3f referencePoint) {
    ...
    /**
     * Count the number of cubes that intersect with the AABB
    ...
    /**
     * Test if any cube in the octree intersects with the AABB
    ...
    public static class IntersectionStatistics {
        ...
        public IntersectionStatistics(long totalCubes, long insideCubes, long intersectingCubes, 
                                    long containingCubes, long outsideCubes) {
        ...
        public double getIntersectedPercentage() {
            return totalCubes > 0 ? (double) (insideCubes + intersectingCubes + containingCubes) / totalCubes * 100.0 : 0.0;
        }
        
        @Override
        public String toString() {
            return String.format("AABBIntersectionStats[total=%d, inside=%d(%.1f%%), intersecting=%d(%.1f%%), containing=%d(%.1f%%), outside=%d(%.1f%%), intersected=%.1f%%]",
                               totalCubes, insideCubes, getInsidePercentage(), 
                               intersectingCubes, getIntersectingPercentage(),
                               containingCubes, getContainingPercentage(),
    ...
    public static <Content> Map<AABB, List<AABBIntersection<Content>>> 
            batchAABBIntersections(List<AABB> aabbs, Octree<Content> octree, Point3f referencePoint) {
        ...
        return aabbs.stream()
            .collect(Collectors.toMap(
                aabb -> aabb,
                aabb -> aabbIntersectedAll(aabb, octree, referencePoint)
            ));
    }
