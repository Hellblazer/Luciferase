...
/**
 * Represents a 3D plane using the equation ax + by + cz + d = 0 All coordinate operations are constrained to positive
 * values only
 *
...
public record Plane3D(float a, float b, float c, float d) {
    ...
    /**
     * Create a plane from three points (all coordinates must be positive) Points must not be collinear
     *
    ...
    /**
     * Calculate the signed distance from a point to this plane Positive distance means the point is on the side of the
    ...
    /**
     * Test if this plane intersects with an axis-aligned bounding box Uses the separating axis theorem for plane-AABB
     * intersection
     *
