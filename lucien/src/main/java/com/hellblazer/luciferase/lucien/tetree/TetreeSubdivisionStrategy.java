...
/**
 * Tetree-specific subdivision strategy implementation.
 * Optimized for tetrahedral spatial decomposition.
 * 
 * Key features:
 * - Tetrahedral geometry-aware subdivision
 * - Support for entity spanning across multiple tetrahedra
 * - Adaptive subdivision based on tetrahedral volume
 * - Efficient child tetrahedron calculation
 *
 ...
 */
public class TetreeSubdivisionStrategy<ID extends EntityID, Content> extends SubdivisionStrategy<ID, Content> {
        ...
        if (context.newEntityBounds != null) {
            ...
            if (targetChild != null) {
                return SubdivisionResult.createSingleChild(targetChild, 
                    "Entity fits in single child tetrahedron");
            }
        }
    ...
    public Set<Long> calculateTargetNodes(long parentIndex, byte parentLevel, 
        ...
        for (int i = 0; i < TETREE_CHILDREN; i++) {
            ...
            if (entityBoundsIntersectsTetrahedron(entityBounds, vertices)) {
                targetNodes.add(childTet.index());
            }
        }
    ...
    private boolean entityBoundsIntersectsTetrahedron(EntityBounds bounds, Point3f[] tetVertices) {
        ...
        return bounds.getMaxX() >= minX && bounds.getMinX() <= maxX &&
