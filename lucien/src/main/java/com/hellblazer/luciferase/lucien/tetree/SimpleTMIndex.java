...
/**
 * Simple implementation of the Tetrahedral Morton Index (TM-index) based on the bit-interleaving scheme described in
 * the tetrahedral space-filling curve papers.
 *
 * The TM-index interleaves coordinate bits with type information to create a Morton-like encoding for tetrahedra. Each
 * level contributes 6 bits: - 3 bits for spatial position (x, y, z interleaved) - 3 bits for tetrahedral type (0-5)
 *
...
public class SimpleTMIndex {
    ...
    /**
     * Create a simple TM-index that just encodes coordinates and type without the full hierarchical type computation.
     *
     * This version treats each level independently and doesn't track how types change through the hierarchy.
     */
