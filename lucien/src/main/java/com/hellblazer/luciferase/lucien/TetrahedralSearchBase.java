...
/**
 * Base class for tetrahedral search operations providing common geometric operations
 * and utilities for handling multiple simplicies per spatial region.
 * 
 * All operations are constrained to positive coordinates only, as required by
 * the tetrahedral space-filling curve implementation.
 * 
...
public abstract class TetrahedralSearchBase {
    ...
    /**
     * Group of simplicies representing the same spatial region
     * (up to 6 tetrahedra per cubic region in tetrahedral decomposition)
     */
    public static class SimplexGroup<Content> {
        ...
        private static <Content> float computeGroupVolume(List<Tetree.Simplex<Content>> simplicies) {
            return (float) simplicies.stream()
                .mapToDouble(simplex -> tetrahedronVolume(simplex.index()))
                .sum();
        }
     ...
     */
    protected static <Content> Tetree.Simplex<Content> selectRepresentativeSimplex(List<Tetree.Simplex<Content>> simplicies) {
        if (simplicies.isEmpty()) {
            return null;
        }
    ...
    protected static <Content> List<SimplexGroup<Content>> groupSimpliciesBySpatialProximity(
        ...
        Map<String, List<Tetree.Simplex<Content>>> spatialGroups = simplicies
            .collect(Collectors.groupingBy(simplex -> {
        ...
        return spatialGroups.values().stream()
            .filter(group -> !group.isEmpty())
            .map(SimplexGroup::new)
    ...
    protected static void validatePositiveCoordinates(float x, float y, float z) {
        if (x < 0 || y < 0 || z < 0) {
            throw new IllegalArgumentException("Coordinates must be positive for tetrahedral operations: (" + x + ", " + y + ", " + z + ")");
        }
    }
}