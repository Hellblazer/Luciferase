...
public class EntityCollectorVisitor<ID extends EntityID, Content> extends AbstractTreeVisitor<ID, Content> {
     ...
     * Entity match record.
     */
    public record EntityMatch<ID extends EntityID, Content>(
        ID entityId,
        Content content,
        long nodeIndex,
        int level
    ...
    @Override
    public boolean visitNode(com.hellblazer.luciferase.lucien.SpatialIndex.SpatialNode<ID> node, 
    ...
    public List<ID> getEntityIds() {
        return collectedEntities.stream()
            .map(EntityMatch::entityId)
            .toList();
    }
