...
public class VONLeaveProtocol {
     ...
     */
    public VONLeaveProtocol(SpatialNeighborIndex index, Consumer<VONEvent> eventEmitter) {
     ...
     */
    public void leave(VONNode leaver) {
        ...
        eventEmitter.accept(new VONEvent.Leave(leaver.id()));
    }
    ...
    public void handleCrash(UUID crashedId, UUID detectedBy) {
        ...
        VONNode crashed = index.get(crashedId);
        if (crashed != null) {
            // Treat crash as forced leave
            leave(crashed);

            // Emit CRASH event (in addition to LEAVE event from leave())
            eventEmitter.accept(new VONEvent.Crash(crashedId, detectedBy));
        }
    }
