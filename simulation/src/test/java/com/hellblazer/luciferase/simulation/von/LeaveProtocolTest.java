 ...
 */
public class LeaveProtocolTest {
     ...
     * <p>
     * Validates: Event.Leave emitted on departure
     */
    @Test
    public void testLeaveEmitsEvent() {
        ...
        var leaveEvent = capturedEvents.stream()
            .filter(e -> e instanceof Event.Leave)
            .map(e -> (Event.Leave) e)
    ...
    public void testCrashedBubbleDetected() {
        ...
        assertTrue(capturedEvents.stream()
                       .anyMatch(e -> e instanceof Event.Crash &&
                                     ((Event.Crash) e).nodeId().equals(crashedNode.id())),
