 ...
 * <p>
 * These tests MUST pass before implementing VONJoinProtocol (TDD red phase).
 *
 * @author hal.hildebrand
 */
public class VONJoinProtocolTest {
    ...
    public void testJoinViaFirefliesMember() {
        ...
        assertTrue(capturedEvents.stream()
                       .anyMatch(e -> e instanceof VONEvent.Join &&
                                     ((VONEvent.Join) e).nodeId().equals(joiner.id())),
    ...
    public void testNewBubbleEstablishesSync() {
        ...
        List<VONEvent> existingEvents = new ArrayList<>();
        var existingWithEvents = new BubbleVONNode(existingBubble, existingEvents::add);
    ...
    public void testJoinWithExistingCluster() {
        // Create 10-bubble cluster
        List<VONNode> cluster = new ArrayList<>();
        for (int i = 0; i < 10; i++) {
